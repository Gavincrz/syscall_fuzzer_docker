...
user *ACLCreateUser(const char *name, size_t namelen) {
    ...
    ((u->passwords)->match = (ACLListMatchSds));
    ((u->passwords)->free = (ACLListFreeSds));
    ((u->passwords)->dup = (ACLListDupSds));
    ((u->patterns)->match = (ACLListMatchSds));
    ((u->patterns)->free = (ACLListFreeSds));
    ((u->patterns)->dup = (ACLListDupSds));
...
user *ACLCreateUnlinkedUser(void) {
    ...
    for (int j = 0; ; j++) {
        ...
        ((retval != 0)?(void)0 : (_serverAssert("retval != 0","acl.c",209),_exit(1)));
        return fakeuser;
    }
}
...
int ACLGetUserCommandBit(user *u, unsigned long id) {
    uint64_t word, bit;
    if (ACLGetCommandBitCoordinates(id,&word,&bit) == -1) return 0;
...
int ACLUserCanExecuteFutureCommands(user *u) {
    return ACLGetUserCommandBit(u,1024-1);
}
...
void ACLSetUserCommandBit(user *u, unsigned long id, int value) {
    uint64_t word, bit;
    if (value == 0) u->flags &= ~(1<<3);
    if (ACLGetCommandBitCoordinates(id,&word,&bit) == -1) return;
    if (value)
...
sds ACLDescribeUserCommandRules(user *u) {
                        ...
                        sizeof(u->allowed_commands)) != 0)
    {
        serverLog(3,
            ...
            rules);
        _serverPanic("acl.c",473,"No bitmap match in ACLDescribeUserCommandRules()"),_exit(1);
    }
    return rules;
}
...
sds ACLDescribeUser(user *u) {
    ...
    for (int j = 0; ACLUserFlags[j].flag; j++) {
        ...
        if (ACLUserFlags[j].flag == (1<<2) ||
            ACLUserFlags[j].flag == (1<<3)) continue;
...
void ACLResetSubcommands(user *u) {
    if (u->allowed_subcommands == ((void *)0)) return;
    for (int j = 0; j < 1024; j++) {
...
void ACLAddAllowedSubcommand(user *u, unsigned long id, const char *sub) {
    ...
    if (u->allowed_subcommands == ((void *)0)) {
        u->allowed_subcommands = zcalloc(1024 *
                                 sizeof(sds*));
    }
...
int ACLSetUser(user *u, const char *op, ssize_t oplen) {
               ...
               !strcasecmp(op,"-@all"))
    {
        ...
        if (ln == ((void *)0)) listAddNodeTail(u->passwords,newpass);
        u->flags &= ~(1<<4);
    } else if (op[0] == '<') {
        ...
        if (ln == ((void *)0)) listAddNodeTail(u->patterns,newpat);
        u->flags &= ~(1<<2);
    } else if (op[0] == '+' && op[1] != '@') {
        if (strchr(op,'|') == ((void *)0)) {
        ...
        int bitval = op[0] == '+' ? 1 : 0;
        if (ACLSetUserCommandBitsForCategory(u,op+2,bitval) == -1) {
            errno = 2;
            return -1;
        }
    } else if (!strcasecmp(op,"reset")) {
        ((ACLSetUser(u,"resetpass",-1) == 0)?(void)0 : (_serverAssert("ACLSetUser(u,\"resetpass\",-1) == C_OK","acl.c",794),_exit(1)));
        ((ACLSetUser(u,"resetkeys",-1) == 0)?(void)0 : (_serverAssert("ACLSetUser(u,\"resetkeys\",-1) == C_OK","acl.c",795),_exit(1)));
        ((ACLSetUser(u,"off",-1) == 0)?(void)0 : (_serverAssert("ACLSetUser(u,\"off\",-1) == C_OK","acl.c",796),_exit(1)));
        ((ACLSetUser(u,"-@all",-1) == 0)?(void)0 : (_serverAssert("ACLSetUser(u,\"-@all\",-1) == C_OK","acl.c",797),_exit(1)));
    } else {
        errno = 22;
        return -1;
    }
...
sds ACLDefaultUserFirstPassword(void) {
    if (((DefaultUser->passwords)->len) == 0) return ((void *)0);
    listNode *first = ((DefaultUser->passwords)->head);
    return ((first)->value);
}
...
int ACLAuthenticateUser(client *c, robj *username, robj *password) {
    if (ACLCheckUserCredentials(username,password) == 0) {
        c->authenticated = 1;
...
int ACLAppendUserForLoading(sds *argv, int argc, int *argc_err) {
    ...
    for (int j = 2; j < argc; j++) {
        if (ACLSetUser(fakeuser,argv[j],sdslen(argv[j])) == -1) {
            if (errno != 2) {
...
int ACLLoadConfiguredUsers(void) {
    ...
    while ((ln = listNext(&li)) != ((void *)0)) {
        sds *aclrules = ((ln)->value);
        sds username = aclrules[0];
        user *u = ACLCreateUser(username,sdslen(username));
        if (!u) {
            u = ACLGetUserByName(username,sdslen(username));
            ((u != ((void *)0))?(void)0 : (_serverAssert("u != NULL","acl.c",1097),_exit(1)));
            ACLSetUser(u,"reset",-1);
        }

         
        for (int j = 1; aclrules[j]; j++) {
            if (ACLSetUser(u,aclrules[j],sdslen(aclrules[j])) != 0) {
                char *errmsg = ACLSetUserStringError();
                serverLog(3,"Error loading ACL rule '%s' for "
                                     "the user named '%s': %s",
        ...
        if (u->flags & (1<<1)) {
            serverLog(2, "The user '%s' is disabled (there is no "
...
sds ACLLoadFromFile(const char *filename) {
    ...
    for (int i = 0; i < totlines; i++) {
        ...
        int j;
        for (j = 2; j < argc; j++) {
            if (ACLSetUser(fakeuser,argv[j],sdslen(argv[j])) != 0) {
                char *errmsg = ACLSetUserStringError();
                errors = sdscatprintf(errors,
                         "%s:%d: %s. ",
        ...
        if (!u) {
            ...
            ((u != ((void *)0))?(void)0 : (_serverAssert("u != NULL","acl.c",1246),_exit(1)));
            ACLSetUser(u,"reset",-1);
        }
        ...
        for (j = 2; j < argc; j++)
            ((ACLSetUser(u,argv[j],sdslen(argv[j])) == 0)?(void)0 : (_serverAssert("ACLSetUser(u,argv[j],sdslen(argv[j])) == C_OK","acl.c",1253),_exit(1)));
    ...
    if (sdslen(errors) == 0) {
        ...
        ((new != ((void *)0))?(void)0 : (_serverAssert("new != NULL","acl.c",1268),_exit(1)));
...
int ACLSaveToFile(const char *filename) {
    ...
    if ((fd = open(tmpfilename,01|0100,0644)) == -1) {
        serverLog(3,"Opening temp ACL file for ACL SAVE: %s",
            strerror(errno));
        goto cleanup;
    }
    ...
    if (write(fd,acl,sdslen(acl)) != (ssize_t)sdslen(acl)) {
        serverLog(3,"Writing ACL file for ACL SAVE: %s",
            strerror(errno));
        goto cleanup;
    }
    close(fd); fd = -1;
    ...
    if (rename(tmpfilename,filename) == -1) {
        serverLog(3,"Renaming ACL file for ACL SAVE: %s",
            strerror(errno));
        goto cleanup;
    }
...
void ACLLoadUsersAtStartup(void) {
    if (server.acl_filename[0] != '\0' && ((UsersToLoad)->len) != 0) {
        serverLog(3,
...
void aclCommand(client *c) {
    ...
    if (!strcasecmp(sub,"setuser") && c->argc >= 3) {
        ...
        for (int j = 3; j < c->argc; j++) {
            if (ACLSetUser(tempu,c->argv[j]->ptr,sdslen(c->argv[j]->ptr)) != 0) {
                char *errmsg = ACLSetUserStringError();
        ...
        ((u != ((void *)0))?(void)0 : (_serverAssert("u != NULL","acl.c",1423),_exit(1)));
        ACLCopyUser(u, tempu);
        ACLFreeUser(tempu);
        ...
        addReplyArrayLen(c,((u->passwords)->len));
        listIter li;
        listNode *ln;
        listRewind(u->passwords,&li);
        while((ln = listNext(&li))) {
        ...
        if (u->flags & (1<<2)) {
            ...
            addReplyArrayLen(c,((u->patterns)->len));
            listIter li;
            listNode *ln;
            listRewind(u->patterns,&li);
            while((ln = listNext(&li))) {
               ...
               (!strcasecmp(sub,"load") || !strcasecmp(sub,"save")))
    {
        ...
        if (ACLSaveToFile(server.acl_filename) == 0) {
            addReply(c,shared.ok);
        } else {
...
void authCommand(client *c) {
    ...
    if (ACLAuthenticateUser(c,username,password) == 0) {
        addReply(c,shared.ok);
    } else {
