...
int clientSubscriptionsCount(client *c) {
    return ((c->pubsub_channels)->ht[0]. used+(c->pubsub_channels)->ht[1]. used)+
           ((c->pubsub_patterns)->len);
}
...
int pubsubSubscribeChannel(client *c, robj *channel) {
    ...
    if (dictAdd(c->pubsub_channels,channel,((void *)0)) == 0) {
        retval = 1;
...
int pubsubUnsubscribeChannel(client *c, robj *channel, int notify) {
    ...
    if (dictDelete(c->pubsub_channels,channel) == 0) {
        retval = 1;
         
        de = dictFind(server.pubsub_channels,channel);
        ((de != ((void *)0))?(void)0 : (_serverAssertWithInfo(c,((void *)0),"de != NULL","pubsub.c",185),_exit(1)));
        clients = ((de)->v . val);
        ln = listSearchKey(clients,c);
        ((ln != ((void *)0))?(void)0 : (_serverAssertWithInfo(c,((void *)0),"ln != NULL","pubsub.c",188),_exit(1)));
...
void unsubscribeCommand(client *c) {
    ...
    if (clientSubscriptionsCount(c) == 0) c->flags &= ~(1<<18);
}
...
((void *)0)
        ...
        (c->argc == 2 || c->argc == 3))
    {
        ...
        for (j = 2; j < c->argc; j++) {
            ...
            addReplyLongLong(c,l ? ((l)->len) : 0);
        }
    } else if (!strcasecmp(c->argv[1]->ptr,"numpat") && c->argc == 2) {
         
        addReplyLongLong(c,((server . pubsub_patterns)->len));
    } else {
