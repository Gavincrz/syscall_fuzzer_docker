...
int decodeGeohash(double bits, double *xy) {
    GeoHashBits hash = { .bits = (uint64_t)bits, .step = 26 };
...
int extractLongLatOrReply(client *c, robj **argv, double *xy) {
    ...
    if (xy[0] < -180 || xy[0] > 180 ||
        xy[1] < -85.05112878  || xy[1] > 85.05112878) {
...
int longLatFromMember(robj *zobj, robj *member, double *xy) {
    ...
    if (zsetScore(zobj, member->ptr, &score) == -1) return -1;
    if (!decodeGeohash(score, xy)) return -1;
    return 0;
}
...
int geoGetPointsInRange(robj *zobj, double min, double max, double lon, double lat, double radius, geoArray *ga) {
    ...
    if (zobj->encoding == 5) {
        ...
        if ((eptr = zzlFirstInRange(zl, &range)) == ((void *)0)) {
        ...
        while (eptr) {
            ...
            member = (vstr == ((void *)0)) ? sdsfromlonglong(vlong) :
                                      sdsnewlen(vstr,vlen);
        ...
        if ((ln = zslFirstInRange(zsl, &range)) == ((void *)0)) {
...
int membersOfAllNeighbors(robj *zobj, GeoHashRadius n, double lon, double lat, double radius, geoArray *ga) {
    ...
    for (i = 0; i < sizeof(neighbors) / sizeof(*neighbors); i++) {
        if ((!(neighbors[i]). bits && !(neighbors[i]). step)) {
            if (debugmsg) do { FILE *fp = fopen("/tmp/log.txt","a"); fprintf(fp,"%s:%s:%d:\t", "geo.c", __func__, 345); fprintf(fp,"neighbors[%d] is zero",i); fprintf(fp,"\n"); fclose(fp); } while (0);;
            continue;
        }
        ...
        if (debugmsg) {
            ...
            do { FILE *fp = fopen("/tmp/log.txt","a"); fprintf(fp,"%s:%s:%d:\t", "geo.c", __func__, 357); fprintf(fp,"neighbors[%d]:\n",i); fprintf(fp,"\n"); fclose(fp); } while (0);;
            do { FILE *fp = fopen("/tmp/log.txt","a"); fprintf(fp,"%s:%s:%d:\t", "geo.c", __func__, 358); fprintf(fp,"area.longitude.min: %f\n", myarea . longitude . min); fprintf(fp,"\n"); fclose(fp); } while (0);;
            do { FILE *fp = fopen("/tmp/log.txt","a"); fprintf(fp,"%s:%s:%d:\t", "geo.c", __func__, 359); fprintf(fp,"area.longitude.max: %f\n", myarea . longitude . max); fprintf(fp,"\n"); fclose(fp); } while (0);;
            do { FILE *fp = fopen("/tmp/log.txt","a"); fprintf(fp,"%s:%s:%d:\t", "geo.c", __func__, 360); fprintf(fp,"area.latitude.min: %f\n", myarea . latitude . min); fprintf(fp,"\n"); fclose(fp); } while (0);;
            do { FILE *fp = fopen("/tmp/log.txt","a"); fprintf(fp,"%s:%s:%d:\t", "geo.c", __func__, 361); fprintf(fp,"area.latitude.max: %f\n", myarea . latitude . max); fprintf(fp,"\n"); fclose(fp); } while (0);;
            do { FILE *fp = fopen("/tmp/log.txt","a"); fprintf(fp,"%s:%s:%d:\t", "geo.c", __func__, 362); fprintf(fp,"\n"); fprintf(fp,"\n"); fclose(fp); } while (0);;
        }
        ...
        {
            if (debugmsg)
                do { FILE *fp = fopen("/tmp/log.txt","a"); fprintf(fp,"%s:%s:%d:\t", "geo.c", __func__, 374); fprintf(fp,"Skipping processing of %d, same as previous\n",i); fprintf(fp,"\n"); fclose(fp); } while (0);;
            continue;
        }
...
void geoaddCommand(client *c) {
    ...
    for (i = 0; i < elements; i++) {
        ...
        if (extractLongLatOrReply(c, (c->argv+2)+(i*3),xy) == -1) {
            for (i = 0; i < argc; i++)
        ...
        robj *score = createObject(0, sdsfromlonglong(bits));
...
void georadiusGeneric(client *c, int flags) {
    ...
    if (flags & (1<<0)) {
        base_args = 6;
        if (extractLongLatOrReply(c, c->argv + 2, xy) == -1)
            return;
    } else if (flags & (1<<1)) {
        base_args = 5;
        robj *member = c->argv[2];
        if (longLatFromMember(zobj, member, xy) == -1) {
    ...
    if (storekey == ((void *)0)) {
        ...
        for (i = 0; i < returned_items; i++) {
            ...
            ((dictAdd(zs->dict,gp->member,&znode ->score) == 0)?(void)0 : (_serverAssert("dictAdd(zs->dict,gp->member,&znode->score) == DICT_OK","geo.c",654),_exit(1)));
            gp->member = ((void *)0);
        }
        ...
        if (returned_items) {
            ...
            decrRefCount(zobj);
            notifyKeyspaceEvent((1<<7),"georadiusstore",storekey,
                                c->db->id);
            server.dirty += returned_items;
        } else if (dbDelete(c->db,storekey)) {
            signalModifiedKey(c->db,storekey);
            notifyKeyspaceEvent((1<<2),"del",storekey,c->db->id);
            server.dirty++;
        }
...
void georadiusCommand(client *c) {
    georadiusGeneric(c, (1<<0));
}

 
void georadiusbymemberCommand(client *c) {
    georadiusGeneric(c, (1<<1));
}

 
void georadiusroCommand(client *c) {
    georadiusGeneric(c, (1<<0)|(1<<2));
}

 
void georadiusbymemberroCommand(client *c) {
    georadiusGeneric(c, (1<<1)|(1<<2));
}
...
void geohashCommand(client *c) {
    ...
    if (zobj && checkType(c, zobj, 3)) return;
    ...
    for (j = 2; j < c->argc; j++) {
        double score;
        if (!zobj || zsetScore(zobj, c->argv[j]->ptr, &score) == -1) {
            addReplyNull(c);
        } else {
...
void geodistCommand(client *c) {
    ...
    if ((zobj = lookupKeyReadOrReply(c, c->argv[1], shared.null[c->resp]))
        == ((void *)0) || checkType(c, zobj, 3)) return;

     
    double score1, score2, xyxy[4];
    if (zsetScore(zobj, c->argv[2]->ptr, &score1) == -1 ||
        zsetScore(zobj, c->argv[3]->ptr, &score2) == -1)
    {
        addReplyNull(c);
        return;
    }
