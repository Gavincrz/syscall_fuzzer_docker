...
unsigned char *lpReplaceInteger(unsigned char *lp, unsigned char **pos, int64_t value) {
    char buf[21];
    int slen = ll2string(buf,sizeof(buf),value);
    return lpInsert(lp, (unsigned char*)buf, slen, *pos, 2, pos);
}
...
int64_t lpGetInteger(unsigned char *ele) {
    ...
    ((retval != 0)?(void)0 : (_serverAssert("retval != 0","t_stream.c",115),_exit(1)));
    v = ll;
    return v;
}
...
void streamLogListpackContent(unsigned char *lp) {
    ...
    while(p) {
        ...
        serverLog(3,"- [%d] '%.*s'", (int)v, (int)v, ele);
        p = lpNext(lp,p);
    }
}
...
int streamAppendItem(stream *s, robj **argv, int64_t numfields, streamID *added_id, streamID *use_id) {
    ...
    if (use_id && streamCompareID(use_id,&s->last_id) <= 0) return -1;
    ...
    if (lp == ((void *)0) || lp_bytes > server.stream_node_max_bytes) {
        ...
        ((ri . key_len == sizeof(rax_key))?(void)0 : (_serverAssert("ri.key_len == sizeof(rax_key)","t_stream.c",273),_exit(1)));
        ...
        if (numfields == master_fields_count) {
            ...
            if (i == master_fields_count) flags |= (1<<1);
        }
    }
...
int64_t streamTrimByLength(stream *s, size_t maxlen, int approx) {
    ...
    while(s->length > maxlen && raxNext(&ri)) {
        ...
        ((to_delete < entries)?(void)0 : (_serverAssert("to_delete < entries","t_stream.c",404),_exit(1)));
...
void streamIteratorStart(streamIterator *si, stream *s, streamID *start, streamID *end, int rev) {
    ...
    if (end) {
        ...
        si->end_key[0] = (18446744073709551615UL);
        si->end_key[1] = (18446744073709551615UL);
    }
...
int streamIteratorGetID(streamIterator *si, streamID *id, int64_t *numfields) {
    while(1) {  
        ...
        if (si->lp == ((void *)0) || si->lp_ele == ((void *)0)) {
            if (!si->rev && !raxNext(&si->ri)) return 0;
            else if (si->rev && !raxPrev(&si->ri)) return 0;
            ((si->ri . key_len == sizeof(streamID))?(void)0 : (_serverAssert("si->ri.key_len == sizeof(streamID)","t_stream.c",541),_exit(1)));
...
robj *createObjectFromStreamID(streamID *id) {
    return createObject(0, sdscatfmt(sdsempty(),"%U-%U",
                        id->ms,id->seq));
}
...
void streamPropagateXCLAIM(client *c, robj *key, streamCG *group, robj *groupname, robj *id, streamNACK *nack) {
    ...
    propagate(server.xclaimCommand,c->db->id,argv,14,1|2);
    decrRefCount(argv[0]);
    decrRefCount(argv[3]);
    decrRefCount(argv[4]);
    decrRefCount(argv[6]);
...
void streamPropagateGroupID(client *c, robj *key, streamCG *group, robj *groupname) {
    ...
    propagate(server.xgroupCommand,c->db->id,argv,5,1|2);
    decrRefCount(argv[0]);
    decrRefCount(argv[1]);
    decrRefCount(argv[4]);
}
...
#define STREAM_RWR_RAWENTRIES (1<<1)    #define STREAM_RWR_HISTORY (1<<2)        
    ...
    while(streamIteratorGetID(&si,&id,&numfields)) {
        ...
        if (group && !(flags & (1<<0))) {
            ...
            if (group_inserted == 0) {
                ...
                ((nack != raxNotFound)?(void)0 : (_serverAssert("nack != raxNotFound","t_stream.c",970),_exit(1)));
                ...
                _serverPanic("t_stream.c",979,"NACK half-created. Should not be possible."),_exit(1);
            }
...
int string2ull(const char *s, unsigned long long *value) {
    ...
    *value = strtoull(s,&endptr,10);
    if (errno == 22 || errno == 34 || !(*s != '\0' && *endptr == '\0'))
        return 0;  
    return 1;  
}
...
int streamGenericParseIDOrReply(client *c, robj *o, streamID *id, uint64_t missing_seq, int strict) {
    ...
    if (buf[0] == '-' && buf[1] == '\0') {
        ...
        id->ms = (18446744073709551615UL);
        id->seq = (18446744073709551615UL);
        return 0;
    }
...
void xaddCommand(client *c) {
    ...
    for (; i < c->argc; i++) {
        ...
        if (opt[0] == '*' && opt[1] == '\0') {
            ...
            if (streamParseStrictIDOrReply(c,c->argv[i],&id,0) != 0) return;
            id_given = 1;
            break;
        }
    }
    ...
    notifyKeyspaceEvent((1<<10),"xadd",c->argv[1],c->db->id);
    server.dirty++;

    if (maxlen >= 0) {
         
        if (streamTrimByLength(s,maxlen,approx_maxlen)) {
            notifyKeyspaceEvent((1<<10),"xtrim",c->argv[1],c->db->id);
        }
...
void xrangeGenericCommand(client *c, int rev) {
    ...
    if (streamParseIDOrReply(c,startarg,&startid,0) == -1) return;
    if (streamParseIDOrReply(c,endarg,&endid,(18446744073709551615UL)) == -1) return;
...
void xreadCommand(client *c) {
    ...
    streamCG **groups = ((void *)0);
    int xreadgroup = sdslen(c->argv[0]->ptr) == 10;  
    robj *groupname = ((void *)0);
    robj *consumername = ((void *)0);
    ...
    for (int i = 1; i < c->argc; i++) {
        ...
        if (!strcasecmp(o,"BLOCK") && moreargs) {
            ...
            i++;
            if (getLongLongFromObjectOrReply(c,c->argv[i],&count,((void *)0)) != 0)
                return;
            if (count < 0) count = 0;
    ...
    if (xreadgroup && groupname == ((void *)0)) {
    ...
    for (int i = streams_arg + streams_count; i < c->argc; i++) {
        ...
        if (o && checkType(c,o,6)) goto cleanup;
        ...
        if (strcmp(c->argv[i]->ptr,"$") == 0) {
            ...
            ids[id_idx].ms = (18446744073709551615UL);
            ids[id_idx].seq = (18446744073709551615UL);
            continue;
        }
        if (streamParseStrictIDOrReply(c,c->argv[i],ids+id_idx,0) != 0)
            goto cleanup;
    }
    ...
    for (int i = 0; i < streams_count; i++) {
        ...
        if (groups) {
            ...
            if (gt->ms != (18446744073709551615UL) ||
                gt->seq != (18446744073709551615UL))
            {
        ...
        if (serve_synchronously) {
            ...
            int flags = 0;
            if (noack) flags |= (1<<0);
            if (serve_history) flags |= (1<<2);
            streamReplyWithRange(c,s,&start,((void *)0),count,0,
                                 groups ? groups[i] : ((void *)0),
                                 consumer, flags, &spi);
    ...
    if (timeout != -1) {
        ...
        blockForKeys(c, 4, c->argv+streams_arg, streams_count,
                     timeout, ((void *)0), ids);
        

 
        c->bpop.xread_count = count ? count : 1000;
...
((void *)0)
    ...
    if (!strcasecmp(opt,"CREATE") && (c->argc == 5 || c->argc == 6)) {
        ...
        } else if (streamParseStrictIDOrReply(c,c->argv[4],&id,0) != 0) {
            return;
        }

         
        if (s == ((void *)0)) {
            ((mkstream)?(void)0 : (_serverAssert("mkstream","t_stream.c",1786),_exit(1)));
            o = createStreamObject();
        ...
        if (cg) {
            ...
            server.dirty++;
            notifyKeyspaceEvent((1<<10),"xgroup-create",
                                c->argv[2],c->db->id);
        } else {
            addReplySds(c,
        ...
        if (!strcmp(c->argv[4]->ptr,"$")) {
            id = s->last_id;
        } else if (streamParseIDOrReply(c,c->argv[4],&id,0) != 0) {
            return;
        }
        cg->last_id = id;
        addReply(c,shared.ok);
        server.dirty++;
        notifyKeyspaceEvent((1<<10),"xgroup-setid",c->argv[2],c->db->id);
        ...
        if (cg) {
            ...
            server.dirty++;
            notifyKeyspaceEvent((1<<10),"xgroup-destroy",
                                c->argv[2],c->db->id);
        } else {
        ...
        server.dirty++;
        notifyKeyspaceEvent((1<<10),"xgroup-delconsumer",
                            c->argv[2],c->db->id);
...
void xsetidCommand(client *c) {
    ...
    if (o == ((void *)0) || checkType(c,o,6)) return;

    stream *s = o->ptr;
    streamID id;
    if (streamParseStrictIDOrReply(c,c->argv[2],&id,0) != 0) return;
    ...
    addReply(c,shared.ok);
    server.dirty++;
    notifyKeyspaceEvent((1<<10),"xsetid",c->argv[1],c->db->id);
}
...
void xackCommand(client *c) {
    ...
    for (int j = 3; j < c->argc; j++) {
        streamID id;
        unsigned char buf[sizeof(streamID)];
        if (streamParseStrictIDOrReply(c,c->argv[j],&id,0) != 0) return;
        streamEncodeID(buf,&id);
...
void xpendingCommand(client *c) {
    ...
    if (c->argc >= 6) {
        if (getLongLongFromObjectOrReply(c,c->argv[5],&count,((void *)0)) == -1)
            return;
        if (count < 0) count = 0;
        if (streamParseIDOrReply(c,c->argv[3],&startid,0) == -1)
            return;
        if (streamParseIDOrReply(c,c->argv[4],&endid,(18446744073709551615UL)) == -1)
            return;
    }
...
void xclaimCommand(client *c) {
    ...
    for (j = 5; j < c->argc; j++) {
        streamID id;
        if (streamParseStrictIDOrReply(((void *)0),c->argv[j],&id,0) != 0) break;
    }
    int last_id_arg = j-1;  
    ...
    for (; j < c->argc; j++) {
        ...
        if (!strcasecmp(opt,"FORCE")) {
            ...
            j++;
            if (streamParseStrictIDOrReply(c,c->argv[j],&last_id,0) != 0) return;
        } else {
    ...
    for (int j = 5; j <= last_id_arg; j++) {
        streamID id;
        unsigned char buf[sizeof(streamID)];
        if (streamParseStrictIDOrReply(c,c->argv[j],&id,0) != 0)
            _serverPanic("t_stream.c",2237,"StreamID invalid after check. Should not be possible."),_exit(1);
        streamEncodeID(buf,&id);
...
((void *)0)
    ...
    if (!strcasecmp(opt,"CONSUMERS") && c->argc == 4) {
        ...
        end.ms = end.seq = (18446744073709551615UL);
        addReplyBulkCString(c,"first-entry");
        count = streamReplyWithRange(c,s,&start,&end,1,0,((void *)0),((void *)0),
                                     (1<<1),((void *)0));
        if (!count) addReplyNull(c);
        addReplyBulkCString(c,"last-entry");
        count = streamReplyWithRange(c,s,&start,&end,1,1,((void *)0),((void *)0),
                                     (1<<1),((void *)0));
        if (!count) addReplyNull(c);
    } else {
        addReplySubcommandSyntaxError(c);
