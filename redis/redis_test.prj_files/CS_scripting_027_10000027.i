...
void luaSortArray(lua_State *lua) {
    ...
    if (lua_pcall(lua,1,0,0)) {
        ...
        lua_gettable(lua,-2);        
        lua_pushvalue(lua,-3);       
        lua_getfield(lua, (-10002), ("__redis__compare_helper"));
...
void luaReplyToRedisReply(client *c, lua_State *lua) {
    ...
    switch(t) {
    case 4:
        addReplyBulkCBuffer(c,(char*)lua_tolstring(lua, (-1), ((void *)0)),lua_objlen(lua, (-1)));
        break;
    case 1:
    ...
    case 5:
        ...
        t = lua_type(lua,-1);
        if (t == 4) {
            sds err = sdsnew(lua_tolstring(lua, (-1), ((void *)0)));
            sdsmapchars(err,"\r\n","  ",2);
        ...
        t = lua_type(lua,-1);
        if (t == 4) {
            sds ok = sdsnew(lua_tolstring(lua, (-1), ((void *)0)));
            sdsmapchars(ok,"\r\n","  ",2);
            addReplySds(c,sdscatprintf(sdsempty(),"+%s\r\n",ok));
            sdsfree(ok);
            lua_settop(lua, -(1)-1);
...
int luaRedisGenericCommand(lua_State *lua, int raise_error) {
    ...
    static robj **argv = ((void *)0);
    static int argv_size = 0;
    static robj *cached_objects[32];
    static size_t cached_objects_len[32];
    static int inuse = 0;    

     
    if (server.lua_multi_emitted || (server.lua_caller->flags & (1<<3))) {
        c->flags |= (1<<3);
    } else {
        c->flags &= ~(1<<3);
    }
    ...
    if (inuse) {
        ...
        serverLog(3,"%s",recursion_warning);
    ...
    for (j = 0; j < argc; j++) {
        ...
        if (lua_type(lua,j+1) == 3) {
            ...
            if (obj_s == ((void *)0)) break;  
        }

         
        if (j < 32 && cached_objects[j] &&
            cached_objects_len[j] >= obj_len)
        {
            sds s = cached_objects[j]->ptr;
            argv[j] = cached_objects[j];
            cached_objects[j] = ((void *)0);
    ...
    if (server.maxmemory &&              
        ...
        (cmd->flags & (1ULL<<2)))
    {
        if (getMaxmemoryState(((void *)0),((void *)0),((void *)0),((void *)0)) != 0) {
            luaPushError(lua, shared.oomerr->ptr);
            goto cleanup;
        }
    }

    if (cmd->flags & (1ULL<<7)) server.lua_random_dirty = 1;
    if (cmd->flags & (1ULL<<0)) server.lua_write_dirty = 1;
        ...
        !(server.lua_caller->flags & (1<<1)))
    {
        ...
        c->flags &= ~((1<<17)|(1<<9));
        c->flags |= server.lua_caller->flags & ((1<<17)|(1<<9));
    ...
    if (server.lua_replicate_commands) {
        ...
        if (server.lua_repl & 1)
            call_flags |= (1<<2);
        if (server.lua_repl & 2)
            call_flags |= (1<<3);
    }
    call(c,call_flags);
    ...
    if (((c->reply)->len) == 0 && c->bufpos < (16*1024)) {
...
int luaRedisReturnSingleFieldTable(lua_State *lua, char *field) {
    if (lua_gettop(lua) != 1 || lua_type(lua,-1) != 4) {
...
void luaLoadLib(lua_State *lua, const char *libname, lua_CFunction luafunc) {
  lua_pushcclosure(lua, (luafunc), 0);
...
void luaLoadLibraries(lua_State *lua) {
    ...
    luaLoadLib(lua, "table", luaopen_table);
    luaLoadLib(lua, "string", luaopen_string);
    luaLoadLib(lua, "math", luaopen_math);
    luaLoadLib(lua, "debug", luaopen_debug);
...
void luaRemoveUnsupportedFunctions(lua_State *lua) {
    lua_pushnil(lua);
    lua_setfield(lua, (-10002), ("loadfile"));
    lua_pushnil(lua);
    lua_setfield(lua, (-10002), ("dofile"));
}
...
void scriptingInit(int setup) {
    ...
    lua_pushcclosure(lua, (luaRedisCallCommand), 0);
    lua_settable(lua,-3);

     
    lua_pushstring(lua,"pcall");
    lua_pushcclosure(lua, (luaRedisPCallCommand), 0);
    lua_settable(lua,-3);

     
    lua_pushstring(lua,"log");
    lua_pushcclosure(lua, (luaLogCommand), 0);
    lua_settable(lua,-3);
    ...
    lua_pushcclosure(lua, (luaRedisSha1hexCommand), 0);
    lua_settable(lua, -3);

     
    lua_pushstring(lua, "error_reply");
    lua_pushcclosure(lua, (luaRedisErrorReplyCommand), 0);
    lua_settable(lua, -3);
    lua_pushstring(lua, "status_reply");
    lua_pushcclosure(lua, (luaRedisStatusReplyCommand), 0);
    lua_settable(lua, -3);

     
    lua_pushstring(lua, "replicate_commands");
    lua_pushcclosure(lua, (luaRedisReplicateCommandsCommand), 0);
    lua_settable(lua, -3);

     
    lua_pushstring(lua,"set_repl");
    lua_pushcclosure(lua, (luaRedisSetReplCommand), 0);
    lua_settable(lua,-3);
    ...
    lua_pushcclosure(lua, (luaRedisBreakpointCommand), 0);
    lua_settable(lua,-3);

     
    lua_pushstring(lua,"debug");
    lua_pushcclosure(lua, (luaRedisDebugCommand), 0);
    lua_settable(lua,-3);

     
    lua_setfield(lua, (-10002), ("redis"));

     
    lua_getfield(lua, (-10002), ("math"));

    lua_pushstring(lua,"random");
    lua_pushcclosure(lua, (redis_math_random), 0);
    lua_settable(lua,-3);

    lua_pushstring(lua,"randomseed");
    lua_pushcclosure(lua, (redis_math_randomseed), 0);
    lua_settable(lua,-3);

    lua_setfield(lua, (-10002), ("math"));
...
int redis_math_random (lua_State *L) {
  ...
  lua_Number r = (lua_Number)(redisLrand48()%(2147483647)) /
                                (lua_Number)(2147483647);
  switch (lua_gettop(L)) {   
    case 0: {   
      lua_pushnumber(L, r);   
      break;
...
int redis_math_randomseed (lua_State *L) {
  redisSrand48(((int)luaL_checkinteger(L, (1))));
  return 0;
}
...
sds luaCreateFunction(client *c, lua_State *lua, robj *body) {
    ...
    ((retval == 0)?(void)0 : (_serverAssertWithInfo(c ? c : server . lua_client,((void *)0),"retval == DICT_OK","scripting.c",1262),_exit(1)));
...
void luaMaskCountHook(lua_State *lua, lua_Debug *ar) {
    ...
    if (elapsed >= server.lua_time_limit && server.lua_timedout == 0) {
        serverLog(3,"Lua slow script detected: still in execution after %lld milliseconds. You can try killing the script using the SCRIPT KILL command.",elapsed);
    ...
    if (server.lua_kill) {
        serverLog(3,"Lua script killed by user with SCRIPT KILL.");
...
void evalGenericCommand(client *c, int evalsha) {
    ...
    if (getLongLongFromObjectOrReply(c,c->argv[2],&numkeys,((void *)0)) != 0)
        return;
    if (numkeys > (c->argc - 3)) {
    ...
    lua_getfield(lua, (-10002), ("__redis__err__handler"));

     
    lua_getfield(lua, (-10002), (funcname));
    if ((lua_type(lua, (-1)) == 0)) {
        lua_settop(lua, -(1)-1);  
        ...
        lua_getfield(lua, (-10002), (funcname));
        ((!(lua_type(lua, (-1)) == 0))?(void)0 : (_serverAssert("!lua_isnil(lua,-1)","scripting.c",1374),_exit(1)));
    }
    ...
    if (server.lua_time_limit > 0 && ldb.active == 0) {
        lua_sethook(lua,luaMaskCountHook,(1 << 3),100000);
        delhook = 1;
    } else if (ldb.active) {
        lua_sethook(server.lua,luaLdbLineHook,(1 << 2)|(1 << 3),100000);
        delhook = 1;
    }
    ...
    if (err) {
        addReplyErrorFormat(c,"Error running script (call to %s): %s\n",
            funcname, lua_tolstring(lua, (-1), ((void *)0)));
        lua_settop(lua, -(2)-1);  
    } else {
    ...
    if (evalsha && !server.lua_replicate_commands) {
        if (!replicationScriptCacheExists(c->argv[1]->ptr)) {
            ...
            ((script != ((void *)0))?(void)0 : (_serverAssertWithInfo(c,((void *)0),"script != NULL","scripting.c",1479),_exit(1)));
...
((void *)0)
        ...
        } else if (server.lua_caller->flags & (1<<1)) {
...
void ldbInit(void) {
    ...
    ldb.active = 0;
    ldb.logs = listCreate();
    ((ldb . logs)->free = ((void (*)(void*))sdsfree));
...
int ldbStartSession(client *c) {
    ...
    if (ldb.forked) {
        ...
        if (cp == -1) {
            ...
            serverLog(3,"Redis forked for debugging eval");
    ...
    anetBlock(((void *)0),ldb.fd);
    anetSendTimeout(((void *)0),ldb.fd,5000);
    ldb.active = 1;
...
void ldbEndSession(client *c) {
    ...
    if (ldb.forked) {
        ...
        serverLog(3,"Lua debugging session child exiting");
        exitFromChild(0);
    } else {
        serverLog(3,
    ...
    anetNonBlock(((void *)0),ldb.fd);
    anetSendTimeout(((void *)0),ldb.fd,0);
...
void ldbKillForkedSessions(void) {
    ...
    while((ln = listNext(&li))) {
        ...
        serverLog(3,"Killing debugging session %ld",(long)pid);
        kill(pid,9);
    }
...
int ldbAddBreakpoint(int line) {
    ...
    if (!ldbIsBreakpoint(line) && ldb.bpcount != 64) {
...
sds ldbCatStackValueRec(sds s, lua_State *lua, int idx, int level) {
    ...
    case 2:
        ...
        if (t == 6) typename = "function";
        else if (t == 7) typename = "userdata";
        else if (t == 8) typename = "thread";
        else if (t == 2) typename = "light-userdata";
...
void ldbPrint(lua_State *lua, char *varname) {
    ...
    if (!strcmp(varname,"ARGV") || !strcmp(varname,"KEYS")) {
        lua_getfield(lua, (-10002), (varname));
        ldbLogStackValue(lua,"<value> ");
        lua_settop(lua, -(1)-1);
    } else {
...
void ldbEval(lua_State *lua, sds *argv, int argc) {
    ...
    if (luaL_loadbuffer(lua,expr,sdslen(expr),"@ldb_eval")) {
        ...
        if (luaL_loadbuffer(lua,code,sdslen(code),"@ldb_eval")) {
            ldbLog(sdscatfmt(sdsempty(),"<error> %s",lua_tolstring(lua, (-1), ((void *)0))));
            lua_settop(lua, -(1)-1);
            sdsfree(code);
            return;
        }
