...
# 49 "server.c"
# 1 "/usr/include/x86_64-linux-gnu/sys/uio.h"
...
# 84 "/usr/include/x86_64-linux-gnu/sys/uio.h"
...
extern ssize_t pwritev (int __fd, const struct iovec *__iovec, int __count, __off64_t __offset) __asm__ ("" "pwritev64") __attribute__ ((__warn_unused_result__));
# 95 "/usr/include/x86_64-linux-gnu/sys/uio.h"
...
extern ssize_t pwritev2 (int __fd, const struct iovec *__iovec, int __count, __off64_t __offset, int __flags) __asm__ ("" "pwritev64v2") __attribute__ ((__warn_unused_result__));



extern ssize_t preadv2 (int __fd, const struct iovec *__iovec, int __count, __off64_t __offset, int __flags) __asm__ ("" "preadv64v2") __attribute__ ((__warn_unused_result__));
# 149 "/usr/include/x86_64-linux-gnu/sys/uio.h"
...
# 1 "/usr/include/x86_64-linux-gnu/bits/uio-ext.h"
...
# 169 "/usr/include/x86_64-linux-gnu/sys/uio.h"
...
# 52 "server.c"
# 1 "/usr/lib/gcc/x86_64-linux-gnu/7/include/float.h"
...
# 246 "/usr/lib/gcc/x86_64-linux-gnu/7/include/float.h"

# 427 "/usr/lib/gcc/x86_64-linux-gnu/7/include/float.h"

# 505 "/usr/lib/gcc/x86_64-linux-gnu/7/include/float.h"
...
# 1 "/usr/include/x86_64-linux-gnu/bits/locale.h"
...
struct redisCommand redisCommandTable[] = {
    {"module",moduleCommand,-2,
     "admin no-script",
     0,((void *)0),0,0,0,0,0,0},

    {"get",getCommand,2,
     "read-only fast @string",
     0,((void *)0),1,1,1,0,0,0},
...
void trackInstantaneousMetric(int metric, long long current_reading) {
    ...
    server.inst_metric[metric].idx %= 16;
...
int clientsCronHandleTimeout(client *c, mstime_t now_ms) {
    ...
    {
        serverLog(1,"Closing idle client");
        freeClient(c);
        return 1;
    } else if (c->flags & (1<<4)) {
...
int clientsCronResizeQueryBuffer(client *c) {
    ...
    if (c->flags & (1<<1)) {
        ...
        if(pending_querybuf_size > (4*1024*1024) &&
...
size_t ClientsPeakMemInput[8];
size_t ClientsPeakMemOutput[8];

int clientsCronTrackExpansiveClients(client *c) {
    size_t in_usage = sdsAllocSize(c->querybuf);
    size_t out_usage = getClientOutputBufferMemoryUsage(c);
    int i = server.unixtime % 8;
    int zeroidx = (i+1) % 8;
...
int serverCron(struct aeEventLoop *eventLoop, long long id, void *clientData) {
    ...
    if (server.dynamic_hz) {
        while (((server . clients)->len) / server.hz >
               200)
        {
            server.hz *= 2;
            if (server.hz > 500) {
    ...
    if ((100 <= 1000/server . hz) || !(server . cronloops%((100)/(1000/server . hz)))) {
        trackInstantaneousMetric(0,server.stat_numcommands);
        trackInstantaneousMetric(1,
                server.stat_net_input_bytes);
        trackInstantaneousMetric(2,
    ...
    if ((100 <= 1000/server . hz) || !(server . cronloops%((100)/(1000/server . hz)))) {
        ...
        if (!server.cron_malloc_stats.allocator_resident) {
            ...
            size_t lua_memory = lua_gc(server.lua,3,0)*1024LL;
    ...
    if (server.shutdown_asap) {
        ...
        serverLog(3,"SIGTERM received but errors trying to shut down the server, check the logs for more information");
    ...
    if ((5000 <= 1000/server . hz) || !(server . cronloops%((5000)/(1000/server . hz)))) {
        for (j = 0; j < server.dbnum; j++) {
            ...
            if (used || vkeys) {
                serverLog(1,"DB %d: %lld keys (%lld volatile) in %lld slots HT.",j,used,vkeys,size);
        ...
        ldbPendingChildren())
    {
        ...
        if ((pid = wait3(&statloc,1,((void *)0))) != 0) {
            int exitcode = (((statloc) & 0xff00) >> 8);
            int bysignal = 0;

            if ((((signed char) (((statloc) & 0x7f) + 1) >> 1) > 0)) bysignal = ((statloc) & 0x7f);

            if (pid == -1) {
                serverLog(3,"wait3() returned an error: %s. "
        ...
        for (j = 0; j < server.saveparamslen; j++) {
            ...
            {
                serverLog(2,"%d changes in %d seconds. Saving...",
            ...
            server.aof_current_size > server.aof_rewrite_min_size)
        {
            ...
            if (growth >= server.aof_rewrite_perc) {
                serverLog(2,"Starting automatic rewriting of AOF on %lld%% growth",growth);
                rewriteAppendOnlyFileBackground();
            }
        }
    }
    ...
    if (server.rdb_child_pid == -1 && server.aof_child_pid == -1 &&
        ...
        (server.unixtime-server.lastbgsave_try > 5 ||
         server.lastbgsave_status == 0))
    {
        rdbSaveInfo rsi, *rsiptr;
        rsiptr = rdbPopulateSaveInfo(&rsi);
        if (rdbSaveBackground(server.rdb_filename,rsiptr) == 0)
...
void createSharedObjects(void) {
    ...
    shared.emptyarray = createObject(0,sdsnew("*0\r\n"));
    ...
    shared.busykeyerr = createObject(0,sdsnew(
    ...
    shared.null[2] = createObject(0,sdsnew("$-1\r\n"));
    shared.null[3] = createObject(0,sdsnew("_\r\n"));

    shared.nullarray[0] = ((void *)0);
    shared.nullarray[1] = ((void *)0);
    shared.nullarray[2] = createObject(0,sdsnew("*-1\r\n"));
    shared.nullarray[3] = createObject(0,sdsnew("_\r\n"));
    ...
    for (j = 0; j < 10000; j++) {
        shared.integers[j] =
            makeObjectShared(createObject(0,(void*)(long)j));
...
void initServerConfig(void) {
    ...
    server.active_defrag_ignore_bytes = (100<<20);
    server.active_defrag_threshold_lower = 10;
    server.active_defrag_threshold_upper = 100;
    server.active_defrag_cycle_min = 5;
    server.active_defrag_cycle_max = 75;
    server.active_defrag_max_scan_fields = 1000;
    ...
    server.rdb_save_incremental_fsync = 1;
    server.aof_load_truncated = 1;
    server.aof_use_rdb_preamble = 1;
    server.pidfile = ((void *)0);
    server.rdb_filename = zstrdup("dump.rdb");
    server.aof_filename = zstrdup("appendonly.aof");
    server.acl_filename = zstrdup("");
    ...
    server.maxmemory = 0;
    server.maxmemory_policy = (7<<8);
    server.maxmemory_samples = 5;
    server.lfu_log_factor = 10;
    server.lfu_decay_time = 1;
    ...
    server.hll_sparse_max_bytes = 3000;
    ...
    server.cluster_slave_validity_factor = 10;
    server.cluster_require_full_coverage = 1;
    server.cluster_slave_no_failover = 0;
    server.cluster_configfile = zstrdup("nodes.conf");
    server.cluster_announce_ip = ((void *)0);
    server.cluster_announce_port = 0;
    server.cluster_announce_bus_port = 0;
    ...
    server.slave_announce_ip = ((void *)0);
    server.slave_announce_port = 0;
...
int restartServer(int flags, mstime_t delay) {
    ...
    if (access(server.executable,1) == -1) {
        serverLog(3,"Can't restart: this process has no "
    ...
    {
        serverLog(3,"Can't restart: configuration rewrite process "
                             "failed");
        return -1;
    }
    ...
    if (flags & (1<<0) &&
        prepareForShutdown(0) != 0)
    {
        serverLog(3,"Can't restart: error preparing for shutdown");
        return -1;
    }
...
void adjustOpenFilesLimit(void) {
    ...
    if (getrlimit(RLIMIT_NOFILE,&limit) == -1) {
        serverLog(3,"Unable to obtain the current NOFILE limit (%s), assuming 1024 and setting the max clients configuration accordingly.",
            strerror(errno));
        ...
        if (oldlimit < maxfiles) {
            ...
            if (bestlimit < maxfiles) {
                ...
                server.maxclients = bestlimit-32;
                

 
                if (bestlimit <= 32) {
                    serverLog(3,"Your current 'ulimit -n' "
                ...
                serverLog(3,"You requested maxclients of %d "
                    "requiring at least %llu max file descriptors.",
                    old_maxclients,
                    (unsigned long long) maxfiles);
                serverLog(3,"Server can't set maximum open files "
                    "to %llu because of OS error: %s.",
                    (unsigned long long) maxfiles, strerror(setrlimit_error));
                serverLog(3,"Current maximum open files is %llu. "
                    "maxclients has been reduced to %d to compensate for "
                    "low ulimit. "
                    "If you need higher maxclients increase 'ulimit -n'.",
                    (unsigned long long) bestlimit, server.maxclients);
            } else {
                serverLog(2,"Increased maximum number of open files "
...
void checkTcpBacklogSettings(void) {
    ...
    if (fgets(buf,sizeof(buf),fp) != ((void *)0)) {
        ...
        if (somaxconn > 0 && somaxconn < server.tcp_backlog) {
            serverLog(3,"WARNING: The TCP backlog setting of %d cannot be enforced because /proc/sys/net/core/somaxconn is set to the lower value of %d.", server.tcp_backlog, somaxconn);
        }
    }
    fclose(fp);
    ...
    for (j = 0; j < server.bindaddr_count || j == 0; j++) {
        if (server.bindaddr[j] == ((void *)0)) {
            ...
            if (fds[*count] != -1) {
                ...
                unsupported++;
                serverLog(3,"Not listening to IPv6: unsupported");
            }
            ...
            if (*count == 1 || unsupported) {
                ...
                if (fds[*count] != -1) {
                    ...
                    unsupported++;
                    serverLog(3,"Not listening to IPv4: unsupported");
                }
            }
...
void initServer(void) {
    ...
    if (server.unixsocket != ((void *)0)) {
        ...
        if (server.sofd == -1) {
            serverLog(3, "Opening Unix socket: %s", server.neterr);
            exit(1);
        }
    ...
    if (server.ipfd_count == 0 && server.sofd < 0) {
        serverLog(3, "Configured to not listen anywhere, exiting.");
        exit(1);
    }
    ...
    if (aeCreateTimeEvent(server.el, 1, serverCron, ((void *)0), ((void *)0)) == -1) {
        _serverPanic("server.c",2814,"Can't create event loop timers."),_exit(1);
        exit(1);
    }
    ...
    for (j = 0; j < server.ipfd_count; j++) {
        if (aeCreateFileEvent(server.el, server.ipfd[j], 1,
            ...
            {
                _serverPanic("server.c",2825,"Unrecoverable error creating server.ipfd file event."),_exit(1);

            }
    }
    if (server.sofd > 0 && aeCreateFileEvent(server.el,server.sofd,1,
        acceptUnixHandler,((void *)0)) == -1) _serverPanic("server.c",2829,"Unrecoverable error creating server.sofd file event."),_exit(1);


    
 
    if (aeCreateFileEvent(server.el, server.module_blocked_pipe[0], 1,
        moduleBlockedClientPipeReadable,((void *)0)) == -1) {
            _serverPanic("server.c",2838,"Error registering the readable event for the module " "blocked clients subsystem."),_exit(1);
    ...
    if (server.aof_state == 1) {
        server.aof_fd = open(server.aof_filename,
                               01|02000|0100,0644);
        if (server.aof_fd == -1) {
            serverLog(3, "Can't open the append-only file: %s",
                strerror(errno));
            exit(1);
        }
    }
    ...
    if (server.arch_bits == 32 && server.maxmemory == 0) {
        serverLog(3,"Warning: 32 bit instance detected but no memory limit set. Setting 3 GB maxmemory limit with 'noeviction' policy now.");
...
int populateCommandTableParseFlags(struct redisCommand *c, char *strflags) {
    ...
    if (!(c->flags & (1ULL<<30))) c->flags |= (1ULL<<31);
...
void populateCommandTable(void) {
    ...
    for (j = 0; j < numcommands; j++) {
        ...
        if (populateCommandTableParseFlags(c,c->sflags) == -1)
            _serverPanic("server.c",2944,"Unsupported command flag"),_exit(1);
        ...
        ((retval1 == 0 && retval2 == 0)?(void)0 : (_serverAssert("retval1 == DICT_OK && retval2 == DICT_OK","server.c",2951),_exit(1)));
    }
}
...
void resetCommandTableStats(void) {
    ...
    while((de = dictNext(di)) != ((void *)0)) {
        c = (struct redisCommand *) ((de)->v . val);
        c->microseconds = 0;
        c->calls = 0;
    }
...
void call(client *c, int flags) {
    ...
    if (flags & (1<<0) && c->cmd->proc != execCommand) {
        char *latency_event = (c->cmd->flags & (1ULL<<13)) ?
                              "fast-command" : "command";
        if (server . latency_monitor_threshold && (duration/1000) >= server . latency_monitor_threshold) latencyAddSample((latency_event),(duration/1000));;
    ...
    if (flags & ((1<<2)|(1<<3)) &&
        (c->flags & ((1<<19)|(1<<20))) != ((1<<19)|(1<<20)))
    {
        int propagate_flags = 0;

        
 
        if (dirty) propagate_flags |= (1|2);

        
 
        if (c->flags & (1<<15)) propagate_flags |= 2;
        if (c->flags & (1<<14)) propagate_flags |= 1;
        ...
        if (propagate_flags != 0 && !(c->cmd->flags & (1ULL<<3)))
    ...
    if (server.also_propagate.numops) {
        ...
        if (flags & ((1<<2)|(1<<3))) {
            for (j = 0; j < server.also_propagate.numops; j++) {
                ...
                if (!(flags&(1<<2))) target &= ~1;
                if (!(flags&(1<<3))) target &= ~2;
                if (target)
...
int processCommand(client *c) {
    ...
    int auth_required = !(DefaultUser->flags & (1<<4)) &&
                        !c->authenticated;
    if (auth_required || DefaultUser->flags & (1<<1)) {
    ...
    if (server.maxmemory && !server.lua_timedout) {
        int out_of_memory = freeMemoryIfNeededAndSafe() == -1;
        
 
        if (server.current_client == ((void *)0)) return -1;
        ...
        if (out_of_memory &&
            (c->cmd->flags & (1ULL<<2) ||
             (c->flags & (1<<3) && c->cmd->proc != execCommand))) {
            flagTransaction(c);
    ...
    if (deny_write_type != 0 &&
        server.masterhost == ((void *)0) &&
        (c->cmd->flags & (1ULL<<0) ||
         c->cmd->proc == pingCommand))
    {
        flagTransaction(c);
        if (deny_write_type == 2)
    ...
    if (server.masterhost && server.repl_state != 15 &&
    ...
    if (server.loading && !(c->cmd->flags & (1ULL<<9))) {
...
void closeListeningSockets(int unlink_unix_socket) {
    ...
    if (unlink_unix_socket && server.unixsocket) {
        serverLog(2,"Removing the unix socket file.");
        unlink(server.unixsocket);  
    }
}
...
int prepareForShutdown(int flags) {
    ...
    serverLog(3,"User requested shutdown...");
    ...
    if (server.rdb_child_pid != -1) {
        serverLog(3,"There is a child saving an .rdb. Killing it!");
        killRDBChild();
    }
    ...
    if (server.aof_state != 0) {
        ...
        if (server.aof_child_pid != -1) {
            ...
            if (server.aof_state == 2) {
                serverLog(3, "Writing initial AOF, can't exit.");
                return -1;
            }
            serverLog(3,
        ...
        serverLog(2,"Calling fsync() on the AOF file.");
    ...
    if ((server.saveparamslen > 0 && !nosave) || save) {
        serverLog(2,"Saving the final RDB snapshot before exiting.");
         
        rdbSaveInfo rsi, *rsiptr;
        rsiptr = rdbPopulateSaveInfo(&rsi);
        if (rdbSave(server.rdb_filename,rsiptr) != 0) {
            



 
            serverLog(3,"Error trying to save the DB, can't exit.");
            return -1;
        }
    }
    ...
    if (server.daemonize || server.pidfile) {
        serverLog(2,"Removing the pid file.");
    ...
    serverLog(3,"%s is now ready to exit, bye bye...",
...
void addReplyCommand(client *c, struct redisCommand *cmd) {
    if (!cmd) {
        ...
        flagcount += addReplyCommandFlag(c,cmd,(1ULL<<0), "write");
        flagcount += addReplyCommandFlag(c,cmd,(1ULL<<1), "readonly");
        flagcount += addReplyCommandFlag(c,cmd,(1ULL<<2), "denyoom");
        flagcount += addReplyCommandFlag(c,cmd,(1ULL<<4), "admin");
        flagcount += addReplyCommandFlag(c,cmd,(1ULL<<5), "pubsub");
        flagcount += addReplyCommandFlag(c,cmd,(1ULL<<6), "noscript");
        flagcount += addReplyCommandFlag(c,cmd,(1ULL<<7), "random");
        flagcount += addReplyCommandFlag(c,cmd,(1ULL<<8),"sort_for_script");
        flagcount += addReplyCommandFlag(c,cmd,(1ULL<<9), "loading");
        flagcount += addReplyCommandFlag(c,cmd,(1ULL<<10), "stale");
        flagcount += addReplyCommandFlag(c,cmd,(1ULL<<11), "skip_monitor");
        flagcount += addReplyCommandFlag(c,cmd,(1ULL<<12), "asking");
        flagcount += addReplyCommandFlag(c,cmd,(1ULL<<13), "fast");
        if ((cmd->getkeys_proc && !(cmd->flags & (1ULL<<3))) ||
            cmd->flags & (1ULL<<14))
        {
...
((void *)0)
        ...
        addReplyArrayLen(c, ((server . commands)->ht[0]. used+(server . commands)->ht[1]. used));
        di = dictGetIterator(server.commands);
        while ((de = dictNext(di)) != ((void *)0)) {
            addReplyCommand(c, ((de)->v . val));
        }
        dictReleaseIterator(di);
        ...
        addReplyLongLong(c, ((server . commands)->ht[0]. used+(server . commands)->ht[1]. used));
...
sds genRedisInfoString(char *section) {
    ...
    if (allsections || defsections || !strcasecmp(section,"memory")) {
        ...
        long long memory_lua = (long long)lua_gc(server.lua,3,0)*1024;
        ...
        info = sdscatprintf(info,
            ...
            ((server . lua_scripts)->ht[0]. used+(server . lua_scripts)->ht[1]. used),
            server.maxmemory,
            maxmemory_hmem,
            evict_policy,
            mh->allocator_frag,
    ...
    if (allsections || defsections || !strcasecmp(section,"persistence")) {
        ...
        if (server.loading) {
            ...
            elapsed = time(((void *)0))-server.loading_start_time;
            if (elapsed == 0) {
                eta = 1; 
    ...
    if (allsections || defsections || !strcasecmp(section,"stats")) {
        ...
        info = sdscatprintf(info,
            ...
            server.stat_numcommands,
            getInstantaneousMetric(0),
            server.stat_net_input_bytes,
            server.stat_net_output_bytes,
            (float)getInstantaneousMetric(1)/1024,
            (float)getInstantaneousMetric(2)/1024,
    ...
    if (allsections || defsections || !strcasecmp(section,"replication")) {
        ...
        if (server.masterhost) {
            ...
            info = sdscatprintf(info,
                ...
                ,server.masterhost,
                server.masterport,
                (server.repl_state == 15) ?
                    "up" : "down",
                server.master ?
                ((int)(server.unixtime-server.master->lastinteraction)) : -1,
                server.repl_state == 14,
                slave_repl_offset
            );

            if (server.repl_state == 14) {
                info = sdscatprintf(info,
        ...
        if (((server . slaves)->len)) {
            ...
            while((ln = listNext(&li))) {
                ...
                char *state = ((void *)0);
                char ip[46], *slaveip = slave->slave_ip;
                int port;
                long lag = 0;
...
void linuxMemoryWarnings(void) {
    if (linuxOvercommitMemoryValue() == 0) {
        serverLog(3,"WARNING overcommit_memory is set to 0! Background save may fail under low memory condition. To fix this issue add 'vm.overcommit_memory = 1' to /etc/sysctl.conf and then reboot or run the command 'sysctl vm.overcommit_memory=1' for this to take effect.");
    }
    if (THPIsEnabled()) {
        serverLog(3,"WARNING you have Transparent Huge Pages (THP) support enabled in your kernel. This will create latency and memory usage issues with Redis. To fix this issue run the command 'echo never > /sys/kernel/mm/transparent_hugepage/enabled' as root, and add it to your /etc/rc.local in order to retain the setting after a reboot. Redis must be restarted after THP is disabled.");
    }
}
...
void createPidFile(void) {
    ...
    if (!server.pidfile) server.pidfile = zstrdup("/var/run/redis.pid");
...
static void sigShutdownHandler(int sig) {
    ...
    if (server.shutdown_asap && sig == 2) {
        serverLogFromHandler(3, "You insist... exiting now.");
        rdbRemoveTempFile(getpid());
        exit(1);  
    } else if (server.loading) {
        serverLogFromHandler(3, "Received shutdown signal during loading, exiting now.");
        exit(0);
    }

    serverLogFromHandler(3, msg);
...
void setupSignalHandlers(void) {
    ...
    act.sa_flags = 0;
    act. __sigaction_handler . sa_handler = sigShutdownHandler;
...
void loadDataFromDisk(void) {
    ...
    if (server.aof_state == 1) {
        if (loadAppendOnlyFile(server.aof_filename) == 0)
            serverLog(2,"DB loaded from append only file: %.3f seconds",(float)(ustime()-start)/1000000);
    } else {
        rdbSaveInfo rsi = {-1,0,"000000000000000000000000000000",-1};
        if (rdbLoad(server.rdb_filename,&rsi) == 0) {
            serverLog(2,"DB loaded from disk: %.3f seconds",
                (float)(ustime()-start)/1000000);

             
            if ((server.masterhost || (server.cluster_enabled && ((server . cluster->myself)->flags & 2)))&&
                rsi.repl_id_is_set &&
                rsi.repl_offset != -1 &&
            ...
            serverLog(3,"Fatal error loading the DB: %s. Exiting.",strerror(errno));
            exit(1);
        }
    }
}
...
void redisOutOfMemoryHandler(size_t allocation_size) {
    serverLog(3,"Out Of Memory allocating %zu bytes!",
        allocation_size);
    _serverPanic("server.c",4591,"Redis aborting for OUT OF MEMORY"),_exit(1);
}
...
int redisSupervisedUpstart(void) {
    ...
    serverLog(2, "supervised by upstart, will stop to signal readiness");
    raise(19);
...
int redisSupervisedSystemd(void) {
    ...
    if ((strchr("@/", notify_socket[0])) == ((void *)0) || strlen(notify_socket) < 2) {
        return 0;
    }

    serverLog(2, "supervised by systemd, will signal readiness");
    if ((fd = socket(1, SOCK_DGRAM, 0)) == -1) {
        serverLog(3,
    ...
    hdr.msg_name = &su;
    hdr.msg_namelen = __builtin_offsetof (struct sockaddr_un, sun_path) +
    ...
    if (sendmsg(fd, &hdr, sendto_flags) < 0) {
        serverLog(3, "Can't send notification to systemd");
        close(fd);
        return 0;
    }
    close(fd);
...
# 4735 "server.c"
    ...
    serverLog(3, "oO0OoO0OoO0Oo Redis is starting oO0OoO0OoO0Oo");
    serverLog(3,
    ...
    if (argc == 1) {
        serverLog(3, "Warning: no config file specified, using the default config. In order to specify a config file use %s /path/to/%s.conf", argv[0], server.sentinel_mode ? "sentinel" : "redis");
    } else {
        serverLog(3, "Configuration loaded");
    }
    ...
    if (!server.sentinel_mode) {
        ...
        serverLog(3,"Server initialized");
        ...
        if (server.ipfd_count > 0)
            serverLog(2,"Ready to accept connections");
        if (server.sofd > 0)
            serverLog(2,"The server is now ready to accept connections at %s", server.unixsocket);
    } else {
        sentinelIsRunning();
    }
    ...
    if (server.maxmemory > 0 && server.maxmemory < 1024*1024) {
        serverLog(3,"WARNING: You specified a maxmemory value that is less than 1MB (current value is %llu bytes). Are you sure this is what you really want?", server.maxmemory);
    }
