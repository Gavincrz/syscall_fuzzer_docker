...
char *replicationGetSlaveName(client *c) {
    static char buf[(46+32)];
    char ip[46];

    ip[0] = '\0';
    buf[0] = '\0';
    if (c->slave_ip[0] != '\0' ||
        anetPeerToString(c->fd,ip,sizeof(ip),((void *)0)) != -1)
    {
...
void createReplicationBacklog(void) {
    ((server . repl_backlog == ((void *)0))?(void)0 : (_serverAssert("server.repl_backlog == NULL","replication.c",79),_exit(1)));
...
void freeReplicationBacklog(void) {
    ((((server . slaves)->len) == 0)?(void)0 : (_serverAssert("listLength(server.slaves) == 0","replication.c",118),_exit(1)));
    zfree(server.repl_backlog);
...
void replicationFeedSlaves(list *slaves, int dictid, robj **argv, int argc) {
    ...
    if (server.repl_backlog == ((void *)0) && ((slaves)->len) == 0) return;

     
    ((!(((slaves)->len) != 0 && server . repl_backlog == ((void *)0)))?(void)0 : (_serverAssert("!(listLength(slaves) != 0 && server.repl_backlog == NULL)","replication.c",192),_exit(1)));
...
long long addReplyReplicationBacklog(client *c, long long offset) {
    ...
    serverLog(0, "[PSYNC] Replica request offset: %lld", offset);

    if (server.repl_backlog_histlen == 0) {
        serverLog(0, "[PSYNC] Backlog history len is zero");
        return 0;
    }

    serverLog(0, "[PSYNC] Backlog size: %lld",
             server.repl_backlog_size);
    serverLog(0, "[PSYNC] First byte: %lld",
             server.repl_backlog_off);
    serverLog(0, "[PSYNC] History len: %lld",
             server.repl_backlog_histlen);
    serverLog(0, "[PSYNC] Current index: %lld",
             server.repl_backlog_idx);

     
    skip = offset - server.repl_backlog_off;
    serverLog(0, "[PSYNC] Skipping: %lld", skip);
    ...
    j = (server.repl_backlog_idx +
        ...
        server.repl_backlog_size;
    serverLog(0, "[PSYNC] Index of first byte: %lld", j);
    ...
    serverLog(0, "[PSYNC] Reply total length: %lld", len);
    while(len) {
        long long thislen =
            ((server.repl_backlog_size - j) < len) ?
            (server.repl_backlog_size - j) : len;

        serverLog(0, "[PSYNC] addReply() length: %lld", thislen);
...
int masterTryPartialResynchronization(client *c) {
    ...
    if (getLongLongFromObjectOrReply(c,c->argv[2],&psync_offset,((void *)0)) !=
       0) goto need_full_resync;
         ...
         psync_offset > server.second_replid_offset))
    {
        ...
        if (master_replid[0] != '?') {
            if (strcasecmp(master_replid, server.replid) &&
                strcasecmp(master_replid, server.replid2))
            {
                serverLog(2,"Partial resynchronization not accepted: "
            ...
            serverLog(2,"Full resync requested by replica %s",
...
int startBgsaveForReplication(int mincapa) {
    int retval;
    int socket_target = server.repl_diskless_sync && (mincapa & (1<<0));
    listIter li;
    listNode *ln;

    serverLog(2,"Starting BGSAVE for SYNC with target: %s",
    ...
    if (rsiptr) {
        ...
        serverLog(3,"BGSAVE for replication: replication information not available, can't generate the RDB file right now. Try later.");
        retval = -1;
    }
    ...
    if (retval == -1) {
        serverLog(3,"BGSAVE for replication failed");
    ...
    if (retval == 0) replicationScriptCacheFlush();
    return retval;
}
...
void syncCommand(client *c) {
    ...
    serverLog(2,"Replica %s asks for synchronization",
        replicationGetSlaveName(c));
    ...
    if (!strcasecmp(c->argv[0]->ptr,"psync")) {
        if (masterTryPartialResynchronization(c) == 0) {
    ...
    if (server.repl_disable_tcp_nodelay)
        anetDisableTcpNoDelay(((void *)0), c->fd);  
    c->repldbfd = -1;
    c->flags |= (1<<0);
    listAddNodeTail(server.slaves,c);

     
    if (((server . slaves)->len) == 1 && server.repl_backlog == ((void *)0)) {
        ...
        server.rdb_child_type == 1)
    {
        ...
        if (ln && ((c->slave_capa & slave->slave_capa) == slave->slave_capa)) {
            ...
            serverLog(2,"Waiting for end of BGSAVE for SYNC");
        } else {
            
 
            serverLog(2,"Can't attach the replica to the current BGSAVE. Waiting for next BGSAVE for SYNC");
        }
               ...
               server.rdb_child_type == 2)
    {
        ...
        serverLog(2,"Current BGSAVE has socket target. Waiting for next BGSAVE for SYNC");

     
    } else {
        if (server.repl_diskless_sync && (c->slave_capa & (1<<0))) {
            

 
            if (server.repl_diskless_sync_delay)
                serverLog(2,"Delay next BGSAVE for diskless SYNC");
        } else {
...
void replconfCommand(client *c) {
    ...
    for (j = 1; j < c->argc; j+=2) {
        if (!strcasecmp(c->argv[j]->ptr,"listening-port")) {
            ...
            if (!(c->flags & (1<<0))) return;
            if ((getLongLongFromObject(c->argv[j+1], &offset) != 0))
                return;
            ...
            if (c->repl_put_online_on_ack && c->replstate == 9)
                putSlaveOnline(c);
...
void putSlaveOnline(client *slave) {
    ...
    if (aeCreateFileEvent(server.el, slave->fd, 2,
        sendReplyToClient, slave) == -1) {
        serverLog(3,"Unable to register writable event for replica bulk transfer: %s", strerror(errno));
        freeClient(slave);
        return;
    }
    refreshGoodSlavesCount();
    serverLog(2,"Synchronization with replica %s succeeded",
...
void sendBulkToSlave(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    if (slave->replpreamble) {
        ...
        if (nwritten == -1) {
            serverLog(1,"Write error sending RDB preamble to replica: %s",
                strerror(errno));
            freeClient(slave);
            return;
        }
    ...
    buflen = read(slave->repldbfd,buf,(1024*16));
    if (buflen <= 0) {
        serverLog(3,"Read error sending DB to replica: %s",
    ...
    if ((nwritten = write(fd,buf,buflen)) == -1) {
        if (errno != 11) {
            serverLog(3,"Write error sending DB to replica: %s",
                strerror(errno));
            freeClient(slave);
        }
        return;
    ...
    while((ln = listNext(&li))) {
        ...
        if (slave->replstate == 6) {
            ...
            if (type == 2) {
                ...
                if (bgsaveerr != 0) {
                    freeClient(slave);
                    serverLog(3,"SYNC failed. BGSAVE child returned an error");
                    continue;
                }
                if ((slave->repldbfd = open(server.rdb_filename,00)) == -1 ||
                    fstat(slave->repldbfd,&buf) == -1) {
                    freeClient(slave);
                    serverLog(3,"SYNC failed. Can't open/stat DB after BGSAVE: %s", strerror(errno));
                    continue;
                }
                slave->repldboff = 0;
                ...
                aeDeleteFileEvent(server.el,slave->fd,2);
                if (aeCreateFileEvent(server.el, slave->fd, 2, sendBulkToSlave, slave) == -1) {
                    freeClient(slave);
                    continue;
                }
            }
...
void changeReplicationId(void) {
    getRandomHexChars(server.replid,40);
    server.replid[40] = '\0';
}
...
void shiftReplicationId(void) {
    ...
    changeReplicationId();
    serverLog(3,"Setting secondary replication ID to %s, valid up to offset: %lld. New replication ID is %s", server.replid2, server.second_replid_offset, server.replid);
}
...
int slaveIsInHandshakeState(void) {
    return server.repl_state >= 3 &&
           server.repl_state <= 13;
}
...
void readSyncBulkPayload(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    if (server.repl_transfer_size == -1) {
        ...
        if (buf[0] == '-') {
            ...
            serverLog(3,"Bad protocol from MASTER, the first byte is not '$' (we received '%s'), are you sure the host and port are right?", buf);
            goto error;
        }
        ...
        if (strncmp(buf+1,"EOF:",4) == 0 && strlen(buf+5) >= 40) {
            ...
            usemark = 0;
            server.repl_transfer_size = strtol(buf+1,((void *)0),10);
            serverLog(2,
    ...
    if (nread <= 0) {
        serverLog(3,"I/O error trying to sync with MASTER: %s",
    ...
    if (usemark) {
        ...
        if (memcmp(lastbytes,eofmark,40) == 0) eof_reached = 1;
    }

    server.repl_transfer_lastio = server.unixtime;
    if ((nwritten = write(server.repl_transfer_fd,buf,nread)) != nread) {
        serverLog(3,"Write error or short write writing to the DB dump file needed for MASTER <-> REPLICA synchronization: %s", 
    ...
    if (usemark && eof_reached) {
        if (ftruncate(server.repl_transfer_fd,
            server.repl_transfer_read - 40) == -1)
        {
            serverLog(3,"Error truncating the RDB file received from the master for SYNC: %s", strerror(errno));
            goto error;
        }
    }
    ...
    if (server.repl_transfer_read >=
        server.repl_transfer_last_fsync_off + (1024*1024*8))
    {
        off_t sync_size = server.repl_transfer_read -
                          server.repl_transfer_last_fsync_off;
        sync_file_range(server . repl_transfer_fd,server . repl_transfer_last_fsync_off,sync_size,1|2);
    ...
    if (eof_reached) {
        int aof_is_enabled = server.aof_state != 0;
        ...
        if (rename(server.repl_transfer_tmpfile,server.rdb_filename) == -1) {
            serverLog(3,"Failed trying to rename the temp DB into %s in MASTER <-> REPLICA synchronization: %s", 
			    server.rdb_filename, strerror(errno));
            cancelReplicationHandshake();
            return;
        }
        serverLog(2, "MASTER <-> REPLICA sync: Flushing old data");
        ...
        emptyDb(
            -1,
            server.repl_slave_lazy_flush ? (1<<0) : 0,
        ...
        serverLog(2, "MASTER <-> REPLICA sync: Loading DB in memory");
        rdbSaveInfo rsi = {-1,0,"000000000000000000000000000000",-1};
        if (rdbLoad(server.rdb_filename,&rsi) != 0) {
            serverLog(3,"Failed trying to load the MASTER synchronization DB from disk");
            cancelReplicationHandshake();
        ...
        serverLog(2, "MASTER <-> REPLICA sync: Finished with success");
...
char *sendSynchronousCommand(int flags, int fd, ...) {
    ...
    if (flags & (1<<1)) {
        ...
        while(1) {
            arg = (*(char* *)(((*(char* *)&(ap)) += sizeof(char*)) - sizeof(char*)));
            if (arg == ((void *)0)) break;
...
int slaveTryPartialResynchronization(int fd, int read_reply) {
    ...
    if (!read_reply) {
        ...
        if (server.cached_master) {
            ...
            serverLog(2,"Trying a partial resynchronization (request %s:%s).", psync_replid, psync_offset);
        } else {
            serverLog(2,"Partial resynchronization not possible (no cached master)");
            psync_replid = "?";
            memcpy(psync_offset,"-1",3);
        }

         
        reply = sendSynchronousCommand((1<<1),fd,"PSYNC",psync_replid,psync_offset,((void *)0));
        if (reply != ((void *)0)) {
            serverLog(3,"Unable to send PSYNC to master: %s",reply);
            sdsfree(reply);
    ...
    reply = sendSynchronousCommand((1<<0),fd,((void *)0));
    if (sdslen(reply) == 0) {
    ...
    if (!strncmp(reply,"+FULLRESYNC",11)) {
        ...
        if (!replid || !offset || (offset-replid-1) != 40) {
            serverLog(3,
            ...
            memset(server.master_replid,0,40+1);
        } else {
            memcpy(server.master_replid, replid, offset-replid-1);
            server.master_replid[40] = '\0';
            server.master_initial_offset = strtoll(offset,((void *)0),10);
            serverLog(2,"Full resync from master: %s:%lld",
    ...
    if (!strncmp(reply,"+CONTINUE",9)) {
        ...
        if (end-start == 40) {
            ...
            if (strcmp(new,server.cached_master->replid)) {
                ...
                serverLog(3,"Master replication ID changed to %s",new);
...
void syncWithMaster(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    if (getsockopt(fd, 1, 4, &sockerr, &errlen) == -1)
        sockerr = errno;
    if (sockerr) {
        serverLog(3,"Error condition on socket for SYNC: %s",
            strerror(sockerr));
        goto error;
    }

     
    if (server.repl_state == 2) {
        serverLog(2,"Non blocking connect for SYNC fired the event.");
        
 
        aeDeleteFileEvent(server.el,fd,2);
        server.repl_state = 3;
        
 
        err = sendSynchronousCommand((1<<1),fd,"PING",((void *)0));
        if (err) goto write_error;
        return;
    }

     
    if (server.repl_state == 3) {
        err = sendSynchronousCommand((1<<0),fd,((void *)0));
        ...
        {
            serverLog(3,"Error reply to PING from master: '%s'",err);
            sdsfree(err);
            goto error;
        } else {
            serverLog(2,
    ...
    if (server.repl_state == 4) {
        if (server.masteruser && server.masterauth) {
            err = sendSynchronousCommand((1<<1),fd,"AUTH",
                                         server.masteruser,server.masterauth,((void *)0));
            if (err) goto write_error;
            server.repl_state = 5;
            return;
        } else if (server.masterauth) {
            err = sendSynchronousCommand((1<<1),fd,"AUTH",server.masterauth,((void *)0));
            if (err) goto write_error;
            server.repl_state = 5;
            return;
        } else {
    ...
    if (server.repl_state == 5) {
        err = sendSynchronousCommand((1<<0),fd,((void *)0));
        if (err[0] == '-') {
            serverLog(3,"Unable to AUTH to MASTER: %s",err);
            sdsfree(err);
            goto error;
        }
        sdsfree(err);
    ...
    if (server.repl_state == 6) {
        sds port = sdsfromlonglong(server.slave_announce_port ?
            server.slave_announce_port : server.port);
        err = sendSynchronousCommand((1<<1),fd,"REPLCONF",
                "listening-port",port, ((void *)0));
        sdsfree(port);
        if (err) goto write_error;
        sdsfree(err);
        server.repl_state = 7;
    ...
    if (server.repl_state == 7) {
        err = sendSynchronousCommand((1<<0),fd,((void *)0));
        
 
        if (err[0] == '-') {
            serverLog(2,"(Non critical) Master does not understand "
    ...
    if (server.repl_state == 8 &&
        server.slave_announce_ip == ((void *)0))
    {
            server.repl_state = 10;
    }
    ...
    if (server.repl_state == 8) {
        err = sendSynchronousCommand((1<<1),fd,"REPLCONF",
                "ip-address",server.slave_announce_ip, ((void *)0));
        if (err) goto write_error;
        sdsfree(err);
        server.repl_state = 9;
        return;
    ...
    if (server.repl_state == 10) {
        err = sendSynchronousCommand((1<<1),fd,"REPLCONF",
                "capa","eof","capa","psync2",((void *)0));
        if (err) goto write_error;
        sdsfree(err);
        server.repl_state = 11;
        return;
    }

     
    if (server.repl_state == 11) {
        err = sendSynchronousCommand((1<<0),fd,((void *)0));
    ...
    if (server.repl_state == 12) {
        if (slaveTryPartialResynchronization(fd,0) == 0) {
    ...
    if (server.repl_state != 13) {
        serverLog(3,"syncWithMaster(): state machine error, "
    ...
    if (psync_result == 1) return;  

    


 
    if (psync_result == 5) goto error;

    
 

    if (psync_result == 2) {
        serverLog(2, "MASTER <-> REPLICA sync: Master accepted a Partial Resynchronization.");
        return;
    }
    ...
    if (psync_result == 4) {
        serverLog(2,"Retrying with SYNC...");
        if (syncWrite(fd,"SYNC\r\n",6,server.repl_syncio_timeout*1000) == -1) {
            serverLog(3,"I/O error writing to MASTER: %s",
                strerror(errno));
            goto error;
        }
    }
    ...
    while(maxtries--) {
        ...
        dfd = open(tmpfile,0100|01|0200,0644);
        if (dfd != -1) break;
        sleep(1);
    }
    if (dfd == -1) {
        serverLog(3,"Opening the temp file needed for MASTER <-> REPLICA synchronization: %s",strerror(errno));
        goto error;
    }

     
    if (aeCreateFileEvent(server.el,fd, 1,readSyncBulkPayload,((void *)0))
            == -1)
    {
        serverLog(3,
...
error:
    aeDeleteFileEvent(server.el,fd,1|2);
    if (dfd != -1) close(dfd);
    close(fd);
...
write_error:  
    serverLog(3,"Sending command to master in replication handshake: %s", err);
    sdsfree(err);
    goto error;
}
...
int connectWithMaster(void) {
    ...
    fd = anetTcpNonBlockBestEffortBindConnect(((void *)0),
        server.masterhost,server.masterport,(server . bindaddr_count ? server . bindaddr[0] : ((void *)0)));
    if (fd == -1) {
        serverLog(3,"Unable to connect to MASTER: %s",
            strerror(errno));
        return -1;
    }

    if (aeCreateFileEvent(server.el,fd,1|2,syncWithMaster,((void *)0)) ==
            -1)
    {
        close(fd);
        serverLog(3,"Can't create readable event for SYNC");
        return -1;
    }
...
void replicationAbortSyncTransfer(void) {
    ((server . repl_state == 14)?(void)0 : (_serverAssert("server.repl_state == REPL_STATE_TRANSFER","replication.c",1934),_exit(1)));
...
void replicaofCommand(client *c) {
    ...
    if (!strcasecmp(c->argv[1]->ptr,"no") &&
        ...
        if (server.masterhost) {
            ...
            serverLog(2,"MASTER MODE enabled (user request from '%s')",
                client);
            sdsfree(client);
        }
    } else {
        ...
        if ((getLongFromObjectOrReply(c, c->argv[2], &port, ((void *)0)) != 0))
            return;

         
        if (server.masterhost && !strcasecmp(server.masterhost,c->argv[1]->ptr)
            && server.masterport == port) {
            serverLog(2,"REPLICAOF would result into synchronization "
        ...
        sds client = catClientInfoString(sdsempty(),c);
        serverLog(2,"REPLICAOF %s:%d enabled (user request from '%s')",
...
void roleCommand(client *c) {
    if (server.masterhost == ((void *)0)) {
        ...
        while((ln = listNext(&li))) {
            ...
            if (slaveip[0] == '\0') {
                if (anetPeerToString(slave->fd,ip,sizeof(ip),((void *)0)) == -1)
                    continue;
                slaveip = ip;
            }
            if (slave->replstate != 9) continue;
            addReplyArrayLen(c,3);
        ...
        if (slaveIsInHandshakeState()) {
            ...
            case 0: slavestate = "none"; break;
            case 1: slavestate = "connect"; break;
            case 2: slavestate = "connecting"; break;
            case 14: slavestate = "sync"; break;
            case 15: slavestate = "connected"; break;
...
void replicationCacheMaster(client *c) {
    ((server . master != ((void *)0) && server . cached_master == ((void *)0))?(void)0 : (_serverAssert("server.master != NULL && server.cached_master == NULL","replication.c",2178),_exit(1)));
    serverLog(2,"Caching the disconnected master state.");
    ...
    server.master->read_reploff = server.master->reploff;
    if (c->flags & (1<<3)) discardTransaction(c);
    listEmpty(c->reply);
    c->sentlen = 0;
    c->reply_bytes = 0;
    c->bufpos = 0;
...
void replicationCacheMasterUsingMyself(void) {
    ...
    server.cached_master = server.master;
    server.master = ((void *)0);
    serverLog(2,"Before turning into a replica, using my master parameters to synthesize a cached master: I may be able to synchronize with the new master with just a partial transfer.");
}
...
void replicationDiscardCachedMaster(void) {
    ...
    serverLog(2,"Discarding previously cached master state.");
    server.cached_master->flags &= ~(1<<1);
...
void replicationResurrectCachedMaster(int newfd) {
    ...
    if (aeCreateFileEvent(server.el, newfd, 1,
                          readQueryFromClient, server.master)) {
        serverLog(3,"Error resurrecting the cached master, impossible to add the readable handler: %s", strerror(errno));
    ...
    if (clientHasPendingReplies(server.master)) {
        if (aeCreateFileEvent(server.el, newfd, 2,
                          sendReplyToClient, server.master)) {
            serverLog(3,"Error resurrecting the cached master, impossible to add the writable handler: %s", strerror(errno));
            freeClientAsync(server.master);  
        }
    }
}
...
void replicationScriptCacheAdd(sds sha1) {
    ...
    if (((server . repl_scriptcache_fifo)->len) == server.repl_scriptcache_size)
    {
        listNode *ln = ((server . repl_scriptcache_fifo)->tail);
        sds oldest = ((ln)->value);

        retval = dictDelete(server.repl_scriptcache_dict,oldest);
        ((retval == 0)?(void)0 : (_serverAssert("retval == DICT_OK","replication.c",2377),_exit(1)));
    ...
    ((retval == 0)?(void)0 : (_serverAssert("retval == DICT_OK","replication.c",2384),_exit(1)));
}
...
void waitCommand(client *c) {
    ...
    if (getLongFromObjectOrReply(c,c->argv[1],&numreplicas,((void *)0)) != 0)
        return;
    if (getTimeoutFromObjectOrReply(c,c->argv[2],&timeout,1)
        != 0) return;

     
    ackreplicas = replicationCountAcksByOffset(c->woff);
    if (ackreplicas >= numreplicas || c->flags & (1<<3)) {
...
void unblockClientWaitingReplicas(client *c) {
    ...
    ((ln != ((void *)0))?(void)0 : (_serverAssert("ln != NULL","replication.c",2488),_exit(1)));
...
void replicationCron(void) {
    ...
    if (server.masterhost &&
        (server.repl_state == 2 ||
         slaveIsInHandshakeState()) &&
         (time(((void *)0))-server.repl_transfer_lastio) > server.repl_timeout)
    {
        serverLog(3,"Timeout connecting to the MASTER...");
        cancelReplicationHandshake();
    }
    ...
    if (server.masterhost && server.repl_state == 14 &&
        (time(((void *)0))-server.repl_transfer_lastio) > server.repl_timeout)
    {
        serverLog(3,"Timeout receiving bulk data from MASTER... If the problem persists try to set the 'repl-timeout' parameter in redis.conf to a larger value.");
        cancelReplicationHandshake();
    }

     
    if (server.masterhost && server.repl_state == 15 &&
        (time(((void *)0))-server.master->lastinteraction) > server.repl_timeout)
    {
        serverLog(3,"MASTER timeout: no data nor PING received...");
        freeClient(server.master);
    }

     
    if (server.repl_state == 1) {
        serverLog(2,"Connecting to MASTER %s:%d",
            server.masterhost, server.masterport);
        if (connectWithMaster() == 0) {
            serverLog(2,"MASTER <-> REPLICA sync started");
        }
    }
    ...
    if (((server . slaves)->len)) {
        ...
        while((ln = listNext(&li))) {
            ...
            if (slave->replstate != 9) continue;
            if (slave->flags & (1<<16)) continue;
            if ((server.unixtime - slave->repl_ack_time) > server.repl_timeout)
            {
                serverLog(3, "Disconnecting timedout replica: %s",
                    replicationGetSlaveName(slave));
                freeClient(slave);
            }
        }
    ...
    if (((server . slaves)->len) == 0 && server.repl_backlog_time_limit &&
        server.repl_backlog && server.masterhost == ((void *)0))
    {
    ...
    if (((server . slaves)->len) == 0 &&
        server.aof_state == 0 &&
        ((server . repl_scriptcache_fifo)->len) != 0)
    {
