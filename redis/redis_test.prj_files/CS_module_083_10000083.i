...
# 19 "/home/gavin/codesonar-5.0p0.20180811-x86_64-pc-linux/codesonar-5.0p0/csurf/csinclude/stddef.h"
# 25 "/usr/include/dlfcn.h"

 
# 1 "/usr/include/x86_64-linux-gnu/bits/dlfcn.h"
...
static pthread_mutex_t moduleUnblockedClientsMutex = { { 0, 0, 0, 0, 0, 0, 0, { 0, 0 } } };
static list *moduleUnblockedClients;


 
static pthread_mutex_t moduleGIL = { { 0, 0, 0, 0, 0, 0, 0, { 0, 0 } } };


 
typedef int (*RedisModuleNotificationFunc) (RedisModuleCtx *ctx, int type, const char *event, robj *key);
...
void moduleFreeContext(RedisModuleCtx *ctx) {
    ...
    if (ctx->flags & (1<<5)) freeClient(ctx->client);
}
...
void RedisModuleCommandDispatcher(client *c) {
    ...
    RedisModuleCtx ctx = {(void*)(unsigned long)&RM_GetApi, ((void *)0), ((void *)0), ((void *)0), ((void *)0), 0, 0, 0, ((void *)0), 0, ((void *)0), ((void *)0), 0, ((void *)0)};
...
int commandFlagsFromString(char *s) {
    ...
    for (j = 0; j < count; j++) {
        char *t = tokens[j];
        if (!strcasecmp(t,"write")) flags |= (1ULL<<0);
        else if (!strcasecmp(t,"readonly")) flags |= (1ULL<<1);
        else if (!strcasecmp(t,"admin")) flags |= (1ULL<<4);
        else if (!strcasecmp(t,"deny-oom")) flags |= (1ULL<<2);
        else if (!strcasecmp(t,"deny-script")) flags |= (1ULL<<6);
        else if (!strcasecmp(t,"allow-loading")) flags |= (1ULL<<9);
        else if (!strcasecmp(t,"pubsub")) flags |= (1ULL<<5);
        else if (!strcasecmp(t,"random")) flags |= (1ULL<<7);
        else if (!strcasecmp(t,"allow-stale")) flags |= (1ULL<<10);
        else if (!strcasecmp(t,"no-monitor")) flags |= (1ULL<<11);
        else if (!strcasecmp(t,"fast")) flags |= (1ULL<<13);
        else if (!strcasecmp(t,"getkeys-api")) flags |= (1ULL<<14);
        else if (!strcasecmp(t,"no-cluster")) flags |= (1ULL<<15);
        else break;
    }
...
int RM_CreateCommand(RedisModuleCtx *ctx, const char *name, RedisModuleCmdFunc cmdfunc, const char *strflags, int firstkey, int lastkey, int keystep) {
    ...
    if (flags == -1) return 1;
    if ((flags & (1ULL<<15)) && server.cluster_enabled)
        return 1;
    ...
    cp->rediscmd->arity = -1;
    cp->rediscmd->flags = flags | (1ULL<<3);
...
void autoMemoryCollect(RedisModuleCtx *ctx) {
    ...
    for (j = 0; j < ctx->amqueue_used; j++) {
        ...
        case 4: RM_FreeDict(((void *)0),ptr); break;
        }
    }
    ctx->flags |= (1<<1);
    zfree(ctx->amqueue);
...
robj *RM_CreateString(RedisModuleCtx *ctx, const char *ptr, size_t len) {
    ...
    if (ctx != ((void *)0)) autoMemoryAdd(ctx,1,o);
    return o;
}
...
robj *RM_CreateStringPrintf(RedisModuleCtx *ctx, const char *fmt, ...) {
    sds s = sdsempty();
...
robj *RM_CreateStringFromString(RedisModuleCtx *ctx, const robj *str) {
    robj *o = dupStringObject(str);
    if (ctx != ((void *)0)) autoMemoryAdd(ctx,1,o);
    return o;
}
...
void RM_FreeString(RedisModuleCtx *ctx, robj *str) {
    decrRefCount(str);
    if (ctx != ((void *)0)) autoMemoryFreed(ctx,1,str);
}
...
void RM_RetainString(RedisModuleCtx *ctx, robj *str) {
    if (ctx == ((void *)0) || !autoMemoryFreed(ctx,1,str)) {
...
const char *RM_StringPtrLen(const robj *str, size_t *len) {
    if (str == ((void *)0)) {
...
int RM_StringToLongLong(const robj *str, long long *ll) {
...
int RM_StringToDouble(const robj *str, double *d) {
...
int RM_StringCompare(robj *a, robj *b) {
    return compareStringObjects(a,b);
}


 
robj *moduleAssertUnsharedString(robj *str) {
    if (str->refcount != 1) {
        serverLog(3,
...
int RM_StringAppendBuffer(RedisModuleCtx *ctx, robj *str, const char *buf, size_t len) {
    ((void) ctx);
...
void moduleReplicateMultiIfNeeded(RedisModuleCtx *ctx) {
    ...
    if (ctx->client->flags & ((1<<3)|(1<<8))) return;
     
    if (ctx->flags & (1<<0)) return;
    

 
    if (ctx->flags & (1<<5)) return;
...
int RM_GetContextFlags(RedisModuleCtx *ctx) {
    ...
    if (ctx->client) {
        if (ctx->client->flags & (1<<8))
         flags |= (1<<0);
        if (ctx->client->flags & (1<<3))
         flags |= (1<<1);
         
        if (ctx->client->flags & (1<<1))
         flags |= (1<<12);
    }
    ...
    if (retval == -1) flags |= (1<<10);
    if (level > 0.75) flags |= (1<<11);
...
int zsetInitScoreRange(RedisModuleKey *key, double min, double max, int minex, int maxex, int first) {
    ...
    if (key->value->encoding == 5) {
        ...
        _serverPanic("module.c",1993,"Unsupported zset encoding"),_exit(1);
    }
...
int zsetInitLexRange(RedisModuleKey *key, robj *min, robj *max, int first) {
    ...
    if (key->value->encoding == 5) {
        ...
        _serverPanic("module.c",2057,"Unsupported zset encoding"),_exit(1);
    }
    if (key->zcurrent == ((void *)0)) key->zer = 1;
...
robj *RM_ZsetRangeCurrentElement(RedisModuleKey *key, double *score) {
    ...
    if (key->value->encoding == 5) {
        ...
        _serverPanic("module.c",2107,"Unsupported zset encoding"),_exit(1);
    }
...
int RM_ZsetRangeNext(RedisModuleKey *key) {
    ...
    if (key->value->encoding == 5) {
        ...
        _serverPanic("module.c",2172,"Unsupported zset encoding"),_exit(1);
    }
}
...
int RM_ZsetRangePrev(RedisModuleKey *key) {
    ...
    if (key->value->encoding == 5) {
        ...
        _serverPanic("module.c",2235,"Unsupported zset encoding"),_exit(1);
    }
}
...
int RM_HashSet(RedisModuleKey *key, int flags, ...) {
    ...
    while(1) {
        ...
        if (flags & (1<<2)) {
            char *cfield = (*(char* *)(((*(char* *)&(ap)) += sizeof(char*)) - sizeof(char*)));
            if (cfield == ((void *)0)) break;
            field = createRawStringObject(cfield,strlen(cfield));
        } else {
            field = (*(robj* *)(((*(char* *)&(ap)) += sizeof(robj*)) - sizeof(robj*)));
            if (field == ((void *)0)) break;
        }
        value = (*(robj* *)(((*(char* *)&(ap)) += sizeof(robj*)) - sizeof(robj*)));
...
int RM_HashGet(RedisModuleKey *key, int flags, ...) {
    ...
    while(1) {
        ...
        if (flags & (1<<3)) {
            existsptr = (*(int* *)(((*(char* *)&(ap)) += sizeof(int*)) - sizeof(int*)));
            if (key->value)
                *existsptr = hashTypeExists(key->value,field->ptr);
            else
                *existsptr = 0;
        } else {
            valueptr = (*(robj* * *)(((*(char* *)&(ap)) += sizeof(robj* *)) - sizeof(robj* *)));
            if (key->value) {
...
long long RM_CallReplyInteger(RedisModuleCallReply *reply) {
    moduleParseCallReply(reply);
    if (reply->type != 2) return (-0x7fffffffffffffffLL - 1LL);
    return reply->val.ll;
}
...
robj **moduleCreateArgvFromUserFormat(const char *cmdname, const char *fmt, int *argcp, int *flags, va_list ap) {
    ...
    while(*p) {
        if (*p == 'c') {
            char *cstr = (*(char* *)(((*(char* *)&(ap)) += sizeof(char*)) - sizeof(char*)));
            argv[argc++] = createStringObject(cstr,strlen(cstr));
        } else if (*p == 's') {
            robj *obj = (*(void* *)(((*(char* *)&(ap)) += sizeof(void*)) - sizeof(void*)));
            argv[argc++] = obj;
            incrRefCount(obj);
        } else if (*p == 'b') {
            char *buf = (*(char* *)(((*(char* *)&(ap)) += sizeof(char*)) - sizeof(char*)));
            size_t len = (*(size_t*)(((*(char* *)&(ap)) += sizeof(size_t)) - sizeof(size_t)));
            argv[argc++] = createStringObject(buf,len);
        } else if (*p == 'l') {
            long ll = (*(long long*)(((*(char* *)&(ap)) += sizeof(long long)) - sizeof(long long)));
            argv[argc++] = createObject(0,sdsfromlonglong(ll));
        } else if (*p == 'v') {
              
             robj **v = (*(void* *)(((*(char* *)&(ap)) += sizeof(void*)) - sizeof(void*)));
             size_t vlen = (*(size_t*)(((*(char* *)&(ap)) += sizeof(size_t)) - sizeof(size_t)));
...
moduleType *moduleTypeLookupModuleByID(uint64_t id) {
    ...
    int j;
    for (j = 0; j < 3 && cache[j].mt != ((void *)0); j++)
    ...
    while ((de = dictNext(di)) != ((void *)0) && mt == ((void *)0)) {
        struct RedisModule *module = ((de)->v . val);
        listIter li;
        listNode *ln;
...
moduleType *RM_CreateDataType(RedisModuleCtx *ctx, const char *name, int encver, void *typemethods_ptr) {
    ...
    if (typemethods_version == 0) return ((void *)0);
...
void RM_SaveUnsigned(RedisModuleIO *io, uint64_t value) {
    ...
    int retval = rdbSaveLen(io->rio, 2);
...
void RM_SaveString(RedisModuleIO *io, robj *s) {
    ...
    ssize_t retval = rdbSaveLen(io->rio, 5);
    if (retval == -1) goto saveerr;
    io->bytes += retval;
...
void RM_SaveDouble(RedisModuleIO *io, double value) {
    ...
    int retval = rdbSaveLen(io->rio, 4);
    if (retval == -1) goto saveerr;
    io->bytes += retval;
...
void RM_SaveFloat(RedisModuleIO *io, float value) {
    ...
    int retval = rdbSaveLen(io->rio, 3);
    if (retval == -1) goto saveerr;
    io->bytes += retval;
...
RedisModuleCtx *RM_GetContextFromIO(RedisModuleIO *io) {
    ...
    RedisModuleCtx ctxtemplate = {(void*)(unsigned long)&RM_GetApi, ((void *)0), ((void *)0), ((void *)0), ((void *)0), 0, 0, 0, ((void *)0), 0, ((void *)0), ((void *)0), 0, ((void *)0)};
...
const robj *RM_GetKeyNameFromIO(RedisModuleIO *io) {
    return io->key;
}
...
void RM_LogRaw(RedisModule *module, const char *levelstr, const char *fmt, va_list ap) {
    ...
    if (!strcasecmp(levelstr,"debug")) level = 0;
    else if (!strcasecmp(levelstr,"verbose")) level = 1;
    else if (!strcasecmp(levelstr,"notice")) level = 2;
    else if (!strcasecmp(levelstr,"warning")) level = 3;
    else level = 1;  
...
RedisModuleBlockedClient *RM_BlockClient(RedisModuleCtx *ctx, RedisModuleCmdFunc reply_callback, RedisModuleCmdFunc timeout_callback, void (*free_privdata)(RedisModuleCtx*,void*), long long timeout_ms) {
    client *c = ctx->client;
    int islua = c->flags & (1<<8);
    int ismulti = c->flags & (1<<3);
...
void moduleHandleBlockedClients(void) {
    ...
    while (((moduleUnblockedClients)->len)) {
...
RedisModuleCtx *RM_GetThreadSafeContext(RedisModuleBlockedClient *bc) {
    ...
    RedisModuleCtx empty = {(void*)(unsigned long)&RM_GetApi, ((void *)0), ((void *)0), ((void *)0), ((void *)0), 0, 0, 0, ((void *)0), 0, ((void *)0), ((void *)0), 0, ((void *)0)};
...
void moduleNotifyKeyspaceEvent(int type, const char *event, robj *key, int dbid) {
    ...
    if (((moduleKeyspaceSubscribers)->len) == 0) return;
...
static moduleClusterReceiver *clusterReceivers[(255)];
...
int RM_SendClusterMessage(RedisModuleCtx *ctx, char *target_id, uint8_t type, unsigned char *msg, uint32_t len) {
    ...
    uint64_t module_id = moduleTypeEncodeId(ctx->module->name,0);
    if (clusterSendModuleMessageToTarget(target_id,module_id,type,msg,len) == 0)
        return 0;
    else
        return 1;
}
...
char **RM_GetClusterNodesList(RedisModuleCtx *ctx, size_t *numnodes) {
    ...
    size_t count = ((server . cluster->nodes)->ht[0]. used+(server . cluster->nodes)->ht[1]. used);
    char **ids = zmalloc((count+1)*40);
    dictIterator *di = dictGetIterator(server.cluster->nodes);
    dictEntry *de;
    int j = 0;
    while((de = dictNext(di)) != ((void *)0)) {
        clusterNode *node = ((de)->v . val);
        if (node->flags & (64|32)) continue;
        ids[j] = zmalloc(40);
        memcpy(ids[j],node->name,40);
        j++;
    }
    *numnodes = j;
    ids[j] = ((void *)0); 
...
size_t RM_GetClusterSize(void) {
    ...
    return ((server . cluster->nodes)->ht[0]. used+(server . cluster->nodes)->ht[1]. used);
}
...
int RM_GetClusterNodeInfo(RedisModuleCtx *ctx, const char *id, char *ip, char *master_id, int *port, int *flags) {
    ...
    if (ip) memcpy(ip,node->name,40);

    if (master_id) {
        

 
        if (node->flags & 1 && node->slaveof)
            memcpy(master_id,node->slaveof->name,40);
        else
            memset(master_id,0,40);
    }
    ...
    if (flags) {
        *flags = 0;
        if (node->flags & 16) *flags |= (1<<0);
        if (node->flags & 1) *flags |= (1<<1);
        if (node->flags & 2) *flags |= (1<<2);
        if (node->flags & 4) *flags |= (1<<3);
        if (node->flags & 8) *flags |= (1<<4);
        if (node->flags & 512) *flags |= (1<<5);
    }
    return 0;
}
...
void RM_SetClusterFlags(RedisModuleCtx *ctx, uint64_t flags) {
    ((void) ctx);
    if (flags & (1<<1))
        server.cluster_module_flags |= (1<<1);
    if (flags & (1<<2))
        server.cluster_module_flags |= (1<<2);
}
...
int moduleTimerHandler(struct aeEventLoop *eventLoop, long long id, void *clientData) {
    ...
    return (raxSize(Timers) > 0) ? next_period : -1;
}
...
int RM_GetTimerInfo(RedisModuleCtx *ctx, RedisModuleTimerID id, uint64_t *remaining, void **data) {
    ...
    if (timer == raxNotFound || timer->module != ctx->module)
        return 1;
    if (remaining) {
        int64_t rem = intrev64(id)-ustime();
        if (rem < 0) rem = 0;
        *remaining = rem/1000;  
    }
    if (data) *data = timer->data;
...
RedisModuleDict *RM_CreateDict(RedisModuleCtx *ctx) {
    ...
    d->rax = raxNew();
    if (ctx != ((void *)0)) autoMemoryAdd(ctx,4,d);
    return d;
}
...
void RM_FreeDict(RedisModuleCtx *ctx, RedisModuleDict *d) {
    if (ctx != ((void *)0)) autoMemoryFreed(ctx,4,d);
    raxFree(d->rax);
    zfree(d);
}
...
int RM_DictReplaceC(RedisModuleDict *d, void *key, size_t keylen, void *ptr) {
    int retval = raxInsert(d->rax,key,keylen,ptr,((void *)0));
    return (retval == 1) ? 0 : 1;
}

 
int RM_DictSet(RedisModuleDict *d, robj *key, void *ptr) {
    return RM_DictSetC(d,key->ptr,sdslen(key->ptr),ptr);
}

 
int RM_DictReplace(RedisModuleDict *d, robj *key, void *ptr) {
...
void *RM_DictGet(RedisModuleDict *d, robj *key, int *nokey) {
...
int RM_DictDel(RedisModuleDict *d, robj *key, void *oldval) {
...
RedisModuleDictIter *RM_DictIteratorStart(RedisModuleDict *d, const char *op, robj *key) {
...
int RM_DictIteratorReseek(RedisModuleDictIter *di, const char *op, robj *key) {
...
robj *RM_DictNext(RedisModuleCtx *ctx, RedisModuleDictIter *di, void **dataptr) {
    size_t keylen;
...
robj *RM_DictPrev(RedisModuleCtx *ctx, RedisModuleDictIter *di, void **dataptr) {
    size_t keylen;
...
int RM_DictCompare(RedisModuleDictIter *di, const char *op, robj *key) {
    if (raxEOF(&di->ri)) return 1;
...
int RM_ExportSharedAPI(RedisModuleCtx *ctx, const char *apiname, void *func) {
    ...
    sapi->func = func;
    if (dictAdd(server.sharedapi, (char*)apiname, sapi) != 0) {
        zfree(sapi);
        return 1;
    }
    return 0;
...
void *RM_GetSharedAPI(RedisModuleCtx *ctx, const char *apiname) {
    ...
    RedisModuleSharedAPI *sapi = ((de)->v . val);
...
int RM_UnregisterCommandFilter(RedisModuleCtx *ctx, RedisModuleCommandFilter *filter) {
    ...
    if (filter->module != ctx->module) return 1;
...
void moduleCallCommandFilters(client *c) {
    if (((moduleCommandFilters)->len) == 0) return;
    ...
    while((ln = listNext(&li))) {
        ...
        if ((f->flags & (1<<0)) && f->module->in_call) continue;
...
const robj *RM_CommandFilterArgGet(RedisModuleCommandFilterCtx *fctx, int pos)
{
...
int RM_CommandFilterArgInsert(RedisModuleCommandFilterCtx *fctx, int pos, robj *arg)
{
    int i;

    if (pos < 0 || pos > fctx->argc) return 1;

    fctx->argv = zrealloc(fctx->argv, (fctx->argc+1)*sizeof(robj *));
...
int RM_CommandFilterArgReplace(RedisModuleCommandFilterCtx *fctx, int pos, robj *arg)
{
    if (pos < 0 || pos >= fctx->argc) return 1;
...
void moduleInitModulesSystem(void) {
    ...
    moduleFreeContextReusedClient->flags |= (1<<27);
...
int moduleUnload(sds name) {
    ...
    if (module == ((void *)0)) {
        errno = 2;
        return 1;
    } else if (((module ->types)->len)) {
        errno = 16;
        return 1;
    } else if (((module ->usedby)->len)) {
        errno = 1;
        return 1;
    }
...
void addReplyLoadedModules(client *c) {
    ...
    addReplyArrayLen(c,((modules)->ht[0]. used+(modules)->ht[1]. used));
    while ((de = dictNext(di)) != ((void *)0)) {
        sds name = ((de)->key);
        struct RedisModule *module = ((de)->v . val);
        addReplyMapLen(c,2);
...
size_t moduleCount(void) {
    return ((modules)->ht[0]. used+(modules)->ht[1]. used);
}
...
void moduleRegisterCoreAPI(void) {
    ...
    moduleRegisterApi("RedisModule_" "Calloc", (void *)(unsigned long)RM_Calloc);
    ...
    moduleRegisterApi("RedisModule_" "IsModuleNameBusy", (void *)(unsigned long)RM_IsModuleNameBusy);
    ...
    moduleRegisterApi("RedisModule_" "CreateStringFromString", (void *)(unsigned long)RM_CreateStringFromString);
    moduleRegisterApi("RedisModule_" "CreateStringPrintf", (void *)(unsigned long)RM_CreateStringPrintf);
    ...
    moduleRegisterApi("RedisModule_" "UnlinkKey", (void *)(unsigned long)RM_UnlinkKey);
    ...
    moduleRegisterApi("RedisModule_" "GetContextFlags", (void *)(unsigned long)RM_GetContextFlags);
    ...
    moduleRegisterApi("RedisModule_" "SaveFloat", (void *)(unsigned long)RM_SaveFloat);
    moduleRegisterApi("RedisModule_" "LoadFloat", (void *)(unsigned long)RM_LoadFloat);
    moduleRegisterApi("RedisModule_" "EmitAOF", (void *)(unsigned long)RM_EmitAOF);
    moduleRegisterApi("RedisModule_" "Log", (void *)(unsigned long)RM_Log);
    moduleRegisterApi("RedisModule_" "LogIOError", (void *)(unsigned long)RM_LogIOError);
    moduleRegisterApi("RedisModule_" "StringAppendBuffer", (void *)(unsigned long)RM_StringAppendBuffer);
    moduleRegisterApi("RedisModule_" "RetainString", (void *)(unsigned long)RM_RetainString);
    moduleRegisterApi("RedisModule_" "StringCompare", (void *)(unsigned long)RM_StringCompare);
    moduleRegisterApi("RedisModule_" "GetContextFromIO", (void *)(unsigned long)RM_GetContextFromIO);
    moduleRegisterApi("RedisModule_" "GetKeyNameFromIO", (void *)(unsigned long)RM_GetKeyNameFromIO);
    moduleRegisterApi("RedisModule_" "BlockClient", (void *)(unsigned long)RM_BlockClient);
    moduleRegisterApi("RedisModule_" "UnblockClient", (void *)(unsigned long)RM_UnblockClient);
    moduleRegisterApi("RedisModule_" "IsBlockedReplyRequest", (void *)(unsigned long)RM_IsBlockedReplyRequest);
    moduleRegisterApi("RedisModule_" "IsBlockedTimeoutRequest", (void *)(unsigned long)RM_IsBlockedTimeoutRequest);
    moduleRegisterApi("RedisModule_" "GetBlockedClientPrivateData", (void *)(unsigned long)RM_GetBlockedClientPrivateData);
    moduleRegisterApi("RedisModule_" "AbortBlock", (void *)(unsigned long)RM_AbortBlock);
    moduleRegisterApi("RedisModule_" "Milliseconds", (void *)(unsigned long)RM_Milliseconds);
    moduleRegisterApi("RedisModule_" "GetThreadSafeContext", (void *)(unsigned long)RM_GetThreadSafeContext);
    moduleRegisterApi("RedisModule_" "FreeThreadSafeContext", (void *)(unsigned long)RM_FreeThreadSafeContext);
    moduleRegisterApi("RedisModule_" "ThreadSafeContextLock", (void *)(unsigned long)RM_ThreadSafeContextLock);
    moduleRegisterApi("RedisModule_" "ThreadSafeContextUnlock", (void *)(unsigned long)RM_ThreadSafeContextUnlock);
    moduleRegisterApi("RedisModule_" "DigestAddStringBuffer", (void *)(unsigned long)RM_DigestAddStringBuffer);
    moduleRegisterApi("RedisModule_" "DigestAddLongLong", (void *)(unsigned long)RM_DigestAddLongLong);
    moduleRegisterApi("RedisModule_" "DigestEndSequence", (void *)(unsigned long)RM_DigestEndSequence);
    moduleRegisterApi("RedisModule_" "SubscribeToKeyspaceEvents", (void *)(unsigned long)RM_SubscribeToKeyspaceEvents);
    moduleRegisterApi("RedisModule_" "RegisterClusterMessageReceiver", (void *)(unsigned long)RM_RegisterClusterMessageReceiver);
    moduleRegisterApi("RedisModule_" "SendClusterMessage", (void *)(unsigned long)RM_SendClusterMessage);
    moduleRegisterApi("RedisModule_" "GetClusterNodeInfo", (void *)(unsigned long)RM_GetClusterNodeInfo);
    moduleRegisterApi("RedisModule_" "GetClusterNodesList", (void *)(unsigned long)RM_GetClusterNodesList);
    moduleRegisterApi("RedisModule_" "FreeClusterNodesList", (void *)(unsigned long)RM_FreeClusterNodesList);
    moduleRegisterApi("RedisModule_" "CreateTimer", (void *)(unsigned long)RM_CreateTimer);
    moduleRegisterApi("RedisModule_" "StopTimer", (void *)(unsigned long)RM_StopTimer);
    moduleRegisterApi("RedisModule_" "GetTimerInfo", (void *)(unsigned long)RM_GetTimerInfo);
    moduleRegisterApi("RedisModule_" "GetMyClusterID", (void *)(unsigned long)RM_GetMyClusterID);
    moduleRegisterApi("RedisModule_" "GetClusterSize", (void *)(unsigned long)RM_GetClusterSize);
    moduleRegisterApi("RedisModule_" "GetRandomBytes", (void *)(unsigned long)RM_GetRandomBytes);
    moduleRegisterApi("RedisModule_" "GetRandomHexChars", (void *)(unsigned long)RM_GetRandomHexChars);
    moduleRegisterApi("RedisModule_" "BlockedClientDisconnected", (void *)(unsigned long)RM_BlockedClientDisconnected);
    moduleRegisterApi("RedisModule_" "SetDisconnectCallback", (void *)(unsigned long)RM_SetDisconnectCallback);
    moduleRegisterApi("RedisModule_" "GetBlockedClientHandle", (void *)(unsigned long)RM_GetBlockedClientHandle);
    moduleRegisterApi("RedisModule_" "SetClusterFlags", (void *)(unsigned long)RM_SetClusterFlags);
    moduleRegisterApi("RedisModule_" "CreateDict", (void *)(unsigned long)RM_CreateDict);
    moduleRegisterApi("RedisModule_" "FreeDict", (void *)(unsigned long)RM_FreeDict);
    moduleRegisterApi("RedisModule_" "DictSize", (void *)(unsigned long)RM_DictSize);
    moduleRegisterApi("RedisModule_" "DictSetC", (void *)(unsigned long)RM_DictSetC);
    moduleRegisterApi("RedisModule_" "DictReplaceC", (void *)(unsigned long)RM_DictReplaceC);
    moduleRegisterApi("RedisModule_" "DictSet", (void *)(unsigned long)RM_DictSet);
    moduleRegisterApi("RedisModule_" "DictReplace", (void *)(unsigned long)RM_DictReplace);
    moduleRegisterApi("RedisModule_" "DictGetC", (void *)(unsigned long)RM_DictGetC);
    moduleRegisterApi("RedisModule_" "DictGet", (void *)(unsigned long)RM_DictGet);
    moduleRegisterApi("RedisModule_" "DictDelC", (void *)(unsigned long)RM_DictDelC);
    moduleRegisterApi("RedisModule_" "DictDel", (void *)(unsigned long)RM_DictDel);
    moduleRegisterApi("RedisModule_" "DictIteratorStartC", (void *)(unsigned long)RM_DictIteratorStartC);
    moduleRegisterApi("RedisModule_" "DictIteratorStart", (void *)(unsigned long)RM_DictIteratorStart);
    moduleRegisterApi("RedisModule_" "DictIteratorStop", (void *)(unsigned long)RM_DictIteratorStop);
    moduleRegisterApi("RedisModule_" "DictIteratorReseekC", (void *)(unsigned long)RM_DictIteratorReseekC);
    moduleRegisterApi("RedisModule_" "DictIteratorReseek", (void *)(unsigned long)RM_DictIteratorReseek);
    moduleRegisterApi("RedisModule_" "DictNextC", (void *)(unsigned long)RM_DictNextC);
    moduleRegisterApi("RedisModule_" "DictPrevC", (void *)(unsigned long)RM_DictPrevC);
    moduleRegisterApi("RedisModule_" "DictNext", (void *)(unsigned long)RM_DictNext);
    moduleRegisterApi("RedisModule_" "DictPrev", (void *)(unsigned long)RM_DictPrev);
    moduleRegisterApi("RedisModule_" "DictCompareC", (void *)(unsigned long)RM_DictCompareC);
    moduleRegisterApi("RedisModule_" "DictCompare", (void *)(unsigned long)RM_DictCompare);
    moduleRegisterApi("RedisModule_" "ExportSharedAPI", (void *)(unsigned long)RM_ExportSharedAPI);
    moduleRegisterApi("RedisModule_" "GetSharedAPI", (void *)(unsigned long)RM_GetSharedAPI);
    moduleRegisterApi("RedisModule_" "RegisterCommandFilter", (void *)(unsigned long)RM_RegisterCommandFilter);
    moduleRegisterApi("RedisModule_" "UnregisterCommandFilter", (void *)(unsigned long)RM_UnregisterCommandFilter);
    moduleRegisterApi("RedisModule_" "CommandFilterArgsCount", (void *)(unsigned long)RM_CommandFilterArgsCount);
    moduleRegisterApi("RedisModule_" "CommandFilterArgGet", (void *)(unsigned long)RM_CommandFilterArgGet);
    moduleRegisterApi("RedisModule_" "CommandFilterArgInsert", (void *)(unsigned long)RM_CommandFilterArgInsert);
    moduleRegisterApi("RedisModule_" "CommandFilterArgReplace", (void *)(unsigned long)RM_CommandFilterArgReplace);
    moduleRegisterApi("RedisModule_" "CommandFilterArgDelete", (void *)(unsigned long)RM_CommandFilterArgDelete);
}
