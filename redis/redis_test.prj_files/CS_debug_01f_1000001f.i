...
void xorObjectDigest(redisDb *db, robj *keyobj, unsigned char *digest, robj *o) {
    ...
    if (o->type == 0) {
        ...
        listTypeIterator *li = listTypeInitIterator(o,0,1);
        listTypeEntry entry;
        ...
        sds sdsele;
        while((sdsele = setTypeNextObject(si)) != ((void *)0)) {
        ...
        if (o->encoding == 5) {
            ...
            ((eptr != ((void *)0))?(void)0 : (_serverAssert("eptr != NULL","debug.c",159),_exit(1)));
            sptr = ziplistNext(zl,eptr);
            ((sptr != ((void *)0))?(void)0 : (_serverAssert("sptr != NULL","debug.c",161),_exit(1)));

            while (eptr != ((void *)0)) {
                ((ziplistGet(eptr,&vstr,&vlen,&vll))?(void)0 : (_serverAssert("ziplistGet(eptr,&vstr,&vlen,&vll)","debug.c",164),_exit(1)));
            ...
            _serverPanic("debug.c",197,"Unknown sorted set encoding"),_exit(1);
        }
    } else if (o->type == 4) {
        ...
        while (hashTypeNext(hi) != -1) {
            ...
            memset(eledigest,0,20);
            sdsele = hashTypeCurrentObjectNewSds(hi,1);
            mixDigest(eledigest,sdsele,sdslen(sdsele));
            sdsfree(sdsele);
            sdsele = hashTypeCurrentObjectNewSds(hi,2);
            mixDigest(eledigest,sdsele,sdslen(sdsele));
            sdsfree(sdsele);
            xorDigest(digest,eledigest,20);
        }
        ...
        streamIterator si;
        streamIteratorStart(&si,o->ptr,((void *)0),((void *)0),0);
        streamID id;
        int64_t numfields;
        ...
        do { memset(md . o,0,sizeof(md . o)); memset(md . x,0,sizeof(md . x)); } while(0);;
        if (mt->digest) {
            mt->digest(&md,mv->value);
            xorDigest(digest,md.x,sizeof(md.x));
        }
    } else {
        _serverPanic("debug.c",246,"Unknown object type"),_exit(1);
    }
...
void computeDatasetDigest(unsigned char *final) {
    ...
    for (j = 0; j < server.dbnum; j++) {
        ...
        if (((db->dict)->ht[0]. used+(db->dict)->ht[1]. used) == 0) continue;
...
((void *)0)
        ...
        *((char*)-1) = 'x';
        ...
        _serverPanic("debug.c",332,"DEBUG PANIC called at Unix time %ld", time(((void *)0))),_exit(1);
               ...
               !strcasecmp(c->argv[1]->ptr,"crash-and-recover"))
    {
        ...
        void *ptr = zmalloc((0x7fffffffffffffffL * 2UL + 1UL));  
        zfree(ptr);
        addReply(c,shared.ok);
    } else if (!strcasecmp(c->argv[1]->ptr,"assert")) {
        ((1 == 2)?(void)0 : (_serverAssertWithInfo(c,c->argv[0],"1 == 2","debug.c",352),_exit(1)));
    } else if (!strcasecmp(c->argv[1]->ptr,"log") && c->argc == 3) {
        serverLog(3, "DEBUG LOG: %s", (char*)c->argv[2]->ptr);
        addReply(c,shared.ok);
        ...
        emptyDb(-1,0,((void *)0));
        protectClient(c);
        int ret = rdbLoad(server.rdb_filename,((void *)0));
        unprotectClient(c);
        if (ret != 0) {
        ...
        if (server.aof_state != 0) flushAppendOnlyFile(1);
        emptyDb(-1,0,((void *)0));
        protectClient(c);
        ...
        if (val->type != 0 || !(val->encoding == 0 || val->encoding == 8)) {
        ...
        if (getLongFromObjectOrReply(c, c->argv[2], &keys, ((void *)0)) != 0)
            return;
        ...
        for (j = 0; j < keys; j++) {
            ...
            if (c->argc == 5)
                if (getLongFromObjectOrReply(c, c->argv[4], &valsize, ((void *)0)) != 0)
                    return;
            ...
            if (valsize==0)
                ...
                val = createStringObject(((void *)0),valsize);
        ...
        for (int j = 2; j < c->argc; j++) {
            unsigned char digest[20];
            memset(digest,0,20);  
            robj *o = lookupKeyReadWithFlags(c->db,c->argv[j],(1<<0));
               ...
               c->argc == 3)
    {
        ...
        if (getLongFromObjectOrReply(c, c->argv[2], &dbid, ((void *)0)) != 0)
            return;
...
void _serverAssert(const char *estr, const char *file, int line) {
    ...
    serverLog(3,"==> %s:%d '%s' is not true",file,line,estr);
...
void _serverAssertPrintClientInfo(const client *c) {
    ...
    serverLog(3,"client->flags = %d", c->flags);
    serverLog(3,"client->fd = %d", c->fd);
    serverLog(3,"client->argc = %d", c->argc);
    for (j=0; j < c->argc; j++) {
        char buf[128];
        char *arg;

        if (c->argv[j]->type == 0 && (c->argv[j]->encoding == 0 || c->argv[j]->encoding == 8)) {
        ...
        serverLog(3,"client->argv[%d] = \"%s\" (refcount: %d)",
            j, arg, c->argv[j]->refcount);
    }
}

void serverLogObjectDebugInfo(const robj *o) {
    serverLog(3,"Object type: %d", o->type);
    serverLog(3,"Object encoding: %d", o->encoding);
    serverLog(3,"Object refcount: %d", o->refcount);
    if (o->type == 0 && (o->encoding == 0 || o->encoding == 8)) {
        serverLog(3,"Object raw string len: %zu", sdslen(o->ptr));
        if (sdslen(o->ptr) < 4096) {
            sds repr = sdscatrepr(sdsempty(),o->ptr,sdslen(o->ptr));
            serverLog(3,"Object raw string content: %s", repr);
            sdsfree(repr);
        }
    } else if (o->type == 1) {
        serverLog(3,"List length: %d", (int) listTypeLength(o));
    } else if (o->type == 2) {
        serverLog(3,"Set size: %d", (int) setTypeSize(o));
    } else if (o->type == 4) {
        serverLog(3,"Hash size: %d", (int) hashTypeLength(o));
    } else if (o->type == 3) {
        serverLog(3,"Sorted set size: %d", (int) zsetLength(o));
        if (o->encoding == 7)
            serverLog(3,"Skiplist level: %d", (int) ((const zset*)o->ptr)->zsl->level);
    }
}
...
void _serverPanic(const char *file, int line, const char *msg, ...) {
    ...
    serverLog(3,"Guru Meditation: %s #%s:%d",fmtmsg,file,line);
...
void logStackContent(void **sp) {
    ...
    for (i = 15; i >= 0; i--) {
        ...
        if (sizeof(long) == 4)
            serverLog(3, "(%08lx) -> %08lx", addr, val);
        else
            serverLog(3, "(%016lx) -> %016lx", addr, val);
    }
}
...
void logCurrentClient(void) {
    ...
    if (cc->argc >= 1) {
        ...
        if (de) {
            val = ((de)->v . val);
            serverLog(3,"key '%s' found in DB containing the following object:", (char*)key->ptr);
...
int memtest_test_linux_anonymous_maps(void) {
    ...
    size_t start_vect[128];
    size_t size_vect[128];
    int regions = 0, j;
...
void sigsegvHandler(int sig, siginfo_t *info, void *secret) {
    ...
    bugReportStart();
    serverLog(3,
        "Redis %s crashed by signal: %d", "999.999.999", sig);
    if (eip != ((void *)0)) {
        serverLog(3,
    ...
    if (sig == 11 || sig == 7) {
        serverLog(3,
        "Accessing address: %p", (void*)info->_sifields . _sigfault . si_addr);
    }
    serverLog(3,
    ...
    serverLogRaw(3|(1<<10), infostring);
    serverLogRaw(3|(1<<10), "\n------ CLIENT LIST OUTPUT ------\n");
    clients = getAllClientsInfoString(-1);
    serverLogRaw(3|(1<<10), clients);
    sdsfree(infostring);
    sdsfree(clients);
    ...
    if (eip != ((void *)0)) {
        ...
        if (dladdr(eip, &info) != 0) {
            ...
            if (len < 1<<13) {  
                ...
                serverLogHexDump(3, "dump of function",
                    info.dli_saddr ,len);
...
"  Suspect RAM error? Use redis-server --test-memory to verify it.\n\n"
    ...
    act.sa_flags = 0x40000000 | 0x08000000 | 0x80000000;
...
void serverLogHexDump(int level, char *descr, void *value, size_t len) {
    ...
    while(len) {
        ...
        v++;
        if (b-buf == 64 || len == 0) {
            serverLogRaw(level|(1<<10),buf);
            b = buf;
        }
    }
    serverLogRaw(level|(1<<10),"\n");
}
