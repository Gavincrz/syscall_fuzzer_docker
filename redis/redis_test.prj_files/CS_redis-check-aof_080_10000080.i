...
int consumeNewline(char *buf) {
    if (strncmp(buf,"\r\n",2) != 0) {
        { char __buf[1024]; snprintf(__buf, sizeof(__buf), "Expected \\r\\n, got: %02x%02x",buf[0],buf[1]); snprintf(error, sizeof(error), "0x%16llx: %s", (long long)epos, __buf); };
        return 0;
    }
    return 1;
}
...
int readLong(FILE *fp, char prefix, long *target) {
    ...
    if (buf[0] != prefix) {
        { char __buf[1024]; snprintf(__buf, sizeof(__buf), "Expected prefix '%c', got: '%c'",prefix,buf[0]); snprintf(error, sizeof(error), "0x%16llx: %s", (long long)epos, __buf); };
        return 0;
    }
...
int readBytes(FILE *fp, char *target, long length) {
    ...
    if (real != length) {
        { char __buf[1024]; snprintf(__buf, sizeof(__buf), "Expected to read %ld bytes, got %ld bytes",length,real); snprintf(error, sizeof(error), "0x%16llx: %s", (long long)epos, __buf); };
        return 0;
    }
    return 1;
}
...
off_t process(FILE *fp) {
        ...
        for (i = 0; i < argc; i++) {
            ...
            if (i == 0) {
                if (strcasecmp(str, "multi") == 0) {
                    if (multi++) {
                        { char __buf[1024]; snprintf(__buf, sizeof(__buf), "Unexpected MULTI"); snprintf(error, sizeof(error), "0x%16llx: %s", (long long)epos, __buf); };
                        break;
                    }
                } else if (strcasecmp(str, "exec") == 0) {
                    if (--multi) {
                        { char __buf[1024]; snprintf(__buf, sizeof(__buf), "Unexpected EXEC"); snprintf(error, sizeof(error), "0x%16llx: %s", (long long)epos, __buf); };
                        break;
                    }
                }
            }
    ...
    if (feof(fp) && multi && strlen(error) == 0) {
        { char __buf[1024]; snprintf(__buf, sizeof(__buf), "Reached EOF before reading EXEC for MULTI"); snprintf(error, sizeof(error), "0x%16llx: %s", (long long)epos, __buf); };
    }
    if (strlen(error) > 0) {
        printf("%s\n", error);
    }
...
int redis_check_aof_main(int argc, char **argv) {
    ...
    if (size >= 8) {     
        ...
        if (has_preamble) {
            ...
            if (redis_check_rdb_main(argc,argv,fp) == -1) {
