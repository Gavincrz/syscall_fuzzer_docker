...
void lwDrawSquare(lwCanvas *canvas, int x, int y, float size, float angle) {
    ...
    float k = 3.14159265358979323846/4 + angle;
    for (int j = 0; j < 4; j++) {
        px[j] = round(sin(k) * size + x);
        py[j] = round(cos(k) * size + y);
        k += 3.14159265358979323846/2;
    }
...
lwCanvas *lwDrawSchotter(int console_cols, int squares_per_row, int squares_per_col) {
    ...
    for (int y = 0; y < squares_per_col; y++) {
        for (int x = 0; x < squares_per_row; x++) {
            ...
            float angle = 0;
            if (y > 1) {
                float r1 = (float)rand() / 2147483647 / squares_per_col * y;
                float r2 = (float)rand() / 2147483647 / squares_per_col * y;
                float r3 = (float)rand() / 2147483647 / squares_per_col * y;
                if (rand() % 2) r1 = -r1;
                if (rand() % 2) r2 = -r2;
                if (rand() % 2) r3 = -r3;
                angle = r1;
...
void lolwut5Command(client *c) {
    ...
    if (c->argc > 1 &&
        getLongFromObjectOrReply(c,c->argv[1],&cols,((void *)0)) != 0)
        return;

    if (c->argc > 2 &&
        getLongFromObjectOrReply(c,c->argv[2],&squares_per_row,((void *)0)) != 0)
        return;

    if (c->argc > 3 &&
        getLongFromObjectOrReply(c,c->argv[3],&squares_per_col,((void *)0)) != 0)
        return;
    ...
    rendered = sdscat(rendered,"999.999.999");
