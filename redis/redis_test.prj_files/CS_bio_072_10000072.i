...
static pthread_t bio_threads[3];
static pthread_mutex_t bio_mutex[3];
static pthread_cond_t bio_newjob_cond[3];
static pthread_cond_t bio_step_cond[3];
static list *bio_jobs[3];
...
static unsigned long long bio_pending[3];
...
void bioInit(void) {
    ...
    for (j = 0; j < 3; j++) {
        ...
        if (pthread_create(&thread,&attr,bioProcessBackgroundJobs,arg) != 0) {
            serverLog(3,"Fatal: Can't initialize Background Jobs.");
            exit(1);
        }
...
void *bioProcessBackgroundJobs(void *arg) {
    ...
    sigemptyset(&sigset);
    sigaddset(&sigset, 14);
    if (pthread_sigmask(0, &sigset, ((void *)0)))
        serverLog(3,
    ...
    while(1) {
        ...
        if (((bio_jobs[type])->len) == 0) {
            pthread_cond_wait(&bio_newjob_cond[type],&bio_mutex[type]);
            continue;
        }
         
        ln = ((bio_jobs[type])->head);
        job = ln->value;
        ...
        if (type == 0) {
            ...
            _serverPanic("bio.c",203,"Wrong job type in bioProcessBackgroundJobs()."),_exit(1);
        }
        zfree(job);
