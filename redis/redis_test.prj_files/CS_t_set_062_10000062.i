...
robj *setTypeCreate(sds value) {
    if (isSdsRepresentableAsLongLong(value,((void *)0)) == 0)
...
int setTypeAdd(robj *subject, sds value) {
    long long llval;
    if (subject->encoding == 2) {
        dict *ht = subject->ptr;
        dictEntry *de = dictAddRaw(ht,value,((void *)0));
        if (de) {
            do { if ((ht)->type ->keyDup) (de)->key = (ht)->type ->keyDup((ht)->privdata, sdsdup(value)); else (de)->key = (sdsdup(value)); } while(0);
            do { if ((ht)->type ->valDup) (de)->v . val = (ht)->type ->valDup((ht)->privdata, ((void *)0)); else (de)->v . val = (((void *)0)); } while(0);
            return 1;
        }
    } else if (subject->encoding == 6) {
        if (isSdsRepresentableAsLongLong(value,&llval) == 0) {
            uint8_t success = 0;
            ...
            ((dictAdd(subject->ptr,sdsdup(value),((void *)0)) == 0)?(void)0 : (_serverAssert("dictAdd(subject->ptr,sdsdup(value),NULL) == DICT_OK","t_set.c",79),_exit(1)));
            return 1;
        }
    } else {
        _serverPanic("t_set.c",83,"Unknown set encoding"),_exit(1);
    }
    return 0;
}
...
int setTypeRemove(robj *setobj, sds value) {
    long long llval;
    if (setobj->encoding == 2) {
        if (dictDelete(setobj->ptr,value) == 0) {
            if (htNeedsResize(setobj->ptr)) dictResize(setobj->ptr);
            return 1;
        }
    } else if (setobj->encoding == 6) {
        if (isSdsRepresentableAsLongLong(value,&llval) == 0) {
            int success;
        ...
        _serverPanic("t_set.c",102,"Unknown set encoding"),_exit(1);
    }
    return 0;
}
...
int setTypeIsMember(robj *subject, sds value) {
    ...
    if (subject->encoding == 2) {
        ...
        _serverPanic("t_set.c",116,"Unknown set encoding"),_exit(1);
    }
    return 0;
}
...
setTypeIterator *setTypeInitIterator(robj *subject) {
    ...
    } else if (si->encoding == 6) {
        si->ii = 0;
    } else {
        _serverPanic("t_set.c",130,"Unknown set encoding"),_exit(1);
    }
    return si;
}
...
int setTypeNext(setTypeIterator *si, sds *sdsele, int64_t *llele) {
    if (si->encoding == 2) {
        ...
        *sdsele = ((void *)0);  
        ...
        _serverPanic("t_set.c",165,"Wrong set encoding in setTypeNext"),_exit(1);
    }
    return si->encoding;
}
...
sds setTypeNextObject(setTypeIterator *si) {
    ...
    switch(encoding) {
        ...
        default:
            _serverPanic("t_set.c",190,"Unsupported encoding"),_exit(1);
    }
    return ((void *)0);  
}
...
int setTypeRandomElement(robj *setobj, sds *sdsele, int64_t *llele) {
    if (setobj->encoding == 2) {
        ...
        *sdsele = ((void *)0);  
        ...
        _serverPanic("t_set.c",217,"Unknown set encoding"),_exit(1);
    }
...
unsigned long setTypeSize(const robj *subject) {
    if (subject->encoding == 2) {
        return (((const dict*)subject->ptr)->ht[0]. used+((const dict*)subject->ptr)->ht[1]. used);
    } else if (subject->encoding == 6) {
        return intsetLen((const intset*)subject->ptr);
    } else {
        _serverPanic("t_set.c",228,"Unknown set encoding"),_exit(1);
    }
}
...
void setTypeConvert(robj *setobj, int enc) {
    setTypeIterator *si;
    ((setobj->type == 2 && setobj->encoding == 6)?(void)0 : (_serverAssertWithInfo(((void *)0),setobj,"setobj->type == OBJ_SET && setobj->encoding == OBJ_ENCODING_INTSET","t_set.c",238),_exit(1)));
    ...
    if (enc == 2) {
        ...
        while (setTypeNext(si,&element,&intele) != -1) {
            ...
            ((dictAdd(d,element,((void *)0)) == 0)?(void)0 : (_serverAssert("dictAdd(d,element,NULL) == DICT_OK","t_set.c",252),_exit(1)));
        }
        ...
        _serverPanic("t_set.c",260,"Unsupported set conversion"),_exit(1);
    }
}
...
void saddCommand(client *c) {
    ...
    if (added) {
        ...
        notifyKeyspaceEvent((1<<5),"sadd",c->argv[1],c->db->id);
    }
    server.dirty += added;
...
void sremCommand(client *c) {
    ...
    if (deleted) {
        signalModifiedKey(c->db,c->argv[1]);
        notifyKeyspaceEvent((1<<5),"srem",c->argv[1],c->db->id);
        if (keyremoved)
...
void smoveCommand(client *c) {
    ...
    if (checkType(c,srcset,2) ||
        (dstset && checkType(c,dstset,2))) return;
    ...
    if (setTypeAdd(dstset,ele->ptr)) {
        server.dirty++;
        notifyKeyspaceEvent((1<<5),"sadd",c->argv[2],c->db->id);
    }
...
void spopWithCountCommand(client *c) {
    ...
    if (getLongFromObjectOrReply(c,c->argv[2],&l,((void *)0)) != 0) return;
    if (l >= 0) {
    ...
    if ((set = lookupKeyWriteOrReply(c,c->argv[1],shared.null[c->resp]))
        == ((void *)0) || checkType(c,set,2)) return;
    ...
    notifyKeyspaceEvent((1<<5),"spop",c->argv[1],c->db->id);
    server.dirty += count;
    ...
    if (count >= size) {
        ...
        sunionDiffGenericCommand(c,c->argv+1,1,((void *)0),0);
...
void srandmemberWithCountCommand(client *c) {
    ...
    if (count*3 > size) {
        ...
        while((encoding = setTypeNext(si,&ele,&llele)) != -1) {
            ...
            ((retval == 0)?(void)0 : (_serverAssert("retval == DICT_OK","t_set.c",695),_exit(1)));
        }
        setTypeReleaseIterator(si);
        ((((d)->ht[0]. used+(d)->ht[1]. used) == size)?(void)0 : (_serverAssert("dictSize(d) == size","t_set.c",698),_exit(1)));
...
void sinterGenericCommand(client *c, robj **setkeys,
    ...
    if (dstkey) {
        ...
        if (setTypeSize(dstset) > 0) {
            ...
            notifyKeyspaceEvent((1<<5),"sinterstore",
                dstkey,c->db->id);
        } else {
            decrRefCount(dstset);
            addReply(c,shared.czero);
...
void sunionDiffGenericCommand(client *c, robj **setkeys, int setnum,
    ...
    if (op == 0) {
    ...
    } else if (op == 1 && sets[0] && diff_algo == 1) {
        ...
        setTypeReleaseIterator(si);
    } else if (op == 1 && sets[0] && diff_algo == 2) {
    ...
    if (!dstkey) {
        ...
        if (setTypeSize(dstset) > 0) {
            dbAdd(c->db,dstkey,dstset);
            addReplyLongLong(c,setTypeSize(dstset));
            notifyKeyspaceEvent((1<<5),
                op == 0 ? "sunionstore" : "sdiffstore",
                dstkey,c->db->id);
        } else {
            decrRefCount(dstset);
            addReply(c,shared.czero);
