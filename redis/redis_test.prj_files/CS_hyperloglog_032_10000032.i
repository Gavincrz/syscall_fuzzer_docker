...
#define HLL_P 14  
#define HLL_Q (64-HLL_P) #define HLL_REGISTERS (1<<HLL_P)  
...
int hllDenseSet(uint8_t *registers, long index, uint8_t count) {
    ...
    do { uint8_t *_p = (uint8_t*) registers; unsigned long _byte = index*6/8; unsigned long _fb = index*6&7; unsigned long _fb8 = 8 - _fb; unsigned long b0 = _p[_byte]; unsigned long b1 = _p[_byte + 1]; oldcount = ((b0 >> _fb) | (b1 << _fb8)) & ((1<<6)-1); } while(0);
    if (count > oldcount) {
        do { uint8_t *_p = (uint8_t*) registers; unsigned long _byte = index*6/8; unsigned long _fb = index*6&7; unsigned long _fb8 = 8 - _fb; unsigned long _v = count; _p[_byte] &= ~(((1<<6)-1) << _fb); _p[_byte] |= _v << _fb; _p[_byte + 1] &= ~(((1<<6)-1) >> _fb8); _p[_byte + 1] |= _v >> _fb8; } while(0);
        return 1;
    } else {
        return 0;
    }
...
void hllDenseRegHisto(uint8_t *registers, int* reghisto) {
    ...
    if ((1<<14) == 16384 && 6 == 6) {
        ...
        for(j = 0; j < (1<<14); j++) {
            unsigned long reg;
            do { uint8_t *_p = (uint8_t*) registers; unsigned long _byte = j*6/8; unsigned long _fb = j*6&7; unsigned long _fb8 = 8 - _fb; unsigned long b0 = _p[_byte]; unsigned long b1 = _p[_byte + 1]; reg = ((b0 >> _fb) | (b1 << _fb8)) & ((1<<6)-1); } while(0);
            reghisto[reg]++;
        }
    }
}
...
int hllSparseToDense(robj *o) {
    ...
    if (hdr->encoding == 0) return 0;

    

 
    dense = sdsnewlen(((void *)0),(sizeof(struct hllhdr)+(((1<<14)*6+7)/8)));
    ...
    *hdr = *oldhdr;  
    ...
    while(p < end) {
        if ((((*(p)) & 0xc0) == 0)) {
            ...
            runlen = (((((*(p)) & 0x3f) << 8) | (*((p)+1)))+1);
            idx += runlen;
            p += 2;
        } else {
            runlen = (((*(p)) & 0x3)+1);
            regval = ((((*(p)) >> 2) & 0x1f)+1);
            if ((runlen + idx) > (1<<14)) break;  
            while(runlen--) {
                do { uint8_t *_p = (uint8_t*) hdr->registers; unsigned long _byte = idx*6/8; unsigned long _fb = idx*6&7; unsigned long _fb8 = 8 - _fb; unsigned long _v = regval; _p[_byte] &= ~(((1<<6)-1) << _fb); _p[_byte] |= _v << _fb; _p[_byte + 1] &= ~(((1<<6)-1) >> _fb8); _p[_byte + 1] |= _v >> _fb8; } while(0);
                idx++;
            }
            p++;
        }
...
int hllSparseSet(robj *o, long index, uint8_t count) {
    ...
    sparse = p = ((uint8_t*)o->ptr) + sizeof(struct hllhdr);
    end = p + sdslen(o->ptr) - sizeof(struct hllhdr);
    ...
    next = (((*(p)) & 0xc0) == 0x40) ? p+2 : p+1;
    if (next >= end) next = ((void *)0);
    ...
    if (is_val) {
        ...
        if (runlen == 1) {
            do { *(p) = (((count)-1)<<2|((1)-1))|0x80; } while(0);
            goto updated;
        }
    }
    ...
    if (is_zero || is_xzero) {
        ...
        if (index != first) {
            len = index-first;
            if (len > 64) {
                do { int _l = (len)-1; *(n) = (_l>>8) | 0x40; *((n)+1) = (_l&0xff); } while(0);
                n += 2;
            } else {
                do { *(n) = (len)-1; } while(0);
                n++;
        ...
        do { *(n) = (((count)-1)<<2|((1)-1))|0x80; } while(0);
        n++;
        if (index != last) {
            len = last-index;
            if (len > 64) {
        ...
        int curval = ((((*(p)) >> 2) & 0x1f)+1);

        if (index != first) {
            len = index-first;
            do { *(n) = (((curval)-1)<<2|((len)-1))|0x80; } while(0);
            n++;
        }
        do { *(n) = (((count)-1)<<2|((1)-1))|0x80; } while(0);
        n++;
...
updated:
    ...
    while (p < end && scanlen--) {
        ...
        if (p+1 < end && ((*(p+1)) & 0x80)) {
            ...
            int v2 = ((((*(p+1)) >> 2) & 0x1f)+1);
            if (v1 == v2) {
                int len = (((*(p)) & 0x3)+1)+(((*(p+1)) & 0x3)+1);
                if (len <= 4) {
                    do { *(p+1) = (((v1)-1)<<2|((len)-1))|0x80; } while(0);
                    memmove(p,p+1,end-p);
                    sdsIncrLen(o->ptr,-1);
                    end--;
...
promote:  
    if (hllSparseToDense(o) == -1) return -1;  
    hdr = o->ptr;
    ...
    ((dense_retval == 1)?(void)0 : (_serverAssert("dense_retval == 1","hyperloglog.c",893),_exit(1)));
    return dense_retval;
}
...
uint64_t hllCount(struct hllhdr *hdr, int *invalid) {
    ...
    if (hdr->encoding == 0) {
        ...
        hllSparseRegHisto(hdr->registers,
                         sdslen((sds)hdr)-sizeof(struct hllhdr),invalid,reghisto);
    } else if (hdr->encoding == 255) {
        hllRawRegHisto(hdr->registers,reghisto);
    } else {
        _serverPanic("hyperloglog.c",1033,"Unknown HyperLogLog encoding in hllCount()"),_exit(1);
    }

    

 
    double z = m * hllTau((m-reghisto[(64-14)+1])/(double)m);
    for (j = (64-14); j >= 1; --j) {
        z += reghisto[j];
        z *= 0.5;
    }
    z += m * hllSigma(reghisto[0]/(double)m);
    E = llroundl(0.721347520444481703680*m*m/z);
...
int hllAdd(robj *o, unsigned char *ele, size_t elesize) {
    ...
    switch(hdr->encoding) {
    case 0: return hllDenseAdd(hdr->registers,ele,elesize);
    case 1: return hllSparseAdd(o,ele,elesize);
    default: return -1;  
    }
}
...
int hllMerge(uint8_t *max, robj *hll) {
    ...
    if (hdr->encoding == 0) {
        ...
        for (i = 0; i < (1<<14); i++) {
            do { uint8_t *_p = (uint8_t*) hdr->registers; unsigned long _byte = i*6/8; unsigned long _fb = i*6&7; unsigned long _fb8 = 8 - _fb; unsigned long b0 = _p[_byte]; unsigned long b1 = _p[_byte + 1]; val = ((b0 >> _fb) | (b1 << _fb8)) & ((1<<6)-1); } while(0);
...
robj *createHLLObject(void) {
    ...
    sds s;
    uint8_t *p;
    int sparselen = sizeof(struct hllhdr) +
                    ((((1<<14)+(16384-1)) /
                     16384)*2);
    int aux;
    ...
    p = (uint8_t*)s + sizeof(struct hllhdr);
    while(aux) {
        int xzero = 16384;
        if (xzero > aux) xzero = aux;
        do { int _l = (xzero)-1; *(p) = (_l>>8) | 0x40; *((p)+1) = (_l&0xff); } while(0);
        p += 2;
        aux -= xzero;
    }
    (((p-(uint8_t*)s) == sparselen)?(void)0 : (_serverAssert("(p-(uint8_t*)s) == sparselen","hyperloglog.c",1136),_exit(1)));
...
int isHLLObjectOrReply(client *c, robj *o) {
    ...
    if (!(o->encoding == 0 || o->encoding == 8)) goto invalid;
    ...
    if (hdr->encoding > 1) goto invalid;

     
    if (hdr->encoding == 0 &&
        stringObjectLen(o) != (sizeof(struct hllhdr)+(((1<<14)*6+7)/8))) goto invalid;
...
void pfaddCommand(client *c) {
    ...
    if (updated) {
        ...
        notifyKeyspaceEvent((1<<3),"pfadd",c->argv[1],c->db->id);
        server.dirty++;
        (hdr)->card[7] |= (1<<7);
    }
...
void pfcountCommand(client *c) {
    ...
    if (c->argc > 2) {
        uint8_t max[sizeof(struct hllhdr)+(1<<14)], *registers;
        int j;
        ...
        hdr->encoding = 255;  
        registers = max + sizeof(struct hllhdr);
        for (j = 1; j < c->argc; j++) {
...
void pfmergeCommand(client *c) {
    ...
    if (use_dense && hllSparseToDense(o) == -1) {
        addReplySds(c,sdsnew(invalid_hll_err));
        return;
    }
    ...
    for (j = 0; j < (1<<14); j++) {
        ...
        hdr = o->ptr;
        switch(hdr->encoding) {
        case 0: hllDenseSet(hdr->registers,j,max[j]); break;
        case 1: hllSparseSet(o,j,max[j]); break;
        }
    }
    hdr = o->ptr; 
...
void pfselftestCommand(client *c) {
    unsigned int j, i;
    sds bitcounters = sdsnewlen(((void *)0),(sizeof(struct hllhdr)+(((1<<14)*6+7)/8)));
    struct hllhdr *hdr = (struct hllhdr*) bitcounters, *hdr2;
    robj *o = ((void *)0);
    uint8_t bytecounters[(1<<14)];
    ...
    for (j = 0; j < 1000; j++) {
        ...
        for (i = 0; i < (1<<14); i++) {
            ...
            bytecounters[i] = r;
            do { uint8_t *_p = (uint8_t*) hdr->registers; unsigned long _byte = i*6/8; unsigned long _fb = i*6&7; unsigned long _fb8 = 8 - _fb; unsigned long _v = r; _p[_byte] &= ~(((1<<6)-1) << _fb); _p[_byte] |= _v << _fb; _p[_byte + 1] &= ~(((1<<6)-1) >> _fb8); _p[_byte + 1] |= _v >> _fb8; } while(0);
        }
    ...
    memset(hdr->registers,0,(sizeof(struct hllhdr)+(((1<<14)*6+7)/8))-sizeof(struct hllhdr));
    o = createHLLObject();
...
void pfdebugCommand(client *c) {
    ...
    if (!strcasecmp(cmd,"getreg")) {
        ...
        for (j = 0; j < (1<<14); j++) {
            ...
            do { uint8_t *_p = (uint8_t*) hdr->registers; unsigned long _byte = j*6/8; unsigned long _fb = j*6&7; unsigned long _fb8 = 8 - _fb; unsigned long b0 = _p[_byte]; unsigned long b1 = _p[_byte + 1]; val = ((b0 >> _fb) | (b1 << _fb8)) & ((1<<6)-1); } while(0);
