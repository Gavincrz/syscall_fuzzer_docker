...
size_t getStringObjectSdsUsedMemory(robj *o) {
    ((o->type == 0)?(void)0 : (_serverAssertWithInfo(((void *)0),o,"o->type == OBJ_STRING","networking.c",51),_exit(1)));
    switch(o->encoding) {
    case 0: return sdsZmallocSize(o->ptr);
    case 8: return je_malloc_usable_size(o)-sizeof(robj);
    default: return 0;  
    }
}
...
client *createClient(int fd) {
    ...
    if (fd != -1) {
        ...
        if (aeCreateFileEvent(server.el,fd,1,
    ...
    c->authenticated = (c->user->flags & (1<<4)) != 0;
    c->replstate = 0;
    ...
    ((c->reply)->free = (freeClientReplyValue));
    ...
    ((c->pubsub_patterns)->free = (decrRefCountVoid));
...
void clientInstallWriteHandler(client *c) {
    ...
    if (!(c->flags & (1<<21)) &&
        (c->replstate == 0 ||
         (c->replstate == 9 && !c->repl_put_online_on_ack)))
    {
...
int prepareClientToWrite(client *c) {
    ...
    if (c->flags & ((1<<8)|(1<<27))) return 0;

     
    if (c->flags & ((1<<22)|(1<<24))) return -1;

    
 
    if ((c->flags & (1<<1)) &&
        !(c->flags & (1<<13))) return -1;
...
int _addReplyToBuffer(client *c, const char *s, size_t len) {
    ...
    if (c->flags & (1<<6)) return 0;

    
 
    if (((c->reply)->len) > 0) return -1;
...
void _addReplyProtoToList(client *c, const char *s, size_t len) {
    if (c->flags & (1<<6)) return;

    listNode *ln = ((c->reply)->tail);
    clientReplyBlock *tail = ln? ((ln)->value): ((void *)0);
    ...
    if (len) {
        ...
        size_t size = len < (16*1024)? (16*1024): len;
        tail = zmalloc(size + sizeof(clientReplyBlock));
         
        tail->size = je_malloc_usable_size(tail) - sizeof(clientReplyBlock);
        tail->used = len;
...
void addReply(client *c, robj *obj) {
    ...
    if ((obj->encoding == 0 || obj->encoding == 8)) {
        if (_addReplyToBuffer(c,obj->ptr,sdslen(obj->ptr)) != 0)
            _addReplyProtoToList(c,obj->ptr,sdslen(obj->ptr));
    } else if (obj->encoding == 1) {
        

 
        char buf[32];
        size_t len = ll2string(buf,sizeof(buf),(long)obj->ptr);
        if (_addReplyToBuffer(c,buf,len) != 0)
            _addReplyProtoToList(c,buf,len);
    } else {
        _serverPanic("networking.c",317,"Wrong obj->encoding in addReply()"),_exit(1);
    }
}
...
void addReplySds(client *c, sds s) {
    ...
    if (_addReplyToBuffer(c,s,sdslen(s)) != 0)
...
void addReplyErrorLength(client *c, const char *s, size_t len) {
    ...
    if (c->flags & ((1<<1)|(1<<0)) && !(c->flags & (1<<2))) {
        char* to = c->flags & (1<<1)? "master": "replica";
        char* from = c->flags & (1<<1)? "replica": "master";
        char *cmdname = c->lastcmd ? c->lastcmd->name : "<unknown>";
        serverLog(3,"== CRITICAL == This %s is sending an error "
...
void *addReplyDeferredLen(client *c) {
    ...
    if (prepareClientToWrite(c) != 0) return ((void *)0);
...
void setDeferredAggregateLen(client *c, void *node, long length, char prefix) {
    ...
    ((!((ln)->value))?(void)0 : (_serverAssert("!listNodeValue(ln)","networking.c",443),_exit(1)));
    ...
    if (ln->next != ((void *)0) && (next = ((ln->next)->value)) &&
        ...
        buf->size = je_malloc_usable_size(buf) - sizeof(clientReplyBlock);
        buf->used = lenstr_len;
...
void addReplyLongLongWithPrefix(client *c, long long ll, char prefix) {
    ...
    if (prefix == '*' && ll < 32 && ll >= 0) {
        addReply(c,shared.mbulkhdr[ll]);
        return;
    } else if (prefix == '$' && ll < 32 && ll >= 0) {
...
void acceptTcpHandler(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    while(max--) {
        ...
        serverLog(1,"Accepted %s:%d", cip, cport);
...
void acceptUnixHandler(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    while(max--) {
        ...
        serverLog(1,"Accepted connection to %s", server.unixsocket);
        acceptCommonHandler(cfd,(1<<11),((void *)0));
    }
}
...
void unlinkClient(client *c) {
    ...
    if (c->flags & (1<<21)) {
        ...
        ((ln != ((void *)0))?(void)0 : (_serverAssert("ln != NULL","networking.c",948),_exit(1)));
    ...
    if (c->flags & (1<<29)) {
        ...
        ((ln != ((void *)0))?(void)0 : (_serverAssert("ln != NULL","networking.c",956),_exit(1)));
    ...
    if (c->flags & (1<<7)) {
        ...
        ((ln != ((void *)0))?(void)0 : (_serverAssert("ln != NULL","networking.c",965),_exit(1)));
...
void freeClient(client *c) {
    ...
    if (server.master && c->flags & (1<<1)) {
        serverLog(3,"Connection with master lost.");
        if (!(c->flags & ((1<<6)|
                          (1<<10)|
                          (1<<4))))
        {
    ...
    if ((c->flags & (1<<0)) && !(c->flags & (1<<2))) {
        serverLog(3,"Connection with replica %s lost.",
    ...
    if (c->flags & (1<<4)) unblockClient(c);
    ...
    if (c->flags & (1<<0)) {
        ...
        list *l = (c->flags & (1<<2)) ? server.monitors : server.slaves;
        ln = listSearchKey(l,c);
        ((ln != ((void *)0))?(void)0 : (_serverAssert("ln != NULL","networking.c",1040),_exit(1)));
        listDelNode(l,ln);
        

 
        if (c->flags & (1<<0) && ((server . slaves)->len) == 0)
    ...
    if (c->flags & (1<<1)) replicationHandleMasterDisconnection();

    
 
    if (c->flags & (1<<10)) {
        ln = listSearchKey(server.clients_to_close,c);
        ((ln != ((void *)0))?(void)0 : (_serverAssert("ln != NULL","networking.c",1058),_exit(1)));
...
void freeClientAsync(client *c) {
    ...
    static pthread_mutex_t async_free_queue_mutex = { { 0, 0, 0, 0, 0, 0, 0, { 0, 0 } } };
    if (c->flags & (1<<10) || c->flags & (1<<8)) return;
    c->flags |= (1<<10);
...
void freeClientsInAsyncFreeQueue(void) {
    while (((server . clients_to_close)->len)) {
        listNode *ln = ((server . clients_to_close)->head);
        client *c = ((ln)->value);
...
int writeToClient(int fd, client *c, int handler_installed) {
    ...
    while(clientHasPendingReplies(c)) {
        if (c->bufpos > 0) {
            ...
            if (c->sentlen == objlen) {
                ...
                if (((c->reply)->len) == 0)
                    ((c->reply_bytes == 0)?(void)0 : (_serverAssert("c->reply_bytes == 0","networking.c",1157),_exit(1)));
            }
        }
    ...
    if (totwritten > 0) {
        ...
        if (!(c->flags & (1<<1))) c->lastinteraction = server.unixtime;
    }
    if (!clientHasPendingReplies(c)) {
        ...
        if (handler_installed) aeDeleteFileEvent(server.el,c->fd,2);
...
int handleClientsWithPendingWrites(void) {
    ...
    while((ln = listNext(&li))) {
        ...
        if (c->flags & (1<<28)) continue;

         
        if (writeToClient(c->fd,c,0) == -1) continue;
...
void resetClient(client *c) {
    ...
    if (!(c->flags & (1<<3)) && prevcmd != askingCommand)
        c->flags &= ~(1<<9);
...
void unprotectClient(client *c) {
    if (c->flags & (1<<28)) {
        c->flags &= ~(1<<28);
        aeCreateFileEvent(server.el,c->fd,1,readQueryFromClient,c);
        if (clientHasPendingReplies(c)) clientInstallWriteHandler(c);
    }
}
...
int processInlineBuffer(client *c) {
    ...
    if (newline == ((void *)0)) {
        if (sdslen(c->querybuf)-c->qb_pos > (1024*64)) {
...
static void setProtocolError(const char *errstr, client *c) {
    if (server.verbosity <= 1) {
        ...
        char buf[256];
        if (sdslen(c->querybuf)-c->qb_pos < 128) {
            snprintf(buf,sizeof(buf),"Query buffer during protocol error: '%s'", c->querybuf+c->qb_pos);
        } else {
            snprintf(buf,sizeof(buf),"Query buffer during protocol error: '%.*s' (... more %zu bytes ...) '%.*s'", 128/2, c->querybuf+c->qb_pos, sdslen(c->querybuf)-c->qb_pos-128, 128/2, c->querybuf+sdslen(c->querybuf)-128/2);
        }
...
int processMultibulkBuffer(client *c) {
    ...
    if (c->multibulklen == 0) {
        ...
        ((c->argc == 0)?(void)0 : (_serverAssertWithInfo(c,((void *)0),"c->argc == 0","networking.c",1433),_exit(1)));
        ...
        ((c->querybuf[c->qb_pos] == '*')?(void)0 : (_serverAssertWithInfo(c,((void *)0),"c->querybuf[c->qb_pos] == '*'","networking.c",1451),_exit(1)));
    ...
    ((c->multibulklen > 0)?(void)0 : (_serverAssertWithInfo(c,((void *)0),"c->multibulklen > 0","networking.c",1470),_exit(1)));
    while(c->multibulklen) {
        ...
        if (sdslen(c->querybuf)-c->qb_pos < (size_t)(c->bulklen+2)) {
            ...
            {
                c->argv[c->argc++] = createObject(0,c->querybuf);
...
int processCommandAndResetClient(client *c) {
    ...
    if (processCommand(c) == 0) {
        if (c->flags & (1<<1) && !(c->flags & (1<<3))) {
...
void processInputBuffer(client *c) {
    ...
    while(c->qb_pos < sdslen(c->querybuf)) {
        ...
        if (!(c->flags & (1<<0)) && clientsArePaused()) break;

         
        if (c->flags & (1<<4)) break;

        
 
        if (c->flags & (1<<30)) break;

        


 
        if (server.lua_timedout && c->flags & (1<<1)) break;
        ...
        if (c->reqtype == 1) {
            if (processInlineBuffer(c) != 0) break;
            ...
            if (processMultibulkBuffer(c) != 0) break;
        } else {
            _serverPanic("networking.c",1652,"Unknown request type"),_exit(1);
        }
...
void readQueryFromClient(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    if (c->reqtype == 2 && c->multibulklen && c->bulklen != -1
        && c->bulklen >= (1024*32))
    {
    ...
    if (nread == -1) {
        if (errno == 11) {
            ...
            serverLog(1, "Reading from client: %s",strerror(errno));
            freeClientAsync(c);
            return;
        }
    } else if (nread == 0) {
        serverLog(1, "Client closed connection");
        freeClientAsync(c);
        return;
    } else if (c->flags & (1<<1)) {
    ...
    if (c->flags & (1<<1)) c->read_reploff += nread;
    server.stat_net_input_bytes += nread;
    if (sdslen(c->querybuf) > server.client_max_querybuf_len) {
        sds ci = catClientInfoString(sdsempty(),c), bytes = sdsempty();

        bytes = sdscatrepr(bytes,c->querybuf,64);
        serverLog(3,"Closing client that reached max query buffer length: %s (qbuf initial bytes: %s)", ci, bytes);
        sdsfree(ci);
        sdsfree(bytes);
        freeClientAsync(c);
        return;
...
void getClientsMaxBuffers(unsigned long *longest_output_list,
    ...
    while ((ln = listNext(&li)) != ((void *)0)) {
        ...
        if (((c->reply)->len) > lol) lol = ((c->reply)->len);
...
sds catClientInfoString(sds s, client *client) {
    ...
    if (client->flags & (1<<1)) *p++ = 'M';
    if (client->flags & (1<<18)) *p++ = 'P';
    if (client->flags & (1<<3)) *p++ = 'x';
    if (client->flags & (1<<4)) *p++ = 'b';
    if (client->flags & (1<<5)) *p++ = 'd';
    if (client->flags & (1<<6)) *p++ = 'c';
    if (client->flags & (1<<7)) *p++ = 'u';
    if (client->flags & (1<<10)) *p++ = 'A';
    if (client->flags & (1<<11)) *p++ = 'U';
    if (client->flags & (1<<17)) *p++ = 'r';
    if (p == flags) *p++ = 'N';
    *p++ = '\0';
    ...
    *p = '\0';
    return sdscatfmt(s,
        ...
        flags,
        client->db->id,
        (int) ((client->pubsub_channels)->ht[0]. used+(client->pubsub_channels)->ht[1]. used),
        (int) ((client->pubsub_patterns)->len),
        (client->flags & (1<<3)) ? client->mstate.count : -1,
        (unsigned long long) sdslen(client->querybuf),
        (unsigned long long) sdsavail(client->querybuf),
        (unsigned long long) client->bufpos,
        (unsigned long long) ((client->reply)->len),
...
sds getAllClientsInfoString(int type) {
    ...
    listIter li;
    client *client;
    sds o = sdsnewlen(SDS_NOINIT,200*((server . clients)->len));
    sdsclear(o);
    listRewind(server.clients,&li);
    while ((ln = listNext(&li)) != ((void *)0)) {
        client = ((ln)->value);
...
((void *)0)
        ...
        if (c->argc == 3) {
            ...
            while(i < c->argc) {
                ...
                if (!strcasecmp(c->argv[i]->ptr,"id") && moreargs) {
                    ...
                    if (getLongLongFromObjectOrReply(c,c->argv[i+1],&tmp,((void *)0))
                        != 0) return;
                    id = tmp;
        ...
        if (close_this_client) c->flags |= (1<<6);
                                                          ...
                                                          c->argc == 4))
    {
        ...
        if (getLongLongFromObjectOrReply(c,c->argv[2],&id,((void *)0))
            != 0) return;
        struct client *target = lookupClientByID(id);
        if (target && target->flags & (1<<4)) {
            if (unblock_error)
        ...
        if (clientSetNameOrReply(c,c->argv[2]) == 0)
            addReply(c,shared.ok);
        ...
        if (getTimeoutFromObjectOrReply(c,c->argv[2],&duration,1)
                                        != 0) return;
...
void helloCommand(client *c) {
    ...
    if (getLongLongFromObject(c->argv[1],&ver) != 0 ||
        ver < 2 || ver > 3)
    {
    ...
    for (int j = 2; j < c->argc; j++) {
        ...
        if (!strcasecmp(opt,"AUTH") && moreargs >= 2) {
            if (ACLAuthenticateUser(c, c->argv[j+1], c->argv[j+2]) == -1) {
                addReplyError(c,"-WRONGPASS invalid username-password pair");
                return;
            }
            j += 2;
        } else if (!strcasecmp(opt,"SETNAME") && moreargs) {
            if (clientSetNameOrReply(c, c->argv[j+1]) == -1) return;
            j++;
        } else {
    ...
    addReplyBulkCString(c,"999.999.999");
...
void securityWarningCommand(client *c) {
    ...
    if (labs(now-logged_time) > 60) {
        serverLog(3,"Possible SECURITY ATTACK detected. It looks like somebody is sending POST or Host: commands to Redis. This is likely due to an attacker attempting to use Cross Protocol Scripting to compromise your Redis instance. Connection aborted.");
        logged_time = now;
    }
    freeClientAsync(c);
}
...
void rewriteClientCommandVector(client *c, int argc, ...) {
    ...
    for (j = 0; j < argc; j++) {
        ...
        a = (*(robj* *)(((*(char* *)&(ap)) += sizeof(robj*)) - sizeof(robj*)));
        argv[j] = a;
        incrRefCount(a);
    }
    ...
    ((c->cmd != ((void *)0))?(void)0 : (_serverAssertWithInfo(c,((void *)0),"c->cmd != NULL","networking.c",2253),_exit(1)));
    ((void)0);
}
...
void replaceClientCommandVector(client *c, int argc, robj **argv) {
    ...
    c->argc = argc;
    c->cmd = lookupCommandOrOriginal(c->argv[0]->ptr);
    ((c->cmd != ((void *)0))?(void)0 : (_serverAssertWithInfo(c,((void *)0),"c->cmd != NULL","networking.c",2264),_exit(1)));
}
...
void rewriteClientCommandArgument(client *c, int i, robj *newval) {
    ...
    if (i == 0) {
        c->cmd = lookupCommandOrOriginal(c->argv[0]->ptr);
        ((c->cmd != ((void *)0))?(void)0 : (_serverAssertWithInfo(c,((void *)0),"c->cmd != NULL","networking.c",2294),_exit(1)));
    }
}
...
unsigned long getClientOutputBufferMemoryUsage(client *c) {
    ...
    return c->reply_bytes + (list_item_size*((c->reply)->len));
}
...
int getClientType(client *c) {
    if (c->flags & (1<<1)) return 3;
    if ((c->flags & (1<<0)) && !(c->flags & (1<<2)))
        return 1;
    if (c->flags & (1<<18)) return 2;
    return 0;
}

int getClientTypeByName(char *name) {
    if (!strcasecmp(name,"normal")) return 0;
    else if (!strcasecmp(name,"slave")) return 1;
    else if (!strcasecmp(name,"replica")) return 1;
    else if (!strcasecmp(name,"pubsub")) return 2;
    else if (!strcasecmp(name,"master")) return 3;
    else return -1;
}
...
void asyncCloseClientOnOutputBufferLimitReached(client *c) {
    if (c->fd == -1) return;  
    ((c->reply_bytes < (18446744073709551615UL)-(1024*64))?(void)0 : (_serverAssert("c->reply_bytes < SIZE_MAX-(1024*64)","networking.c",2398),_exit(1)));
    if (c->reply_bytes == 0 || c->flags & (1<<10)) return;
    if (checkClientOutputBufferLimits(c)) {
        sds client = catClientInfoString(sdsempty(),c);

        freeClientAsync(c);
        serverLog(3,"Client %s scheduled to be closed ASAP for overcoming of output buffer limits.", client);
        sdsfree(client);
    }
}
...
int processEventsWhileBlocked(void) {
    ...
    while (iterations--) {
        int events = 0;
        events += aeProcessEvents(server.el, 1|4);
...
pthread_t io_threads[128];
pthread_mutex_t io_threads_mutex[128];
_Atomic unsigned long io_threads_pending[128];
int io_threads_active;   
int io_threads_op;       
list *io_threads_list[128];
...
void *IOThreadMain(void *myid) {
    ...
    while(1) {
        ...
        ((io_threads_pending[id] != 0)?(void)0 : (_serverAssert("io_threads_pending[id] != 0","networking.c",2548),_exit(1)));

        if (tio_debug) printf("[%ld] %d to handle\n", id, (int)((io_threads_list[id])->len));
        ...
        while((ln = listNext(&li))) {
            ...
            } else if (io_threads_op == 0) {
                readQueryFromClient(((void *)0),c->fd,c,0);
            } else {
                _serverPanic("networking.c",2564,"io_threads_op value is unknown"),_exit(1);
            }
        }
...
void initThreadedIO(void) {
    ...
    if (server.io_threads_num > 128) {
        serverLog(3,"Fatal: too many I/O threads configured. "
                             "The maximum number is %d.", 128);
        exit(1);
    }
    ...
    for (int i = 0; i < server.io_threads_num; i++) {
        ...
        if (pthread_create(&tid,((void *)0),IOThreadMain,(void*)(long)i) != 0) {
            serverLog(3,"Fatal: Can't initialize IO thread.");
            exit(1);
        }
        io_threads[i] = tid;
    }
...
void startThreadedIO(void) {
    ...
    ((io_threads_active == 0)?(void)0 : (_serverAssert("io_threads_active == 0","networking.c",2606),_exit(1)));
...
void stopThreadedIO(void) {
    ...
    if (tio_debug) printf("--- STOPPING THREADED IO [R%d] [W%d] ---\n",
        (int) ((server . clients_pending_read)->len),
        (int) ((server . clients_pending_write)->len));
    ((io_threads_active == 1)?(void)0 : (_serverAssert("io_threads_active == 1","networking.c",2620),_exit(1)));
    for (int j = 0; j < server.io_threads_num; j++)
...
int stopThreadedIOIfNeeded(void) {
    int pending = ((server . clients_pending_write)->len);
...
int handleClientsWithPendingWritesUsingThreads(void) {
    ...
    io_threads_op = 1;
    for (int j = 0; j < server.io_threads_num; j++) {
        int count = ((io_threads_list[j])->len);
...
int handleClientsWithPendingReadsUsingThreads(void) {
    ...
    int processed = ((server . clients_pending_read)->len);
    if (processed == 0) return 0;
