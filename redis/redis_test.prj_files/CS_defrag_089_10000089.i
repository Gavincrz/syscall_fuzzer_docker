...
robj *activeDefragStringOb(robj* ob, long *defragged) {
    ...
    if (ob->type == 0) {
        ...
        } else if (ob->encoding!=1) {
            _serverPanic("defrag.c",133,"Unknown string encoding"),_exit(1);
        }
    }
    return ret;
}
...
void zslUpdateNode(zskiplist *zsl, zskiplistNode *oldnode, zskiplistNode *newnode, zskiplistNode **update) {
    ...
    ((zsl->header!=oldnode)?(void)0 : (_serverAssert("zsl->header!=oldnode","defrag.c",196),_exit(1)));
    if (newnode->level[0].forward) {
        ((newnode ->level[0]. forward->backward==oldnode)?(void)0 : (_serverAssert("newnode->level[0].forward->backward==oldnode","defrag.c",198),_exit(1)));
        newnode->level[0].forward->backward = newnode;
    } else {
        ((zsl->tail==oldnode)?(void)0 : (_serverAssert("zsl->tail==oldnode","defrag.c",201),_exit(1)));
        zsl->tail = newnode;
    }
}
...
double *zslDefrag(zskiplist *zsl, double score, sds oldele, sds newele) {
    zskiplistNode *update[64], *x, *newx;
    int i;
    ...
    x = x->level[0].forward;
    ((x && score == x->score && x->ele==oldele)?(void)0 : (_serverAssert("x && score == x->score && x->ele==oldele","defrag.c",235),_exit(1)));
    if (newele)
        x->ele = newele;
...
long activeDefragZsetEntry(zset *zs, dictEntry *de) {
    ...
    newscore = zslDefrag(zs->zsl, *(double*)((de)->v . val), sdsele, newsds);
    if (newscore) {
        do { if ((zs->dict)->type ->valDup) (de)->v . val = (zs->dict)->type ->valDup((zs->dict)->privdata, newscore); else (de)->v . val = (newscore); } while(0);
        defragged++;
    }
    return defragged;
}
...
long activeDefragSdsDict(dict* d, int val_type) {
    ...
    while((de = dictNext(di)) != ((void *)0)) {
        ...
        if (val_type == 1) {
            ...
            robj *newele, *ele = ((de)->v . val);
            if ((newele = activeDefragStringOb(ele, &defragged)))
                de->v.val = newele;
        } else if (val_type == 3) {
            void *newptr, *ptr = ((de)->v . val);
...
long activeDefragSdsListAndDict(list *l, dict *d, int dict_val_type) {
    ...
    while((de = dictNext(di)) != ((void *)0)) {
        if (dict_val_type == 1) {
            sds newsds, sdsele = ((de)->v . val);
            if ((newsds = activeDefragSds(sdsele)))
                de->v.val = newsds, defragged++;
        } else if (dict_val_type == 2) {
            robj *newele, *ele = ((de)->v . val);
            if ((newele = activeDefragStringOb(ele, &defragged)))
                de->v.val = newele, defragged++;
        } else if (dict_val_type == 3) {
            void *newptr, *ptr = ln->value;
            if ((newptr = activeDefragAlloc(ptr)))
                ln->value = newptr, defragged++;
        }
...
long defragQuicklist(redisDb *db, dictEntry *kde) {
    ...
    ((ob->type == 1 && ob->encoding == 9)?(void)0 : (_serverAssert("ob->type == OBJ_LIST && ob->encoding == OBJ_ENCODING_QUICKLIST","defrag.c",516),_exit(1)));
...
long defragZsetSkiplist(redisDb *db, dictEntry *kde) {
    ...
    ((ob->type == 3 && ob->encoding == 7)?(void)0 : (_serverAssert("ob->type == OBJ_ZSET && ob->encoding == OBJ_ENCODING_SKIPLIST","defrag.c",535),_exit(1)));
    ...
    if (((zs->dict)->ht[0]. used+(zs->dict)->ht[1]. used) > server.active_defrag_max_scan_fields)
        defragLater(db, kde);
    else {
...
long defragHash(redisDb *db, dictEntry *kde) {
    ...
    robj *ob = ((kde)->v . val);
    dict *d, *newd;
    ((ob->type == 4 && ob->encoding == 2)?(void)0 : (_serverAssert("ob->type == OBJ_HASH && ob->encoding == OBJ_ENCODING_HT","defrag.c",563),_exit(1)));
    d = ob->ptr;
    if (((d)->ht[0]. used+(d)->ht[1]. used) > server.active_defrag_max_scan_fields)
        defragLater(db, kde);
    else
        defragged += activeDefragSdsDict(d, 1);
...
long defragSet(redisDb *db, dictEntry *kde) {
    ...
    robj *ob = ((kde)->v . val);
    dict *d, *newd;
    ((ob->type == 2 && ob->encoding == 2)?(void)0 : (_serverAssert("ob->type == OBJ_SET && ob->encoding == OBJ_ENCODING_HT","defrag.c",581),_exit(1)));
    d = ob->ptr;
    if (((d)->ht[0]. used+(d)->ht[1]. used) > server.active_defrag_max_scan_fields)
        defragLater(db, kde);
    else
        defragged += activeDefragSdsDict(d, 0);
...
int scanLaterStraemListpacks(robj *ob, unsigned long *cursor, long long endtime, long long *defragged) {
    ...
    while (raxNext(&ri)) {
        ...
        if (++iterations > 16) {
            if (ustime() > endtime) {
                ((ri . key_len==sizeof(last))?(void)0 : (_serverAssert("ri.key_len==sizeof(last)","defrag.c",643),_exit(1)));
...
void* defragStreamConsumerPendingEntry(raxIterator *ri, void *privdata, long *defragged) {
    ...
    if (newnack) {
        ...
        ((prev==nack)?(void)0 : (_serverAssert("prev==nack","defrag.c",703),_exit(1)));
...
void* defragStreamConsumerGroup(raxIterator *ri, void *privdata, long *defragged) {
    ...
    if (cg->pel)
        *defragged += defragRadixTree(&cg->pel, 0, ((void *)0), ((void *)0));
    return ((void *)0);
}
...
long defragStream(redisDb *db, dictEntry *kde) {
    long defragged = 0;
    robj *ob = ((kde)->v . val);
    ((ob->type == 6 && ob->encoding == 10)?(void)0 : (_serverAssert("ob->type == OBJ_STREAM && ob->encoding == OBJ_ENCODING_STREAM","defrag.c",740),_exit(1)));
    ...
    if (raxSize(s->rax) > server.active_defrag_max_scan_fields) {
        ...
        defragged += defragRadixTree(&s->rax, 1, ((void *)0), ((void *)0));

    if (s->cgroups)
        defragged += defragRadixTree(&s->cgroups, 1, defragStreamConsumerGroup, ((void *)0));
    return defragged;
}
...
long defragKey(redisDb *db, dictEntry *de) {
    ...
    if (newsds)
        defragged++, de->key = newsds;
    if (((db->expires)->ht[0]. used+(db->expires)->ht[1]. used)) {
    ...
    if (ob->type == 0) {
        ...
        } else if (ob->encoding == 5) {
            if ((newzl = activeDefragAlloc(ob->ptr)))
                defragged++, ob->ptr = newzl;
        } else {
            _serverPanic("defrag.c",798,"Unknown list encoding"),_exit(1);
        }
    } else if (ob->type == 2) {
        if (ob->encoding == 2) {
            defragged += defragSet(db, de);
        } else if (ob->encoding == 6) {
            intset *newis, *is = ob->ptr;
            if ((newis = activeDefragAlloc(is)))
                defragged++, ob->ptr = newis;
        } else {
            _serverPanic("defrag.c",808,"Unknown set encoding"),_exit(1);
        }
    } else if (ob->type == 3) {
        if (ob->encoding == 5) {
            if ((newzl = activeDefragAlloc(ob->ptr)))
                defragged++, ob->ptr = newzl;
        } else if (ob->encoding == 7) {
            defragged += defragZsetSkiplist(db, de);
        } else {
            _serverPanic("defrag.c",817,"Unknown sorted set encoding"),_exit(1);
        }
    } else if (ob->type == 4) {
        if (ob->encoding == 5) {
            if ((newzl = activeDefragAlloc(ob->ptr)))
                defragged++, ob->ptr = newzl;
        } else if (ob->encoding == 2) {
            defragged += defragHash(db, de);
        } else {
            _serverPanic("defrag.c",826,"Unknown hash encoding"),_exit(1);
        }
    } else if (ob->type == 6) {
        ...
        _serverPanic("defrag.c",834,"Unknown object type"),_exit(1);
    }
    return defragged;
}
...
long defragOtherGlobals() {
    ...
    defragged += activeDefragSdsDict(server.lua_scripts, 2);
    defragged += activeDefragSdsListAndDict(server.repl_scriptcache_fifo, server.repl_scriptcache_dict, 0);
    return defragged;
}
...
int defragLaterStep(redisDb *db, long long endtime) {
    ...
    do {
        ...
        if (!cursor) {
            listNode *head = ((db->defrag_later)->head);

             
            if (current_key) {
                ((current_key == head->value)?(void)0 : (_serverAssert("current_key == head->value","defrag.c",938),_exit(1)));
                sdsfree(head->value);
...
void computeDefragCycles() {
    ...
    int cpu_pct = ( (server . active_defrag_cycle_min) + ((frag_pct)-(server . active_defrag_threshold_lower)) * ((server . active_defrag_cycle_max)-(server . active_defrag_cycle_min)) / ((server . active_defrag_threshold_upper)-(server . active_defrag_threshold_lower)) );




    cpu_pct = ((cpu_pct)<(server . active_defrag_cycle_min)? server . active_defrag_cycle_min: ((cpu_pct)>(server . active_defrag_cycle_max)? server . active_defrag_cycle_max: (cpu_pct)));
...
void activeDefragCycle(void) {
    ...
    if (server . latency_monitor_threshold && (latency) >= server . latency_monitor_threshold) latencyAddSample(("active-defrag-cycle"),(latency));;
}
