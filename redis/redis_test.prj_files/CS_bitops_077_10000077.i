...
long redisBitpos(void *s, unsigned long count, int bit) {
    ...
    l = (unsigned long*) c;
    if (!found) {
        skipval = bit ? 0 : (0x7fffffffffffffffL * 2UL + 1UL);
    ...
    one = (0x7fffffffffffffffL * 2UL + 1UL);  
    one >>= 1;        
    one = ~one;       
    ...
    _serverPanic("bitops.c",184,"End of redisBitpos() reached."),_exit(1);
    return 0;  
}
...
int checkUnsignedBitfieldOverflow(uint64_t value, int64_t incr, uint64_t bits, int owtype, uint64_t *limit) {
    uint64_t max = (bits == 64) ? (18446744073709551615UL) : (((uint64_t)1<<bits)-1);
...
int checkSignedBitfieldOverflow(int64_t value, int64_t incr, uint64_t bits, int owtype, int64_t *limit) {
    int64_t max = (bits == 64) ? (9223372036854775807L) : (((int64_t)1<<(bits-1))-1);
    int64_t min = (-max)-1;
...
robj *lookupStringForBitCommand(client *c, size_t maxbit) {
    ...
    if (o == ((void *)0)) {
        o = createObject(0,sdsnewlen(((void *)0), byte+1));
        dbAdd(c->db,c->argv[1],o);
    } else {
        if (checkType(c,o,0)) return ((void *)0);
...
unsigned char *getObjectReadOnlyString(robj *o, long *len, char *llbuf) {
    ((o->type == 0)?(void)0 : (_serverAssert("o->type == OBJ_STRING","bitops.c",507),_exit(1)));
    ...
    if (o && o->encoding == 1) {
        ...
        if (len) *len = ll2string(llbuf,21,(long)o->ptr);
    } else if (o) {
...
void setbitCommand(client *c) {
    ...
    if (getBitOffsetFromArgument(c,c->argv[2],&bitoffset,0,0) != 0)
        return;

    if (getLongFromObjectOrReply(c,c->argv[3],&on,err) != 0)
        return;
    ...
    notifyKeyspaceEvent((1<<3),"setbit",c->argv[1],c->db->id);
    server.dirty++;
...
void bitopCommand(client *c) {
    ...
    if (maxlen) {
        res = (unsigned char*) sdsnewlen(((void *)0),maxlen);
        ...
        for (; j < maxlen; j++) {
            ...
            if (op == 3) output = ~output;
            for (i = 1; i < numkeys; i++) {
                byte = (len[i] <= j) ? 0 : src[i][j];
                switch(op) {
                case 0: output &= byte; break;
    ...
    if (maxlen) {
        o = createObject(0,res);
        setKey(c->db,targetkey,o);
        notifyKeyspaceEvent((1<<3),"set",targetkey,c->db->id);
        decrRefCount(o);
    } else if (dbDelete(c->db,targetkey)) {
        signalModifiedKey(c->db,targetkey);
        notifyKeyspaceEvent((1<<2),"del",targetkey,c->db->id);
    }
    server.dirty++;
...
void bitcountCommand(client *c) {
    ...
    if (c->argc == 4) {
        if (getLongFromObjectOrReply(c,c->argv[2],&start,((void *)0)) != 0)
            return;
        if (getLongFromObjectOrReply(c,c->argv[3],&end,((void *)0)) != 0)
            return;
...
void bitposCommand(client *c) {
    ...
    if (getLongFromObjectOrReply(c,c->argv[2],&bit,((void *)0)) != 0)
        return;
    if (bit != 0 && bit != 1) {
    ...
    if (checkType(c,o,0)) return;
    p = getObjectReadOnlyString(o,&strlen,llbuf);

     
    if (c->argc == 4 || c->argc == 5) {
        if (getLongFromObjectOrReply(c,c->argv[3],&start,((void *)0)) != 0)
            return;
        if (c->argc == 5) {
            if (getLongFromObjectOrReply(c,c->argv[4],&end,((void *)0)) != 0)
                return;
            end_given = 1;
        } else {
            end = strlen-1;
...
void bitfieldCommand(client *c) {
    ...
    for (j = 2; j < c->argc; j++) {
        ...
        if (getBitfieldTypeFromArgument(c,c->argv[j+1],&sign,&bits) != 0) {
            zfree(ops);
            return;
        }

        if (getBitOffsetFromArgument(c,c->argv[j+2],&bitoffset,1,bits) != 0){
            zfree(ops);
            return;
        }
        ...
        if (opcode != 0) {
            ...
            if (getLongLongFromObjectOrReply(c,c->argv[j+3],&i64,((void *)0)) != 0){
                zfree(ops);
                return;
            }
        }
