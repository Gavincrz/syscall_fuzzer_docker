...
static size_t rioFdsetWrite(rio *r, const void *buf, size_t len) {
    ...
    if (len) {
        ...
        len = 0;  
        if (sdslen(r->io.fdset.buf) > (1024*16)) doflush = 1;
    }
        ...
        for (j = 0; j < r->io.fdset.numfds; j++) {
            ...
            while(nwritten != count) {
                ...
                if (retval <= 0) {
                    ...
                    if (retval == -1 && errno == 11) errno = 110;
                    break;
                }
                nwritten += retval;
            }
            ...
            if (nwritten != count) {
                ...
                if (r->io.fdset.state[j] == 0) r->io.fdset.state[j] = 5;
            }
        }
...
void rioSetAutoSync(rio *r, off_t bytes) {
    ((r->read == rioFileIO . read)?(void)0 : (_serverAssert("r->read == rioFileIO.read","rio.c",303),_exit(1)));
