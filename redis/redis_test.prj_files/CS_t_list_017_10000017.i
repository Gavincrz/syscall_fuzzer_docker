...
void listTypePush(robj *subject, robj *value, int where) {
    if (subject->encoding == 9) {
        ...
        _serverPanic("t_list.c",49,"Unknown list encoding"),_exit(1);
    }
}
...
robj *listTypePop(robj *subject, int where) {
    ...
    if (subject->encoding == 9) {
        ...
        _serverPanic("t_list.c",69,"Unknown list encoding"),_exit(1);
    }
    return value;
}
...
unsigned long listTypeLength(const robj *subject) {
    if (subject->encoding == 9) {
        ...
        _serverPanic("t_list.c",78,"Unknown list encoding"),_exit(1);
    }
}
...
listTypeIterator *listTypeInitIterator(robj *subject, long index,
    ...
    if (li->encoding == 9) {
        ...
        _serverPanic("t_list.c",98,"Unknown list encoding"),_exit(1);
    }
    return li;
}
...
int listTypeNext(listTypeIterator *li, listTypeEntry *entry) {
    ...
    ((li->subject->encoding == li->encoding)?(void)0 : (_serverAssert("li->subject->encoding == li->encoding","t_list.c",114),_exit(1)));

    entry->li = li;
    if (li->encoding == 9) {
        return quicklistNext(li->iter, &entry->entry);
    } else {
        _serverPanic("t_list.c",120,"Unknown list encoding"),_exit(1);
    }
    return 0;
}
...
robj *listTypeGet(listTypeEntry *entry) {
    ...
    if (entry->li->encoding == 9) {
        ...
        _serverPanic("t_list.c",136,"Unknown list encoding"),_exit(1);
    }
    return value;
}
...
void listTypeInsert(listTypeEntry *entry, robj *value, int where) {
    if (entry->li->encoding == 9) {
        ...
        _serverPanic("t_list.c",155,"Unknown list encoding"),_exit(1);
    }
}
...
int listTypeEqual(listTypeEntry *entry, robj *o) {
    if (entry->li->encoding == 9) {
        (((o->encoding == 0 || o->encoding == 8))?(void)0 : (_serverAssertWithInfo(((void *)0),o,"sdsEncodedObject(o)","t_list.c",162),_exit(1)));
        return quicklistCompare(entry->entry.zi,o->ptr,sdslen(o->ptr));
    } else {
        _serverPanic("t_list.c",165,"Unknown list encoding"),_exit(1);
    }
}
...
void listTypeDelete(listTypeIterator *iter, listTypeEntry *entry) {
    if (entry->li->encoding == 9) {
        ...
        _serverPanic("t_list.c",174,"Unknown list encoding"),_exit(1);
    }
}

 
void listTypeConvert(robj *subject, int enc) {
    ((subject->type==1)?(void)0 : (_serverAssertWithInfo(((void *)0),subject,"subject->type==OBJ_LIST","t_list.c",180),_exit(1)));
    ((subject->encoding==5)?(void)0 : (_serverAssertWithInfo(((void *)0),subject,"subject->encoding==OBJ_ENCODING_ZIPLIST","t_list.c",181),_exit(1)));
    ...
    if (enc == 9) {
        ...
        _serverPanic("t_list.c",189,"Unsupported list conversion"),_exit(1);
    }
}
...
void pushGenericCommand(client *c, int where) {
    ...
    if (pushed) {
        char *event = (where == 0) ? "lpush" : "rpush";

        signalModifiedKey(c->db,c->argv[1]);
        notifyKeyspaceEvent((1<<4),event,c->argv[1],c->db->id);
    }
    server.dirty += pushed;
}
...
void linsertCommand(client *c) {
    ...
    iter = listTypeInitIterator(subject,0,1);
    ...
    if (inserted) {
        signalModifiedKey(c->db,c->argv[1]);
        notifyKeyspaceEvent((1<<4),"linsert",
                            c->argv[1],c->db->id);
        server.dirty++;
    } else {
...
void llenCommand(client *c) {
    ...
    if (o == ((void *)0) || checkType(c,o,1)) return;
...
void lindexCommand(client *c) {
    ...
    if ((getLongFromObjectOrReply(c, c->argv[2], &index, ((void *)0)) != 0))
        return;
    ...
    if (o->encoding == 9) {
        ...
        _serverPanic("t_list.c",337,"Unknown list encoding"),_exit(1);
    }
}
...
void lsetCommand(client *c) {
    ...
    if (o->encoding == 9) {
        ...
        if (!replaced) {
            ...
            signalModifiedKey(c->db,c->argv[1]);
            notifyKeyspaceEvent((1<<4),"lset",c->argv[1],c->db->id);
            server.dirty++;
        }
    } else {
        _serverPanic("t_list.c",363,"Unknown list encoding"),_exit(1);
    }
}
...
void popGenericCommand(client *c, int where) {
    ...
    if (value == ((void *)0)) {
        ...
        char *event = (where == 0) ? "lpop" : "rpop";

        addReplyBulk(c,value);
        decrRefCount(value);
        notifyKeyspaceEvent((1<<4),event,c->argv[1],c->db->id);
        if (listTypeLength(o) == 0) {
            notifyKeyspaceEvent((1<<2),"del",
                                c->argv[1],c->db->id);
...
void lrangeCommand(client *c) {
    ...
    if (o->encoding == 9) {
        listTypeIterator *iter = listTypeInitIterator(o, start, 1);
        ...
        _serverPanic("t_list.c",440,"List encoding is not QUICKLIST!"),_exit(1);
    }
}
...
void ltrimCommand(client *c) {
    ...
    if (o->encoding == 9) {
        ...
        _serverPanic("t_list.c",477,"Unknown list encoding"),_exit(1);
    }

    notifyKeyspaceEvent((1<<4),"ltrim",c->argv[1],c->db->id);
    if (listTypeLength(o) == 0) {
        dbDelete(c->db,c->argv[1]);
        notifyKeyspaceEvent((1<<2),"del",c->argv[1],c->db->id);
    }
    signalModifiedKey(c->db,c->argv[1]);
    server.dirty++;
    addReply(c,shared.ok);
...
void lremCommand(client *c) {
    ...
    if ((getLongFromObjectOrReply(c, c->argv[2], &toremove, ((void *)0)) != 0))
        return;

    subject = lookupKeyWriteOrReply(c,c->argv[1],shared.czero);
    if (subject == ((void *)0) || checkType(c,subject,1)) return;

    listTypeIterator *li;
    if (toremove < 0) {
        toremove = -toremove;
        li = listTypeInitIterator(subject,-1,0);
    } else {
        li = listTypeInitIterator(subject,0,1);
    }
    ...
    if (removed) {
        signalModifiedKey(c->db,c->argv[1]);
        notifyKeyspaceEvent((1<<4),"lrem",c->argv[1],c->db->id);
    }
...
void rpoplpushHandlePush(client *c, robj *dstkey, robj *dstobj, robj *value) {
    ...
    notifyKeyspaceEvent((1<<4),"lpush",dstkey,c->db->id);
...
void rpoplpushCommand(client *c) {
    ...
    if ((sobj = lookupKeyWriteOrReply(c,c->argv[1],shared.null[c->resp]))
        == ((void *)0) || checkType(c,sobj,1)) return;
    ...
    if (listTypeLength(sobj) == 0) {
        ...
        if (dobj && checkType(c,dobj,1)) return;
        ...
        notifyKeyspaceEvent((1<<4),"rpop",touchedkey,c->db->id);
        if (listTypeLength(sobj) == 0) {
...
int serveClientBlockedOnList(client *receiver, robj *key, robj *dstkey, redisDb *db, robj *value, int where)
{
    ...
    if (dstkey == ((void *)0)) {
        ...
        char *event = (where == 0) ? "lpop" : "rpop";
        notifyKeyspaceEvent((1<<4),event,key,receiver->db->id);
    } else {
         
        robj *dstobj =
            lookupKeyWrite(receiver->db,dstkey);
        if (!(dstobj &&
             checkType(receiver,dstobj,1)))
        {
            ...
            notifyKeyspaceEvent((1<<4),"rpop",key,receiver->db->id);
        } else {
...
void blockingPopGenericCommand(client *c, int where) {
        ...
        if (o != ((void *)0)) {
            if (o->type != 1) {
                ...
                if (listTypeLength(o) != 0) {
                    ...
                    char *event = (where == 0) ? "lpop" : "rpop";
                    robj *value = listTypePop(o,where);
                    ((value != ((void *)0))?(void)0 : (_serverAssert("value != NULL","t_list.c",705),_exit(1)));

                    addReplyArrayLen(c,2);
                    addReplyBulk(c,c->argv[j]);
                    addReplyBulk(c,value);
                    decrRefCount(value);
                    notifyKeyspaceEvent((1<<4),event,
                                        c->argv[j],c->db->id);
                    if (listTypeLength(o) == 0) {
                    ...
                    rewriteClientCommandVector(c,2,
                        (where == 0) ? shared.lpop : shared.rpop,
                        c->argv[j]);
                    return;
                }
            }
    ...
    blockForKeys(c,1,c->argv + 1,c->argc - 2,timeout,((void *)0),((void *)0));
}
...
void brpoplpushCommand(client *c) {
    ...
    if (getTimeoutFromObjectOrReply(c,c->argv[3],&timeout,0)
        != 0) return;
    ...
    if (key == ((void *)0)) {
        if (c->flags & (1<<3)) {
            ...
            blockForKeys(c,1,c->argv + 1,1,timeout,c->argv[2],((void *)0));
        }
    } else {
        if (key->type != 1) {
            addReply(c, shared.wrongtypeerr);
            ...
            ((listTypeLength(key) > 0)?(void)0 : (_serverAssertWithInfo(c,key,"listTypeLength(key) > 0","t_list.c",773),_exit(1)));
            rpoplpushCommand(c);
        }
    }
}
