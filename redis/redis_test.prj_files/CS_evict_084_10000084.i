...
unsigned long long estimateObjectIdleTime(robj *o) {
    ...
    if (lruclock >= o->lru) {
        return (lruclock - o->lru) * 1000;
    } else {
        return (lruclock + (((1<<24)-1) - o->lru)) *
                    1000;
    }
}
...
void evictionPoolAlloc(void) {
    ...
    ep = zmalloc(sizeof(*ep)*16);
    for (j = 0; j < 16; j++) {
        ep[j].idle = 0;
        ep[j].key = ((void *)0);
        ep[j].cached = sdsnewlen(((void *)0),255);
        ep[j].dbid = 0;
    }
    EvictionPoolLRU = ep;
}
...
void evictionPoolPopulate(int dbid, dict *sampledict, dict *keydict, struct evictionPoolEntry *pool) {
    ...
    for (j = 0; j < count; j++) {
        ...
        if (server.maxmemory_policy != (2<<8)) {
        ...
        if (server.maxmemory_policy & (1<<0)) {
            idle = estimateObjectIdleTime(o);
        } else if (server.maxmemory_policy & (1<<1)) {
        ...
        } else if (server.maxmemory_policy == (2<<8)) {
             
            idle = (0x7fffffffffffffffLL * 2ULL + 1ULL) - (long)((de)->v . val);
        } else {
            _serverPanic("evict.c",202,"Unknown eviction policy in evictionPoolPopulate()"),_exit(1);
        }
        ...
        while (k < 16 &&
               pool[k].key &&
               pool[k].idle < idle) k++;
        if (k == 0 && pool[16-1].key != ((void *)0)) {
            
 
            continue;
        } else if (k < 16 && pool[k].key == ((void *)0)) {
             
        } else {
            
 
            if (pool[16-1].key == ((void *)0)) {
                
 

                 
                sds cached = pool[16-1].cached;
                memmove(pool+k+1,pool+k,
                    sizeof(pool[0])*(16-k-1));
...
int getMaxmemoryState(size_t *total, size_t *logical, size_t *tofree, float *level) {
    ...
    if (return_ok_asap && !level) return 0;
...
int freeMemoryIfNeeded(void) {
    ...
    if (getMaxmemoryState(&mem_reported,((void *)0),&mem_tofree,((void *)0)) == 0)
        return 0;
    ...
    while (mem_freed < mem_tofree) {
        ...
        if (server.maxmemory_policy & ((1<<0)|(1<<1)) ||
            server.maxmemory_policy == (2<<8))
        {
            ...
            while(bestkey == ((void *)0)) {
                ...
                for (i = 0; i < server.dbnum; i++) {
                    db = server.db+i;
                    dict = (server.maxmemory_policy & (1<<2)) ?
                            db->dict : db->expires;
                    if ((keys = ((dict)->ht[0]. used+(dict)->ht[1]. used)) != 0) {
        ...
        else if (server.maxmemory_policy == ((6<<8)|(1<<2)) ||
                 server.maxmemory_policy == (3<<8))
        {
            ...
            for (i = 0; i < server.dbnum; i++) {
                ...
                db = server.db+j;
                dict = (server.maxmemory_policy == ((6<<8)|(1<<2))) ?
                        db->dict : db->expires;
                if (((dict)->ht[0]. used+(dict)->ht[1]. used) != 0) {
        ...
        if (bestkey) {
            ...
            if (server . latency_monitor_threshold) { eviction_latency = mstime(); } else { eviction_latency = 0; };
            if (server.lazyfree_lazy_eviction)
                dbAsyncDelete(db,keyobj);
            else
                dbSyncDelete(db,keyobj);
            if (server . latency_monitor_threshold) { eviction_latency = mstime() - eviction_latency; };
            if (server . latency_monitor_threshold && (eviction_latency) >= server . latency_monitor_threshold) latencyAddSample(("eviction-del"),(eviction_latency));;
            latency += eviction_latency;;
            delta -= (long long) zmalloc_used_memory();
            mem_freed += delta;
        ...
        if (!keys_freed) {
            ...
            if (server . latency_monitor_threshold && (latency) >= server . latency_monitor_threshold) latencyAddSample(("eviction-cycle"),(latency));;
            goto cant_free;  
        }
    }
    if (server . latency_monitor_threshold) { latency = mstime() - latency; };
