...
# 43 "rdb.c"
# 1 "/usr/include/x86_64-linux-gnu/sys/param.h"
...
# 19 "/home/gavin/codesonar-5.0p0.20180811-x86_64-pc-linux/codesonar-5.0p0/csurf/csinclude/stddef.h"
# 24 "/usr/include/x86_64-linux-gnu/sys/param.h"
...
# 27 "/usr/include/x86_64-linux-gnu/sys/param.h"



 
# 1 "/usr/include/x86_64-linux-gnu/bits/param.h"
...
# 1 "/usr/include/linux/param.h"
 

#define _LINUX_PARAM_H

# 1 "/usr/include/x86_64-linux-gnu/asm/param.h"
# 1 "/usr/include/asm-generic/param.h"
...
# 2 "/usr/include/x86_64-linux-gnu/asm/param.h"
# 6 "/usr/include/linux/param.h"

# 29 "/usr/include/x86_64-linux-gnu/bits/param.h"
...
# 32 "/usr/include/x86_64-linux-gnu/sys/param.h"
...
# 65 "/usr/include/x86_64-linux-gnu/sys/param.h"
...
void rdbCheckThenExit(int linenum, char *reason, ...) {
    ...
    if (!rdbCheckMode) {
        ...
        redis_check_rdb_main(2,argv,((void *)0));
    } else {
        rdbCheckError("%s",msg);
    }
    exit(1);
...
void rdbLoadRaw(rio *rdb, void *buf, uint64_t len) {
    if (rioRead(rdb,buf,len) == 0) {
        rdbCheckThenExit(85,"Impossible to read %llu bytes in rdbLoadRaw()", (unsigned long long) len);
...
int rdbLoadLenByRef(rio *rdb, int *isencoded, uint64_t *lenptr) {
    ...
    if (type == 3) {
        ...
        *lenptr = intrev64(len);
        ...
        rdbCheckThenExit(213,"Unknown length encoding %d in rdbLoadLen()",type);
...
uint64_t rdbLoadLen(rio *rdb, int *isencoded) {
    ...
    if (rdbLoadLenByRef(rdb,isencoded,&len) == -1) return (18446744073709551615UL);
    return len;
}
...
void *rdbLoadIntegerObject(rio *rdb, int enctype, int flags, size_t *lenptr) {
    int plain = flags & (1<<1);
    int sds = flags & (1<<2);
    int encode = flags & (1<<0);
    unsigned char enc[4];
    long long val;

    if (enctype == 0) {
        if (rioRead(rdb,enc,1) == 0) return ((void *)0);
        val = (signed char)enc[0];
    } else if (enctype == 1) {
        uint16_t v;
        if (rioRead(rdb,enc,2) == 0) return ((void *)0);
        v = enc[0]|(enc[1]<<8);
        val = (int16_t)v;
    } else if (enctype == 2) {
        uint32_t v;
        if (rioRead(rdb,enc,4) == 0) return ((void *)0);
        v = enc[0]|(enc[1]<<8)|(enc[2]<<16)|(enc[3]<<24);
        val = (int32_t)v;
    } else {
        val = 0;  
        rdbCheckThenExit(281,"Unknown RDB integer encoding type %d",enctype);
    }
    if (plain || sds) {
        char buf[21], *p;
        ...
        return createObject(0,sdsfromlonglong(val));
    }
}
...
void *rdbLoadLzfStringObject(rio *rdb, int flags, size_t *lenptr) {
    ...
    if ((clen = rdbLoadLen(rdb,((void *)0))) == (18446744073709551615UL)) return ((void *)0);
    if ((len = rdbLoadLen(rdb,((void *)0))) == (18446744073709551615UL)) return ((void *)0);
...
ssize_t rdbSaveLongLongAsStringObject(rio *rdb, long long value) {
    ...
    if (enclen > 0) {
        ...
        ((enclen < 32)?(void)0 : (_serverAssert("enclen < 32","rdb.c",447),_exit(1)));
...
ssize_t rdbSaveStringObject(rio *rdb, robj *obj) {
    ...
    if (obj->encoding == 1) {
        ...
        (((obj->encoding == 0 || obj->encoding == 8))?(void)0 : (_serverAssertWithInfo(((void *)0),obj,"sdsEncodedObject(obj)","rdb.c",463),_exit(1)));
...
void *rdbGenericLoadStringObject(rio *rdb, int flags, size_t *lenptr) {
    ...
    if (isencoded) {
        ...
        default:
            rdbCheckThenExit(498,"Unknown RDB string encoding type %d",len);
        }
    }

    if (len == (18446744073709551615UL)) return ((void *)0);
    if (plain || sds) {
...
robj *rdbLoadStringObject(rio *rdb) {
    return rdbGenericLoadStringObject(rdb,0,((void *)0));
}

robj *rdbLoadEncodedStringObject(rio *rdb) {
    return rdbGenericLoadStringObject(rdb,(1<<0),((void *)0));
}
...
int rdbSaveDoubleValue(rio *rdb, double val) {
    ...
    if (( cs_isnan( (double) (val) ) )) {
        buf[0] = 253;
        len = 1;
    } else if (!( cs_isfinite( (double) (val) ) )) {
        len = 1;
        buf[0] = (val < 0) ? 255 : 254;
    } else {
# 568 "rdb.c"
...
int rdbSaveObjectType(rio *rdb, robj *o) {
    ...
    case 1:
        if (o->encoding == 9)
            return rdbSaveType(rdb,14);
        else
            _serverPanic("rdb.c",633,"Unknown list encoding"),_exit(1);
    case 2:
        if (o->encoding == 6)
            ...
            _serverPanic("rdb.c",640,"Unknown set encoding"),_exit(1);
    case 3:
        if (o->encoding == 5)
            return rdbSaveType(rdb,12);
        else if (o->encoding == 7)
            return rdbSaveType(rdb,5);
        else
            _serverPanic("rdb.c",647,"Unknown sorted set encoding"),_exit(1);
    case 4:
        if (o->encoding == 5)
            return rdbSaveType(rdb,13);
        else if (o->encoding == 2)
            return rdbSaveType(rdb,4);
        else
            _serverPanic("rdb.c",654,"Unknown hash encoding"),_exit(1);
    case 6:
        return rdbSaveType(rdb,15);
    case 5:
        return rdbSaveType(rdb,7);
    default:
        _serverPanic("rdb.c",660,"Unknown object type"),_exit(1);
    }
    return -1;  
}
...
int rdbLoadObjectType(rio *rdb) {
    ...
    if (!((type >= 0 && type <= 7) || (type >= 9 && type <= 15))) return -1;
    return type;
}
...
ssize_t rdbSaveObject(rio *rdb, robj *o, robj *key) {
    ...
    if (o->type == 0) {
        ...
        if (o->encoding == 9) {
            ...
            _serverPanic("rdb.c",783,"Unknown list encoding"),_exit(1);
        }
    } else if (o->type == 2) {
        ...
        if (o->encoding == 2) {
            ...
            if ((n = rdbSaveLen(rdb,((set)->ht[0]. used+(set)->ht[1]. used))) == -1) {
            ...
            dictReleaseIterator(di);
        } else if (o->encoding == 6) {
            size_t l = intsetBlobLen((intset*)o->ptr);

            if ((n = rdbSaveRawString(rdb,o->ptr,l)) == -1) return -1;
            nwritten += n;
        } else {
            _serverPanic("rdb.c",815,"Unknown set encoding"),_exit(1);
        }
    } else if (o->type == 3) {
        ...
        if (o->encoding == 5) {
            ...
            _serverPanic("rdb.c",851,"Unknown sorted set encoding"),_exit(1);
        }
    } else if (o->type == 4) {
        ...
        if (o->encoding == 5) {
            ...
            if ((n = rdbSaveLen(rdb,(((dict*)o->ptr)->ht[0]. used+((dict*)o->ptr)->ht[1]. used))) == -1) {
                dictReleaseIterator(di);
                return -1;
            }
            nwritten += n;
            ...
            _serverPanic("rdb.c",892,"Unknown hash encoding"),_exit(1);
        }
    } else if (o->type == 6) {
        ...
        do { io . rio = rdb; io . type = mt; io . bytes = 0; io . error = 0; io . ver = 0; io . key = key; io . ctx = ((void *)0); } while(0);;
        ...
        _serverPanic("rdb.c",989,"Unknown object type"),_exit(1);
    }
    return nwritten;
}
...
size_t rdbSavedObjectLen(robj *o) {
    ...
    ((len != -1)?(void)0 : (_serverAssertWithInfo(((void *)0),o,"len != -1","rdb.c",1000),_exit(1)));
    return len;
}
...
int rdbSaveKeyValuePair(rio *rdb, robj *key, robj *val, long long expiretime) {
    int savelru = server.maxmemory_policy & (1<<0);
    int savelfu = server.maxmemory_policy & (1<<1);

     
    if (expiretime != -1) {
        if (rdbSaveType(rdb,252) == -1) return -1;
    ...
    if (savelru) {
        ...
        idletime /= 1000;  
        if (rdbSaveType(rdb,248) == -1) return -1;
    ...
    if (savelfu) {
        ...
        if (rdbSaveType(rdb,249) == -1) return -1;
        if (rdbWriteRaw(rdb,buf,1) == -1) return -1;
    }
...
ssize_t rdbSaveAuxField(rio *rdb, void *key, size_t keylen, void *val, size_t vallen) {
    ssize_t ret, len = 0;
    if ((ret = rdbSaveType(rdb,250)) == -1) return -1;
    len += ret;
...
int rdbSaveInfoAuxFields(rio *rdb, int flags, rdbSaveInfo *rsi) {
    ...
    int aof_preamble = (flags & (1<<0)) != 0;

     
    if (rdbSaveAuxFieldStrStr(rdb,"redis-ver","999.999.999") == -1) return -1;
...
int rdbSaveRio(rio *rdb, int *error, int flags, rdbSaveInfo *rsi) {
    ...
    snprintf(magic,sizeof(magic),"REDIS%04d",9);
    ...
    for (j = 0; j < server.dbnum; j++) {
        ...
        dict *d = db->dict;
        if (((d)->ht[0]. used+(d)->ht[1]. used) == 0) continue;
        di = dictGetSafeIterator(d);

         
        if (rdbSaveType(rdb,254) == -1) goto werr;
        ...
        db_size = ((db->dict)->ht[0]. used+(db->dict)->ht[1]. used);
        expires_size = ((db->expires)->ht[0]. used+(db->expires)->ht[1]. used);
        if (rdbSaveType(rdb,251) == -1) goto werr;
        ...
        while((de = dictNext(di)) != ((void *)0)) {
            ...
            do { key . refcount = 1; key . type = 0; key . encoding = 0; key . ptr = keystr; } while(0);
            ...
            if (flags & (1<<0) &&
                rdb->processed_bytes > processed+(1024*10))
            {
    ...
    if (rsi && ((server . lua_scripts)->ht[0]. used+(server . lua_scripts)->ht[1]. used)) {
    ...
    if (rdbSaveType(rdb,255) == -1) goto werr;
...
int rdbSaveRioWithEOFMark(rio *rdb, int *error, rdbSaveInfo *rsi) {
    ...
    if (rioWrite(rdb,eofmark,40) == 0) goto werr;
    if (rioWrite(rdb,"\r\n",2) == 0) goto werr;
    if (rdbSaveRio(rdb,error,0,rsi) == -1) goto werr;
    if (rioWrite(rdb,eofmark,40) == 0) goto werr;
    return 0;
...
int rdbSave(char *filename, rdbSaveInfo *rsi) {
    ...
    if (server.rdb_save_incremental_fsync)
        rioSetAutoSync(&rdb,(1024*1024*32));

    if (rdbSaveRio(&rdb,&error,0,rsi) == -1) {
        errno = error;
        goto werr;
    }

     
    if (fflush(fp) == (-1)) goto werr;
    if (fsync(fileno(fp)) == -1) goto werr;
    if (fclose(fp) == (-1)) goto werr;
...
werr:
    serverLog(3,"Write error saving DB on disk: %s", strerror(errno));
    fclose(fp);
    unlink(tmpfile);
    return -1;
}
...
int rdbSaveBackground(char *filename, rdbSaveInfo *rsi) {
    ...
    if (server.aof_child_pid != -1 || server.rdb_child_pid != -1) return -1;
    ...
    if ((childpid = fork()) == 0) {
        ...
        if (server . latency_monitor_threshold && (server . stat_fork_time/1000) >= server . latency_monitor_threshold) latencyAddSample(("fork"),(server . stat_fork_time/1000));;
        if (childpid == -1) {
            closeChildInfoPipe();
            server.lastbgsave_status = -1;
            serverLog(3,"Can't save in background: fork: %s",
                strerror(errno));
            return -1;
        }
        serverLog(2,"Background saving started by pid %d",childpid);
        server.rdb_save_time_start = time(((void *)0));
        server.rdb_child_pid = childpid;
        server.rdb_child_type = 1;
...
robj *rdbLoadCheckModuleValue(rio *rdb, char *modulename) {
    uint64_t opcode;
    while((opcode = rdbLoadLen(rdb,((void *)0))) != 0) {
        if (opcode == 1 ||
            opcode == 2)
        {
            uint64_t len;
            if (rdbLoadLenByRef(rdb,((void *)0),&len) == -1) {
                rdbCheckThenExit(1355,"Error reading integer from module %s value", modulename);

            }
        } else if (opcode == 5) {
            robj *o = rdbGenericLoadStringObject(rdb,0,((void *)0));
            if (o == ((void *)0)) {
                rdbCheckThenExit(1361,"Error reading string from module %s value", modulename);
            ...
            if (rdbLoadBinaryFloatValue(rdb,&val) == -1) {
                rdbCheckThenExit(1368,"Error reading float from module %s value", modulename);

            }
        } else if (opcode == 4) {
            double val;
            if (rdbLoadBinaryDoubleValue(rdb,&val) == -1) {
                rdbCheckThenExit(1374,"Error reading double from module %s value", modulename);
...
robj *rdbLoadObject(int rdbtype, rio *rdb, robj *key) {
    robj *o = ((void *)0), *ele, *dec;
    uint64_t len;
    unsigned int i;

    if (rdbtype == 0) {
         
        if ((o = rdbLoadEncodedStringObject(rdb)) == ((void *)0)) return ((void *)0);
        ...
        while(len--) {
            if ((ele = rdbLoadEncodedStringObject(rdb)) == ((void *)0)) return ((void *)0);
        ...
        for (i = 0; i < len; i++) {
            ...
            if ((sdsele = rdbGenericLoadStringObject(rdb,(1<<2),((void *)0)))
                == ((void *)0)) return ((void *)0);

            if (o->encoding == 6) {
                 
                if (isSdsRepresentableAsLongLong(sdsele,&llval) == 0) {
                    o->ptr = intsetAdd(o->ptr,llval,((void *)0));
                } else {
                    setTypeConvert(o,2);
                    dictExpand(o->ptr,len);
                }
            ...
            if (o->encoding == 2) {
                dictAdd((dict*)o->ptr,sdsele,((void *)0));
            } else {
                sdsfree(sdsele);
            }
        }
        ...
        if ((zsetlen = rdbLoadLen(rdb,((void *)0))) == (18446744073709551615UL)) return ((void *)0);
        o = createZsetObject();
        zs = o->ptr;
        ...
        while(zsetlen--) {
            ...
            if (rdbtype == 5) {
                if (rdbLoadBinaryDoubleValue(rdb,&score) == -1) return ((void *)0);
            } else {
                if (rdbLoadDoubleValue(rdb,&score) == -1) return ((void *)0);
            }
        ...
        while (o->encoding == 5 && len > 0) {
            len--;
             
            if ((field = rdbGenericLoadStringObject(rdb,(1<<2),((void *)0)))
                == ((void *)0)) return ((void *)0);
            if ((value = rdbGenericLoadStringObject(rdb,(1<<2),((void *)0)))
                == ((void *)0)) return ((void *)0);
        ...
        if (o->encoding == 2 && len > 4)
            dictExpand(o->ptr,len);

         
        while (o->encoding == 2 && len > 0) {
            len--;
            ...
            if (ret == 1) {
                rdbCheckThenExit(1546,"Duplicate keys detected");
            }
        }

         
        ((len == 0)?(void)0 : (_serverAssert("len == 0","rdb.c",1551),_exit(1)));
    } else if (rdbtype == 14) {
        if ((len = rdbLoadLen(rdb,((void *)0))) == (18446744073709551615UL)) return ((void *)0);
        o = createQuicklistObject();
        quicklistSetOptions(o->ptr, server.list_max_ziplist_size,
        ...
        while (len--) {
            unsigned char *zl =
                rdbGenericLoadStringObject(rdb,(1<<1),((void *)0));
            if (zl == ((void *)0)) return ((void *)0);
               ...
               rdbtype == 13)
    {
        ...
        switch(rdbtype) {
            case 9:
 ...
 
                {
                    ...
                    while ((zi = zipmapNext(zi, &fstr, &flen, &vstr, &vlen)) != ((void *)0)) {
                        if (flen > maxlen) maxlen = flen;
                        if (vlen > maxlen) maxlen = vlen;
                        zl = ziplistPush(zl, fstr, flen, 1);
                        zl = ziplistPush(zl, vstr, vlen, 1);
                    }
            ...
            case 13:
                ...
                break;
            default:
                rdbCheckThenExit(1635,"Unknown RDB encoding type %d",rdbtype);
                break;
        }
    } else if (rdbtype == 15) {
        o = createStreamObject();
        stream *s = o->ptr;
        uint64_t listpacks = rdbLoadLen(rdb,((void *)0));

        while(listpacks--) {
            

 
            sds nodekey = rdbGenericLoadStringObject(rdb,(1<<2),((void *)0));
            if (nodekey == ((void *)0)) {
                rdbCheckThenExit(1649,"Stream master ID loading failed: invalid encoding or I/O error.");
            }
            if (sdslen(nodekey) != sizeof(streamID)) {
                rdbCheckThenExit(1653,"Stream node key entry is not the " "size of a stream ID");
            ...
            if (first == ((void *)0)) {
                ...
                rdbCheckThenExit(1665,"Empty listpack inside stream");
            }

             
            int retval = raxInsert(s->rax,
                (unsigned char*)nodekey,sizeof(streamID),lp,((void *)0));
            sdsfree(nodekey);
            if (!retval)
                rdbCheckThenExit(1673,"Listpack re-added with existing key");
        }
         
        s->length = rdbLoadLen(rdb,((void *)0));
         
        s->last_id.ms = rdbLoadLen(rdb,((void *)0));
        s->last_id.seq = rdbLoadLen(rdb,((void *)0));

         
        size_t cgroups_count = rdbLoadLen(rdb,((void *)0));
        while(cgroups_count--) {
            

 
            streamID cg_id;
            sds cgname = rdbGenericLoadStringObject(rdb,(1<<2),((void *)0));
            if (cgname == ((void *)0)) {
                rdbCheckThenExit(1691,"Error reading the consumer group name from Stream");

            }
            cg_id.ms = rdbLoadLen(rdb,((void *)0));
            cg_id.seq = rdbLoadLen(rdb,((void *)0));
            streamCG *cgroup = streamCreateCG(s,cgname,sdslen(cgname),&cg_id);
            if (cgroup == ((void *)0))
                rdbCheckThenExit(1698,"Duplicated consumer group name %s", cgname);
            ...
            size_t pel_size = rdbLoadLen(rdb,((void *)0));
            while(pel_size--) {
                unsigned char rawid[sizeof(streamID)];
                rdbLoadRaw(rdb,rawid,sizeof(rawid));
                streamNACK *nack = streamCreateNACK(((void *)0));
                nack->delivery_time = rdbLoadMillisecondTime(rdb,9);
                nack->delivery_count = rdbLoadLen(rdb,((void *)0));
                if (!raxInsert(cgroup->pel,rawid,sizeof(rawid),nack,((void *)0)))
                    rdbCheckThenExit(1715,"Duplicated gobal PEL entry " "loading stream consumer group");

            }

            
 
            size_t consumers_num = rdbLoadLen(rdb,((void *)0));
            while(consumers_num--) {
                sds cname = rdbGenericLoadStringObject(rdb,(1<<2),((void *)0));
                if (cname == ((void *)0)) {
                    rdbCheckThenExit(1725,"Error reading the consumer name from Stream group");

                }
                streamConsumer *consumer = streamLookupConsumer(cgroup,cname,
                                           1);
                sdsfree(cname);
                consumer->seen_time = rdbLoadMillisecondTime(rdb,9);

                
 
                pel_size = rdbLoadLen(rdb,((void *)0));
                while(pel_size--) {
                    unsigned char rawid[sizeof(streamID)];
                    rdbLoadRaw(rdb,rawid,sizeof(rawid));
                    streamNACK *nack = raxFind(cgroup->pel,rawid,sizeof(rawid));
                    if (nack == raxNotFound)
                        rdbCheckThenExit(1741,"Consumer entry not found in " "group global PEL");
                    ...
                    if (!raxInsert(consumer->pel,rawid,sizeof(rawid),nack,((void *)0)))
                        rdbCheckThenExit(1750,"Duplicated consumer PEL entry " " loading a stream consumer " "group");


                }
            }
        }
    } else if (rdbtype == 6 || rdbtype == 7) {
        uint64_t moduleid = rdbLoadLen(rdb,((void *)0));
        moduleType *mt = moduleTypeLookupModuleByID(moduleid);
        char name[10];

        if (rdbCheckMode && rdbtype == 7) {
        ...
        if (mt == ((void *)0)) {
            moduleTypeNameByID(name,moduleid);
            serverLog(3,"The RDB file contains module data I can't load: no matching module '%s'", name);
            exit(1);
        }
        RedisModuleIO io;
        do { io . rio = rdb; io . type = mt; io . bytes = 0; io . error = 0; io . ver = 0; io . key = key; io . ctx = ((void *)0); } while(0);;
        io.ver = (rdbtype == 6) ? 1 : 2;
        ...
        if (io.ver == 2) {
            ...
            if (eof != 0) {
                serverLog(3,"The RDB file contains module data for the module '%s' that is not terminated by the proper module value EOF marker", name);
                exit(1);
            }
        }
        ...
        if (ptr == ((void *)0)) {
            moduleTypeNameByID(name,moduleid);
            serverLog(3,"The RDB file contains module data for the module type '%s', that the responsible module is not able to load. Check for modules log above for additional clues.", name);
            exit(1);
        }
        o = createModuleObject(mt,ptr);
    } else {
        rdbCheckThenExit(1796,"Unknown RDB encoding type %d",rdbtype);
    }
    return o;
}
...
void rdbLoadProgressCallback(rio *r, const void *buf, size_t len) {
        ...
        (r->processed_bytes + len)/server.loading_process_events_interval_bytes > r->processed_bytes/server.loading_process_events_interval_bytes)
    {
        ...
        updateCachedTime();
        if (server.masterhost && server.repl_state == 14)
...
int rdbLoadRio(rio *rdb, rdbSaveInfo *rsi, int loading_aof) {
    ...
    if (memcmp(buf,"REDIS",5) != 0) {
        serverLog(3,"Wrong signature trying to load DB from file");
        errno = 22;
        return -1;
    }
    rdbver = atoi(buf+5);
    if (rdbver < 1 || rdbver > 9) {
        serverLog(3,"Can't handle RDB format version %d",rdbver);
        errno = 22;
        return -1;
    }
    ...
    while(1) {
        ...
        if (type == 253) {
            ...
            uint64_t qword;
            if ((qword = rdbLoadLen(rdb,((void *)0))) == (18446744073709551615UL)) goto eoferr;
            lru_idle = qword;
            continue;  
        } else if (type == 255) {
            ...
            if ((dbid = rdbLoadLen(rdb,((void *)0))) == (18446744073709551615UL)) goto eoferr;
            ...
            if (((char*)auxkey->ptr)[0] == '%') {
                ...
                if (luaCreateFunction(((void *)0),server.lua,auxval) == ((void *)0)) {
                    rdbCheckThenExit(1964,"Can't load Lua script from RDB file! " "BODY: %s", auxval->ptr);


                }
            } else if (!strcasecmp(auxkey->ptr,"redis-ver")) {
                serverLog(2,"Loading RDB produced by version %s",
                    (char*)auxval->ptr);
            } else if (!strcasecmp(auxkey->ptr,"ctime")) {
                time_t age = time(((void *)0))-strtol(auxval->ptr,((void *)0),10);
                if (age < 0) age = 0;
                serverLog(2,"RDB age %ld seconds",
                    (unsigned long) age);
            } else if (!strcasecmp(auxkey->ptr,"used-mem")) {
                long long usedmem = strtoll(auxval->ptr,((void *)0),10);
                serverLog(2,"RDB memory usage when created %.2f Mb",
                    (double) usedmem / (1024*1024));
            } else if (!strcasecmp(auxkey->ptr,"aof-preamble")) {
                long long haspreamble = strtoll(auxval->ptr,((void *)0),10);
                if (haspreamble) serverLog(2,"RDB has an AOF tail");
            } else if (!strcasecmp(auxkey->ptr,"redis-bits")) {
                 
            } else {
                
 
                serverLog(0,"Unrecognized RDB AUX field: '%s'",
                    (char*)auxkey->ptr);
            }
            ...
            if (!rdbCheckMode && mt == ((void *)0)) {
                ...
                serverLog(3,"The RDB file contains AUX module data I can't load: no matching module '%s'", name);
                exit(1);
            } else if (!rdbCheckMode && mt != ((void *)0)) {
                
 
                serverLog(3,"The RDB file contains AUX module data I can't load for the module '%s'. Probably you want to use a newer version of Redis which implements aux data callbacks", name);
                exit(1);
            } else {
        ...
        if (server.masterhost == ((void *)0) && !loading_aof && expiretime != -1 && expiretime < now) {
            decrRefCount(key);
            decrRefCount(val);
        } else {
    ...
    if (rdbver >= 5) {
        ...
        if (server.rdb_checksum) {
            ((void)(0));
            if (cksum == 0) {
                serverLog(3,"RDB file was saved with checksum disabled: no check performed.");
            } else if (cksum != expected) {
                serverLog(3,"Wrong RDB checksum. Aborting now.");
                rdbCheckThenExit(2067,"RDB CRC error");
            }
        }
    }
    return 0;
...
eoferr:  
    serverLog(3,"Short read or OOM loading DB. Unrecoverable error, aborting now.");
    rdbCheckThenExit(2075,"Unexpected EOF reading RDB file");
    return -1;  
}
...
int rdbLoad(char *filename, rdbSaveInfo *rsi) {
    ...
    if ((fp = fopen(filename,"r")) == ((void *)0)) return -1;
    startLoading(fp);
    rioInitWithFile(&rdb,fp);
...
void backgroundSaveDoneHandlerDisk(int exitcode, int bysignal) {
    if (!bysignal && exitcode == 0) {
        ...
        serverLog(3, "Background saving error");
        server.lastbgsave_status = -1;
    } else {
        mstime_t latency;
        ...
        if (server . latency_monitor_threshold) { latency = mstime(); } else { latency = 0; };
        rdbRemoveTempFile(server.rdb_child_pid);
        if (server . latency_monitor_threshold) { latency = mstime() - latency; };
        if (server . latency_monitor_threshold && (latency) >= server . latency_monitor_threshold) latencyAddSample(("rdb-unlink-temp-file"),(latency));;
    ...
    updateSlavesWaitingBgsave((!bysignal && exitcode == 0) ? 0 : -1, 1);
}
...
void backgroundSaveDoneHandlerSocket(int exitcode, int bysignal) {
    ...
    if (!bysignal && exitcode == 0) {
        ...
        serverLog(3, "Background transfer error");
    } else {
        serverLog(3,
    ...
    while((ln = listNext(&li))) {
        ...
        if (slave->replstate == 7) {
            ...
            if (j == ok_slaves[0] || errorcode != 0) {
                ...
                anetNonBlock(((void *)0),slave->fd);
                anetSendTimeout(((void *)0),slave->fd,0);
            }
        }
    }
    zfree(ok_slaves);

    updateSlavesWaitingBgsave((!bysignal && exitcode == 0) ? 0 : -1, 2);
}
...
void backgroundSaveDoneHandler(int exitcode, int bysignal) {
    ...
    case 2:
        ...
        break;
    default:
        _serverPanic("rdb.c",2240,"Unknown RDB child type."),_exit(1);
        break;
    }
}
...
int rdbSaveToSlavesSockets(rdbSaveInfo *rsi) {
    ...
    while((ln = listNext(&li))) {
        ...
        if (slave->replstate == 6) {
            ...
            anetBlock(((void *)0),slave->fd);
            anetSendTimeout(((void *)0),slave->fd,server.repl_timeout*1000);
        }
    }
    ...
    if ((childpid = fork()) == 0) {
        ...
        retval = rdbSaveRioWithEOFMark(&slave_sockets,((void *)0),rsi);
        if (retval == 0 && rioFlush(&slave_sockets) == 0)
            retval = -1;
        ...
        if (childpid == -1) {
            ...
            serverLog(2,"Background RDB transfer started by pid %d",
                childpid);
            server.rdb_save_time_start = time(((void *)0));
            server.rdb_child_pid = childpid;
            server.rdb_child_type = 2;
            updateDictResizePolicy();
        }
        zfree(clientids);
        zfree(fds);
        return (childpid == -1) ? -1 : 0;
    }
    return 0;  
}
...
void bgsaveCommand(client *c) {
    ...
    } else if (rdbSaveBackground(server.rdb_filename,rsiptr) == 0) {
...
rdbSaveInfo *rdbPopulateSaveInfo(rdbSaveInfo *rsi) {
    rdbSaveInfo rsi_init = {-1,0,"000000000000000000000000000000",-1};
    *rsi = rsi_init;
