...
int getTimeoutFromObjectOrReply(client *c, robj *object, mstime_t *timeout, int unit) {
    ...
    if (unit == 0) {
        if (getLongDoubleFromObjectOrReply(c,object,&ftval,
            "timeout is not an float or out of range") != 0)
            return -1;
        tval = (long long) (ftval * 1000.0);
    } else {
        if (getLongLongFromObjectOrReply(c,object,&tval,
            "timeout is not an integer or out of range") != 0)
            return -1;
    }
...
void processUnblockedClients(void) {
    ...
    while (((server . unblocked_clients)->len)) {
        ...
        ((ln != ((void *)0))?(void)0 : (_serverAssert("ln != NULL","blocked.c",125),_exit(1)));
        c = ln->value;
...
void unblockClient(client *c) {
    if (c->btype == 1 ||
        ...
        _serverPanic("blocked.c",179,"Unknown btype in unblockClient()."),_exit(1);
    }
...
void replyToBlockedClientTimedOut(client *c) {
    if (c->btype == 1 ||
        ...
        _serverPanic("blocked.c",203,"Unknown btype in replyToBlockedClientTimedOut()."),_exit(1);
    }
}
        ...
        while(((l)->len) != 0) {
            ...
            if (o != ((void *)0) && o->type == 1) {
                ...
                if (de) {
                    ...
                    int numclients = ((clients)->len);

                    while(numclients--) {
                        listNode *clientnode = ((clients)->head);
                ...
                if (listTypeLength(o) == 0) {
                    ...
                    notifyKeyspaceEvent((1<<2),"del",rl->key,rl->db->id);
                }
                ...
                if (de) {
                    ...
                    while(numclients-- && zcard) {
                        ...
                        robj *argv[2];
                        struct redisCommand *cmd = where == 0 ?
...
void blockForKeys(client *c, int btype, robj **keys, int numkeys, mstime_t timeout, robj *target, streamID *ids) {
    ...
    for (j = 0; j < numkeys; j++) {
        ...
        if (dictAdd(c->bpop.keys,keys[j],key_data) != 0) {
            zfree(key_data);
            continue;
        }
        incrRefCount(keys[j]);
        ...
        if (de == ((void *)0)) {
            ...
            incrRefCount(keys[j]);
            ((retval == 0)?(void)0 : (_serverAssertWithInfo(c,keys[j],"retval == DICT_OK","blocked.c",543),_exit(1)));
        } else {
            l = ((de)->v . val);
        }
        listAddNodeTail(l,c);
...
void unblockClientWaitingData(client *c) {
    ...
    ((((c->bpop . keys)->ht[0]. used+(c->bpop . keys)->ht[1]. used) != 0)?(void)0 : (_serverAssertWithInfo(c,((void *)0),"dictSize(c->bpop.keys) != 0","blocked.c",559),_exit(1)));
    ...
    while((de = dictNext(di)) != ((void *)0)) {
        ...
        ((l != ((void *)0))?(void)0 : (_serverAssertWithInfo(c,key,"l != NULL","blocked.c",567),_exit(1)));
...
void signalKeyAsReady(redisDb *db, robj *key) {
    ...
    incrRefCount(key);
    ((dictAdd(db->ready_keys,key,((void *)0)) == 0)?(void)0 : (_serverAssert("dictAdd(db->ready_keys,key,NULL) == DICT_OK","blocked.c",616),_exit(1)));
}
