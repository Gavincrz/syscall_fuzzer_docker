...
void activeExpireCycle(int type) {
    ...
    elapsed = ustime()-start;
    if (server . latency_monitor_threshold && (elapsed/1000) >= server . latency_monitor_threshold) latencyAddSample(("expire-cycle"),(elapsed/1000));;
...
dict *slaveKeysWithExpire = ((void *)0);


 
void expireSlaveKeys(void) {
    if (slaveKeysWithExpire == ((void *)0) ||
        ((slaveKeysWithExpire)->ht[0]. used+(slaveKeysWithExpire)->ht[1]. used) == 0) return;
    ...
    while(1) {
        ...
        if (((slaveKeysWithExpire)->ht[0]. used+(slaveKeysWithExpire)->ht[1]. used) == 0) break;
    }
}
...
void rememberSlaveKeyWithExpire(redisDb *db, robj *key) {
    if (slaveKeysWithExpire == ((void *)0)) {
        ...
        slaveKeysWithExpire = dictCreate(&dt,((void *)0));
    }
    if (db->id > 63) return;
    ...
    if (de->key == key->ptr) {
        ...
        do { (de)->v . u64 = 0; } while(0);
    }

    uint64_t dbids = ((de)->v . u64);
    dbids |= (uint64_t)1 << db->id;
    do { (de)->v . u64 = dbids; } while(0);
}

 
size_t getSlaveKeyWithExpireCount(void) {
    if (slaveKeysWithExpire == ((void *)0)) return 0;
    return ((slaveKeysWithExpire)->ht[0]. used+(slaveKeysWithExpire)->ht[1]. used);
}
...
void expireGenericCommand(client *c, long long basetime, int unit) {
    ...
    if (getLongLongFromObjectOrReply(c, param, &when, ((void *)0)) != 0)
        return;
    ...
    if (when <= mstime() && !server.loading && !server.masterhost) {
        ...
        ((deleted)?(void)0 : (_serverAssertWithInfo(c,key,"deleted","expire.c",432),_exit(1)));
        server.dirty++;
        ...
        addReply(c,shared.cone);
        signalModifiedKey(c->db,key);
        notifyKeyspaceEvent((1<<2),"expire",key,c->db->id);
        server.dirty++;
        return;
    }
}
...
void expireCommand(client *c) {
    expireGenericCommand(c,mstime(),0);
}
...
void pexpireCommand(client *c) {
    expireGenericCommand(c,mstime(),1);
}
...
void ttlGenericCommand(client *c, int output_ms) {
    ...
    if (lookupKeyReadWithFlags(c->db,c->argv[1],(1<<0)) == ((void *)0)) {
