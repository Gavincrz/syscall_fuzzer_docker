...
void setGenericCommand(client *c, int flags, robj *key, robj *val, robj *expire, int unit, robj *ok_reply, robj *abort_reply) {
    ...
    if (expire) {
        if (getLongLongFromObjectOrReply(c, expire, &milliseconds, ((void *)0)) != 0)
            return;
        if (milliseconds <= 0) {
            addReplyErrorFormat(c,"invalid expire time in %s",c->cmd->name);
            return;
        }
        if (unit == 0) milliseconds *= 1000;
    }

    if ((flags & (1<<0) && lookupKeyWrite(c->db,key) != ((void *)0)) ||
        (flags & (1<<1) && lookupKeyWrite(c->db,key) == ((void *)0)))
    {
    ...
    notifyKeyspaceEvent((1<<3),"set",key,c->db->id);
    if (expire) notifyKeyspaceEvent((1<<2),
        "expire",key,c->db->id);
...
void setCommand(client *c) {
    ...
    for (j = 3; j < c->argc; j++) {
        char *a = c->argv[j]->ptr;
        robj *next = (j == c->argc-1) ? ((void *)0) : c->argv[j+1];
    ...
    setGenericCommand(c,flags,c->argv[1],c->argv[2],expire,unit,((void *)0),((void *)0));
}

void setnxCommand(client *c) {
    c->argv[2] = tryObjectEncoding(c->argv[2]);
    setGenericCommand(c,(1<<0),c->argv[1],c->argv[2],((void *)0),0,shared.cone,shared.czero);
}

void setexCommand(client *c) {
    c->argv[3] = tryObjectEncoding(c->argv[3]);
    setGenericCommand(c,0,c->argv[1],c->argv[3],c->argv[2],0,((void *)0),((void *)0));
}

void psetexCommand(client *c) {
    c->argv[3] = tryObjectEncoding(c->argv[3]);
    setGenericCommand(c,0,c->argv[1],c->argv[3],c->argv[2],1,((void *)0),((void *)0));
}
...
void getsetCommand(client *c) {
    if (getGenericCommand(c) == -1) return;
    c->argv[2] = tryObjectEncoding(c->argv[2]);
    setKey(c->db,c->argv[1],c->argv[2]);
    notifyKeyspaceEvent((1<<3),"set",c->argv[1],c->db->id);
    server.dirty++;
}
...
void setrangeCommand(client *c) {
    ...
    if (getLongFromObjectOrReply(c,c->argv[2],&offset,((void *)0)) != 0)
        return;
    ...
    if (o == ((void *)0)) {
        ...
        if (checkStringLength(c,offset+sdslen(value)) != 0)
            return;

        o = createObject(0,sdsnewlen(((void *)0), offset+sdslen(value)));
...
void getrangeCommand(client *c) {
    ...
    if (getLongLongFromObjectOrReply(c,c->argv[2],&start,((void *)0)) != 0)
        return;
    if (getLongLongFromObjectOrReply(c,c->argv[3],&end,((void *)0)) != 0)
        return;
    if ((o = lookupKeyReadOrReply(c,c->argv[1],shared.emptybulk)) == ((void *)0) ||
        checkType(c,o,0)) return;
...
void msetGenericCommand(client *c, int nx) {
    ...
    for (j = 1; j < c->argc; j += 2) {
        ...
        notifyKeyspaceEvent((1<<3),"set",c->argv[j],c->db->id);
    }
...
void incrDecrCommand(client *c, long long incr) {
    ...
    o = lookupKeyWrite(c->db,c->argv[1]);
    if (o != ((void *)0) && checkType(c,o,0)) return;
    if (getLongLongFromObjectOrReply(c,o,&value,((void *)0)) != 0) return;
    ...
    if (o && o->refcount == 1 && o->encoding == 1 &&
        (value < 0 || value >= 10000) &&
        value >= (-0x7fffffffffffffffL - 1L) && value <= 0x7fffffffffffffffL)
    {
        new = o;
    ...
    signalModifiedKey(c->db,c->argv[1]);
    notifyKeyspaceEvent((1<<3),"incrby",c->argv[1],c->db->id);
    server.dirty++;
...
void incrbyCommand(client *c) {
    ...
    if (getLongLongFromObjectOrReply(c, c->argv[2], &incr, ((void *)0)) != 0) return;
...
void incrbyfloatCommand(client *c) {
    ...
    o = lookupKeyWrite(c->db,c->argv[1]);
    if (o != ((void *)0) && checkType(c,o,0)) return;
    if (getLongDoubleFromObjectOrReply(c,o,&value,((void *)0)) != 0 ||
        getLongDoubleFromObjectOrReply(c,c->argv[2],&incr,((void *)0)) != 0)
        return;
    ...
    if (o)
        ...
        dbAdd(c->db,c->argv[1],new);
    signalModifiedKey(c->db,c->argv[1]);
    notifyKeyspaceEvent((1<<3),"incrbyfloat",c->argv[1],c->db->id);
    server.dirty++;
    addReplyBulk(c,new);
...
void appendCommand(client *c) {
    ...
    if (o == ((void *)0)) {
        ...
        if (checkStringLength(c,totlen) != 0)
            return;
    ...
    signalModifiedKey(c->db,c->argv[1]);
    notifyKeyspaceEvent((1<<3),"append",c->argv[1],c->db->id);
    server.dirty++;
