...
#define SRI_MASTER_DOWN (1<<5) #define SRI_FAILOVER_IN_PROGRESS (1<<6) #define SRI_PROMOTED (1<<7)             
...
#define SENTINEL_SCRIPT_MAX_RUNTIME 60000  
...
struct redisCommand sentinelcmds[] = {
    ...
    {"publish",sentinelPublishCommand,3,"",0,((void *)0),0,0,0,0,0},
    {"info",sentinelInfoCommand,-1,"",0,((void *)0),0,0,0,0,0},
    {"role",sentinelRoleCommand,1,"ok-loading",0,((void *)0),0,0,0,0,0},
    {"client",clientCommand,-2,"read-only no-script",0,((void *)0),0,0,0,0,0},
    {"shutdown",shutdownCommand,-1,"",0,((void *)0),0,0,0,0,0},
    {"auth",authCommand,2,"no-script ok-loading ok-stale fast",0,((void *)0),0,0,0,0,0},
    {"hello",helloCommand,-2,"no-script fast",0,((void *)0),0,0,0,0,0}
};
...
void initSentinel(void) {
    ...
    dictEmpty(server.commands,((void *)0));
    for (j = 0; j < sizeof(sentinelcmds)/sizeof(sentinelcmds[0]); j++) {
        int retval;
        struct redisCommand *cmd = sentinelcmds+j;

        retval = dictAdd(server.commands, sdsnew(cmd->name), cmd);
        ((retval == 0)?(void)0 : (_serverAssert("retval == DICT_OK","sentinel.c",479),_exit(1)));

        
 
        if (populateCommandTableParseFlags(cmd,cmd->sflags) == -1)
            _serverPanic("sentinel.c",484,"Unsupported command flag"),_exit(1);
    }
    ...
    sentinel.announce_ip = ((void *)0);
    sentinel.announce_port = 0;
    sentinel.simfailure_flags = 0;
    sentinel.deny_scripts_reconfig = 1;
...
void sentinelIsRunning(void) {
    ...
    if (server.configfile == ((void *)0)) {
        ...
        exit(1);
    } else if (access(server.configfile,2) == -1) {
        serverLog(3,
    ...
    if (j == 40) {
        ...
        getRandomHexChars(sentinel.myid,40);
        sentinelFlushConfig();
    }

     
    serverLog(3,"Sentinel ID is %s", sentinel.myid);
...
sentinelAddr *createSentinelAddr(char *hostname, int port) {
    ...
    if (anetResolve(((void *)0),hostname,ip,sizeof(ip)) == -1) {
        errno = 2;
        return ((void *)0);
    }
...
void sentinelScheduleScriptExecution(char *path, ...) {
    ...
    (*(char* *)&(ap) = (char*)(&__builtin_va_alist));
    while(argc < 16) {
        argv[argc] = (*(char* *)(((*(char* *)&(ap)) += sizeof(char*)) - sizeof(char*)));
        if (!argv[argc]) break;
    ...
    if (((sentinel . scripts_queue)->len) > 256) {
        ...
        ((((sentinel . scripts_queue)->len) <= 256)?(void)0 : (_serverAssert("listLength(sentinel.scripts_queue) <= SENTINEL_SCRIPT_MAX_QUEUE","sentinel.c",737),_exit(1)));
...
void sentinelCollectTerminatedScripts(void) {
    ...
    while ((pid = wait3(&statloc,1,((void *)0))) > 0) {
        ...
        if (ln == ((void *)0)) {
            serverLog(3,"wait3() returned a pid (%ld) we can't find in our scripts execution queue!", (long)pid);
            continue;
        }
        sj = ln->value;
...
void sentinelPendingScriptsCommand(client *c) {
    ...
    addReplyArrayLen(c,((sentinel . scripts_queue)->len));
    listRewind(sentinel.scripts_queue,&li);
    while ((ln = listNext(&li)) != ((void *)0)) {
        sentinelScriptJob *sj = ln->value;
        int j = 0;
...
{
    ((link->refcount > 0)?(void)0 : (_serverAssert("link->refcount > 0","sentinel.c",1004),_exit(1)));
    link->refcount--;
    if (link->refcount != 0) {
        if (ri && ri->link->cc) {
...
int sentinelTryConnectionSharing(sentinelRedisInstance *ri) {
    ((ri->flags & (1<<2))?(void)0 : (_serverAssert("ri->flags & SRI_SENTINEL","sentinel.c",1047),_exit(1)));
    dictIterator *di;
    dictEntry *de;

    if (ri->runid == ((void *)0)) return -1;  
    if (ri->link->refcount > 1) return -1;  

    di = dictGetIterator(sentinel.masters);
    while((de = dictNext(di)) != ((void *)0)) {
        sentinelRedisInstance *master = ((de)->v . val), *match;
...
int sentinelUpdateSentinelAddressInAllMasters(sentinelRedisInstance *ri) {
    ((ri->flags & (1<<2))?(void)0 : (_serverAssert("ri->flags & SRI_SENTINEL","sentinel.c",1083),_exit(1)));
    dictIterator *di;
    dictEntry *de;
    int reconfigured = 0;
    ...
    dictReleaseIterator(di);
    if (reconfigured)
        sentinelEvent(2,"+sentinel-address-update", ri,
...
void sentinelLinkEstablishedCallback(const redisAsyncContext *c, int status) {
    if (status != 0) instanceLinkConnectionError(c);
}
...
sentinelRedisInstance *createSentinelRedisInstance(char *name, int flags, char *hostname, int port, int quorum, sentinelRedisInstance *master) {
    ...
    sentinelAddr *addr;
    dict *table = ((void *)0);
    char slavename[(46+32)], *sdsname;

    ((flags & ((1<<0)|(1<<1)|(1<<2)))?(void)0 : (_serverAssert("flags & (SRI_MASTER|SRI_SLAVE|SRI_SENTINEL)","sentinel.c",1179),_exit(1)));
    (((flags & (1<<0)) || master != ((void *)0))?(void)0 : (_serverAssert("(flags & SRI_MASTER) || master != NULL","sentinel.c",1180),_exit(1)));
...
void releaseSentinelRedisInstance(sentinelRedisInstance *ri) {
    ...
    if ((ri->flags & (1<<1)) && (ri->flags & (1<<7)) && ri->master)
                ...
                sentinelRedisInstance *ri, char *ip, int port)
{
    ...
    ((ri->flags & (1<<0))?(void)0 : (_serverAssert("ri->flags & SRI_MASTER","sentinel.c",1303),_exit(1)));
...
sentinelRedisInstance *getSentinelRedisInstanceByAddrAndRunID(dict *instances, char *ip, int port, char *runid) {
    ...
    ((ip || runid)?(void)0 : (_serverAssert("ip || runid","sentinel.c",1361),_exit(1)));    
...
void sentinelResetMaster(sentinelRedisInstance *ri, int flags) {
    ((ri->flags & (1<<0))?(void)0 : (_serverAssert("ri->flags & SRI_MASTER","sentinel.c",1429),_exit(1)));
    dictRelease(ri->slaves);
    ri->slaves = dictCreate(&instancesDictType,((void *)0));
    if (!(flags & (1<<0))) {
        dictRelease(ri->sentinels);
...
char *sentinelHandleConfiguration(char **argv, int argc) {
    ...
    if (!strcasecmp(argv[0],"monitor") && argc == 5) {
        ...
        memcpy(sentinel.myid,argv[1],40);
                ...
                !strcasecmp(argv[0],"known-replica")) && argc == 4)
    {
        ...
        ri = sentinelGetMasterByName(argv[1]);
        if (!ri) return "No such master with specified name.";
        if ((slave = createSentinelRedisInstance(((void *)0),(1<<1),argv[2],
        ...
        if (argc == 5) {  
            ...
            ri = sentinelGetMasterByName(argv[1]);
            if (!ri) return "No such master with specified name.";
            if ((si = createSentinelRedisInstance(argv[4],(1<<2),argv[2],
                        atoi(argv[3]), ri->quorum, ri)) == ((void *)0))
            {
        ...
        if (dictAdd(ri->renamed_commands,oldcmd,newcmd) != 0) {
            sdsfree(oldcmd);
            sdsfree(newcmd);
...
void rewriteConfigSentinelOption(struct rewriteConfigState *state) {
    ...
    while((de = dictNext(di)) != ((void *)0)) {
        ...
        if (master->down_after_period != 30000) {
        ...
        if (master->failover_timeout != (60*3*1000)) {
            line = sdscatprintf(sdsempty(),
...
void sentinelFlushConfig(void) {
    ...
    if ((fd = open(server.configfile,00)) == -1) goto werr;
    if (fsync(fd) == -1) goto werr;
    if (close(fd) == (-1)) goto werr;
    return;

werr:
    if (fd != -1) close(fd);
    serverLog(3,"WARNING: Sentinel was not able to save the new configuration on disk!!!: %s", strerror(errno));
}
...
void sentinelSendAuthIfNeeded(sentinelRedisInstance *ri, redisAsyncContext *c) {
    ...
    } else if (ri->flags & (1<<1)) {
        auth_pass = ri->master->auth_pass;
    } else if (ri->flags & (1<<2)) {
    ...
    if (auth_pass) {
        if (redisAsyncCommand(c, sentinelDiscardReplyCallback, ri, "%s %s",
            ...
            auth_pass) == 0) ri->link->pending_commands++;
    }
}
...
void sentinelReconnectInstance(sentinelRedisInstance *ri) {
    ...
    if (now - ri->link->last_reconn_time < 1000) return;
    ri->link->last_reconn_time = now;

     
    if (link->cc == ((void *)0)) {
        link->cc = redisAsyncConnectBind(ri->addr->ip,ri->addr->port,(server . bindaddr_count ? server . bindaddr[0] : ((void *)0)));
        if (link->cc->err) {
    ...
    if ((ri->flags & ((1<<0)|(1<<1))) && link->pc == ((void *)0)) {
        link->pc = redisAsyncConnectBind(ri->addr->ip,ri->addr->port,(server . bindaddr_count ? server . bindaddr[0] : ((void *)0)));
        if (link->pc->err) {
    ...
    if (link->cc && (ri->flags & (1<<2) || link->pc))
        link->disconnected = 0;
}
...
int sentinelMasterLooksSane(sentinelRedisInstance *master) {
    return
        master->flags & (1<<0) &&
        master->role_reported == (1<<0) &&
        (master->flags & ((1<<3)|(1<<4))) == 0 &&
        (mstime() - master->info_refresh) < 10000*2;
}
...
void sentinelRefreshInstanceInfo(sentinelRedisInstance *ri, const char *info) {
    ...
    if (role != ri->role_reported) {
        ...
        if (role == (1<<1)) ri->slave_conf_change_time = mstime();
        
 
        sentinelEvent(1,
            ((ri->flags & ((1<<0)|(1<<1))) == role) ?
            "+role-change" : "-role-change",
            ri, "%@ new reported role is %s",
            role == (1<<0) ? "master" : "slave",
            ri->flags & (1<<0) ? "master" : "slave");
    }
         ...
         strcasecmp(ri->slave_master_host,ri->master->addr->ip)))
    {
            ...
            mstime() - ri->slave_conf_change_time > wait_time)
        {
            int retval = sentinelSendSlaveOf(ri,
                    ...
                    ri->master->addr->port);
            if (retval == 0)
                sentinelEvent(2,"+fix-slave-config",ri,"%@");
        }
    }
            ...
            strncmp(r->str,"MASTERDOWN",10) == 0)
        {
                ...
                !(ri->flags & (1<<12)))
            {
                if (redisAsyncCommand(ri->link->cc,
                        ...
                        "%s KILL",
                        sentinelInstanceMapCommand(ri,"SCRIPT")) == 0)
                {
                    ri->link->pending_commands++;
                }
                ri->flags |= (1<<12);
...
void sentinelProcessHelloMessage(char *hello, int hello_len) {
    ...
    if (numtokens == 8) {
        ...
        current_epoch = strtoull(token[3],((void *)0),10);
        master_config_epoch = strtoull(token[7],((void *)0),10);
        ...
        if (!si) {
            ...
            if (removed) {
                sentinelEvent(2,"+sentinel-address-switch",master,
                ...
                sentinelRedisInstance *other =
                    getSentinelRedisInstanceByAddrAndRunID(
                        master->sentinels, token[0],port,((void *)0));
                if (other) {
            ...
            si = createSentinelRedisInstance(token[2],(1<<2),
                            token[0],port,master->quorum,master);

            if (si) {
                if (!removed) sentinelEvent(2,"+sentinel",si,"%@");
...
int sentinelSendHello(sentinelRedisInstance *ri) {
    ...
    char *announce_ip;
    int announce_port;
    sentinelRedisInstance *master = (ri->flags & (1<<0)) ? ri : ri->master;
    sentinelAddr *master_addr = sentinelGetCurrentMasterAddress(master);

    if (ri->link->disconnected) return -1;
    ...
    if (sentinel.announce_ip) {
        ...
        if (anetSockName(ri->link->cc->c.fd,ip,sizeof(ip),((void *)0)) == -1)
            return -1;
        announce_ip = ip;
    }
...
void sentinelForceHelloUpdateDictOfRedisInstances(dict *instances) {
    ...
    while((de = dictNext(di)) != ((void *)0)) {
        sentinelRedisInstance *ri = ((de)->v . val);
        if (ri->last_pub_time >= (2000+1))
            ri->last_pub_time -= (2000+1);
    }
    dictReleaseIterator(di);
}
...
int sentinelForceHelloUpdateForMaster(sentinelRedisInstance *master) {
    if (!(master->flags & (1<<0))) return -1;
    if (master->last_pub_time >= (2000+1))
        master->last_pub_time -= (2000+1);
...
void sentinelSendPeriodicCommands(sentinelRedisInstance *ri) {
    ...
    if (ping_period > 1000) ping_period = 1000;
        ...
        (now - ri->info_refresh) > info_period))
    {
        ...
        if (retval == 0) ri->link->pending_commands++;
    }
...
void addReplySentinelRedisInstance(client *c, sentinelRedisInstance *ri) {
    ...
    addReplyBulkCString(c,"flags");
    if (ri->flags & (1<<3)) flags = sdscat(flags,"s_down,");
    if (ri->flags & (1<<4)) flags = sdscat(flags,"o_down,");
    ...
    if (ri->flags & (1<<5)) flags = sdscat(flags,"master_down,");
    if (ri->flags & (1<<6))
        flags = sdscat(flags,"failover_in_progress,");
    if (ri->flags & (1<<7)) flags = sdscat(flags,"promoted,");
    if (ri->flags & (1<<8)) flags = sdscat(flags,"reconf_sent,");
    if (ri->flags & (1<<9)) flags = sdscat(flags,"reconf_inprog,");
    if (ri->flags & (1<<10)) flags = sdscat(flags,"reconf_done,");
...
void addReplyDictOfRedisInstances(client *c, dict *instances) {
    ...
    di = dictGetIterator(instances);
    addReplyArrayLen(c,((instances)->ht[0]. used+(instances)->ht[1]. used));
    while((de = dictNext(di)) != ((void *)0)) {
        sentinelRedisInstance *ri = ((de)->v . val);
...
int sentinelIsQuorumReachable(sentinelRedisInstance *master, int *usableptr) {
    ...
    int usable = 1;  
    int result = 0;
    int voters = ((master->sentinels)->ht[0]. used+(master->sentinels)->ht[1]. used)+1;  

    di = dictGetIterator(master->sentinels);
    while((de = dictNext(di)) != ((void *)0)) {
        sentinelRedisInstance *ri = ((de)->v . val);

        if (ri->flags & ((1<<3)|(1<<4))) continue;
        usable++;
    }
    dictReleaseIterator(di);

    if (usable < (int)master->quorum) result |= (1<<0);
    if (usable < voters/2+1) result |= (1<<1);
...
void sentinelCommand(client *c) {
               ...
               !strcasecmp(c->argv[1]->ptr,"replicas"))
    {
        ...
        if (!sentinel.tilt && ri && (ri->flags & (1<<3)) &&
                                    (ri->flags & (1<<0)))
            isdown = 1;
        ...
        serverLog(3,"Executing user requested FAILOVER of '%s'",
            ri->name);
        ...
        if (anetResolveIP(((void *)0),c->argv[3]->ptr,ip,sizeof(ip)) == -1) {
            addReplyError(c,"Invalid IP address specified");
            return;
        }

         
        ri = createSentinelRedisInstance(c->argv[2]->ptr,(1<<0),
                c->argv[3]->ptr,port,quorum,((void *)0));
        if (ri == ((void *)0)) {
            switch(errno) {
            case 16:
        ...
        copy_keeper.valDestructor = ((void *)0);
        dict *masters_local = sentinel.masters;
        if (c->argc > 2) {
            masters_local = dictCreate(&copy_keeper, ((void *)0));
        ...
        addReplyArrayLen(c,((masters_local)->ht[0]. used+(masters_local)->ht[1]. used) * 2);
        ...
        while ((de = dictNext(di)) != ((void *)0)) {
            ...
            addReplyArrayLen(c,((ri->slaves)->ht[0]. used+(ri->slaves)->ht[1]. used) + 1);  
            addReplyArrayLen(c,2);
            ...
            while ((sde = dictNext(sdi)) != ((void *)0)) {
                sentinelRedisInstance *sri = ((sde)->v . val);
                addReplyArrayLen(c,2);
        ...
        for (j = 2; j < c->argc; j++) {
            if (!strcasecmp(c->argv[j]->ptr,"crash-after-election")) {
                sentinel.simfailure_flags |=
                    (1<<0);
                serverLog(3,"Failure simulation: this Sentinel "
...
void sentinelInfoCommand(client *c) {
    ...
    char *section = c->argc == 2 ? c->argv[1]->ptr : ((void *)0);
    if (section) {
    ...
    if (defsections || allsections || !strcasecmp(section,"sentinel")) {
        ...
        while((de = dictNext(di)) != ((void *)0)) {
            ...
            if (ri->flags & (1<<4)) status = "odown";
            else if (ri->flags & (1<<3)) status = "sdown";
            info = sdscatprintf(info,
...
void sentinelRoleCommand(client *c) {
    ...
    addReplyArrayLen(c,((sentinel . masters)->ht[0]. used+(sentinel . masters)->ht[1]. used));
...
void sentinelSetCommand(client *c) {
    ...
    for (j = 3; j < c->argc; j++) {
        ...
        if (!strcasecmp(option,"down-after-milliseconds") && moreargs > 0) {
            ...
            if (strlen(value) && access(value,1) == -1) {
                addReplyError(c,
        ...
        switch(numargs) {
        case 2:
            sentinelEvent(3,"+set",ri,"%@ %s %s",c->argv[old_j]->ptr,
                                                          c->argv[old_j+1]->ptr);
            break;
        case 3:
            sentinelEvent(3,"+set",ri,"%@ %s %s %s",c->argv[old_j]->ptr,
                                                             c->argv[old_j+1]->ptr,
                                                             c->argv[old_j+2]->ptr);
            break;
        default:
            sentinelEvent(3,"+set",ri,"%@ %s",c->argv[old_j]->ptr);
            break;
        }
    }
...
void sentinelPublishCommand(client *c) {
    if (strcmp(c->argv[1]->ptr,"__sentinel__:hello")) {
...
void sentinelCheckSubjectivelyDown(sentinelRedisInstance *ri) {
    ...
    if (ri->link->pc &&
        (mstime() - ri->link->pc_conn_time) >
         15000 &&
        (mstime() - ri->link->pc_last_activity) > (2000*3))
    {
        instanceLinkCloseConnection(ri->link,ri->link->pc);
    }
...
void sentinelAskMasterStateToOtherSentinels(sentinelRedisInstance *master, int flags) {
    ...
    while((de = dictNext(di)) != ((void *)0)) {
        ...
        if ((master->flags & (1<<3)) == 0) continue;
...
char *sentinelVoteLeader(sentinelRedisInstance *master, uint64_t req_epoch, char *req_runid, uint64_t *leader_epoch) {
    ...
    if (master->leader_epoch < req_epoch && sentinel.current_epoch <= req_epoch)
    {
        ...
        if (strcasecmp(master->leader,sentinel.myid))
            master->failover_start_time = mstime()+rand()%1000;
    }
...
int sentinelLeaderIncr(dict *counters, char *runid) {
    ...
    if (existing) {
        ...
        do { (existing)->v . u64 = oldval+1; } while(0);
        return oldval+1;
    } else {
        ((de != ((void *)0))?(void)0 : (_serverAssert("de != NULL","sentinel.c",3833),_exit(1)));
...
char *sentinelGetLeader(sentinelRedisInstance *master, uint64_t epoch) {
    ...
    ((master->flags & ((1<<4)|(1<<6)))?(void)0 : (_serverAssert("master->flags & (SRI_O_DOWN|SRI_FAILOVER_IN_PROGRESS)","sentinel.c",3855),_exit(1)));
    counters = dictCreate(&leaderVotesDictType,((void *)0));

    voters = ((master->sentinels)->ht[0]. used+(master->sentinels)->ht[1]. used)+1;  
...
void sentinelStartFailover(sentinelRedisInstance *master) {
    ((master->flags & (1<<0))?(void)0 : (_serverAssert("master->flags & SRI_MASTER","sentinel.c",3984),_exit(1)));
...
int sentinelStartFailoverIfNeeded(sentinelRedisInstance *master) {
    ...
    if (!(master->flags & (1<<4))) return 0;

     
    if (master->flags & (1<<6)) return 0;
...
sentinelRedisInstance *sentinelSelectSlave(sentinelRedisInstance *master) {
    ...
    while((de = dictNext(di)) != ((void *)0)) {
        ...
        if (slave->flags & ((1<<3)|(1<<4))) continue;
        if (slave->link->disconnected) continue;
        if (mstime() - slave->link->last_avail_time > 1000*5) continue;
        ...
        if (master->flags & (1<<3))
            info_validity_time = 1000*5;
        else
            info_validity_time = 10000*3;
...
void sentinelFailoverWaitStart(sentinelRedisInstance *ri) {
    ...
    if (!isleader && !(ri->flags & (1<<11))) {
        int election_timeout = 10000;
    ...
    if (sentinel.simfailure_flags & (1<<0))
        sentinelSimFailureCrash();
    ri->failover_state = 2;
...
void sentinelFailoverSendSlaveOfNoOne(sentinelRedisInstance *ri) {
    ...
    if (ri->promoted_slave->link->disconnected) {
        if (mstime() - ri->failover_state_change_time > ri->failover_timeout) {
            sentinelEvent(3,"-failover-abort-slave-timeout",ri,"%@");
            sentinelAbortFailover(ri);
        }
        return;
    }
...
void sentinelFailoverDetectEnd(sentinelRedisInstance *master) {
    ...
    if (master->promoted_slave == ((void *)0) ||
        master->promoted_slave->flags & (1<<3)) return;
    ...
    while((de = dictNext(di)) != ((void *)0)) {
        ...
        if (slave->flags & ((1<<7)|(1<<10))) continue;
        if (slave->flags & (1<<3)) continue;
        not_reconfigured++;
    }
    dictReleaseIterator(di);
    ...
    if (timeout) {
        ...
        while((de = dictNext(di)) != ((void *)0)) {
            ...
            if (slave->flags & ((1<<10)|(1<<8))) continue;
            if (slave->link->disconnected) continue;
...
void sentinelFailoverReconfNextSlave(sentinelRedisInstance *master) {
          ...
          (de = dictNext(di)) != ((void *)0))
    {
        ...
        if (slave->flags & ((1<<8)|(1<<9))) continue;
        if (slave->link->disconnected) continue;
...
void sentinelFailoverStateMachine(sentinelRedisInstance *ri) {
    ((ri->flags & (1<<0))?(void)0 : (_serverAssert("ri->flags & SRI_MASTER","sentinel.c",4370),_exit(1)));

    if (!(ri->flags & (1<<6))) return;
...
void sentinelAbortFailover(sentinelRedisInstance *ri) {
    ((ri->flags & (1<<6))?(void)0 : (_serverAssert("ri->flags & SRI_FAILOVER_IN_PROGRESS","sentinel.c",4399),_exit(1)));
    ((ri->failover_state <= 4)?(void)0 : (_serverAssert("ri->failover_state <= SENTINEL_FAILOVER_STATE_WAIT_PROMOTION","sentinel.c",4400),_exit(1)));

    ri->flags &= ~((1<<6)|(1<<11));
    ri->failover_state = 0;
    ri->failover_state_change_time = mstime();
    if (ri->promoted_slave) {
        ri->promoted_slave->flags &= ~(1<<7);
        ri->promoted_slave = ((void *)0);
    }
}
