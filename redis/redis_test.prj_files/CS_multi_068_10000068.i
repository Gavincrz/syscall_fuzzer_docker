...
void execCommand(client *c) {
    ...
    if (c->flags & ((1<<5)|(1<<12))) {
        addReply(c, c->flags & (1<<12) ? shared.execaborterr :
    ...
    if (!server.loading && server.masterhost && server.repl_slave_ro &&
        !(c->flags & (1<<1)) && c->mstate.cmd_flags & (1ULL<<0))
    {
        addReplyError(c,
    ...
    for (j = 0; j < c->mstate.count; j++) {
        ...
        if (!must_propagate && !(c->cmd->flags & ((1ULL<<1)|(1ULL<<4)))) {
            execCommandPropagateMulti(c);
            must_propagate = 1;
        }

        call(c,server.loading ? 0 : ((1<<0) | (1<<1) | ((1<<2)|(1<<3))));
...
void unwatchAllKeys(client *c) {
    ...
    if (((c->watched_keys)->len) == 0) return;
    listRewind(c->watched_keys,&li);
    while((ln = listNext(&li))) {
        list *clients;
        watchedKey *wk;
        ...
        ((clients != ((void *)0))?(void)0 : (_serverAssertWithInfo(c,((void *)0),"clients != NULL","multi.c",279),_exit(1)));
...
void touchWatchedKey(redisDb *db, robj *key) {
    ...
    if (((db->watched_keys)->ht[0]. used+(db->watched_keys)->ht[1]. used) == 0) return;
