...
void aofRewriteBufferReset(void) {
    ...
    ((server . aof_rewrite_buf_blocks)->free = (zfree));
}
...
void aofChildWriteDiffData(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    while(1) {
        ln = ((server . aof_rewrite_buf_blocks)->head);
...
void aofRewriteBufferAppend(unsigned char *s, unsigned long len) {
    listNode *ln = ((server . aof_rewrite_buf_blocks)->tail);
    ...
    while(len) {
        ...
        if (len) {  
            ...
            block->free = (1024*1024*10);
            block->used = 0;
            listAddNodeTail(server.aof_rewrite_buf_blocks,block);

            
 
            numblocks = ((server . aof_rewrite_buf_blocks)->len);
            if (((numblocks+1) % 10) == 0) {
                int level = ((numblocks+1) % 100) == 0 ? 3 :
                                                         2;
    ...
    if (aeGetFileEvents(server.el,server.aof_pipe_write_data_to_child) == 0) {
        aeCreateFileEvent(server.el, server.aof_pipe_write_data_to_child,
            2, aofChildWriteDiffData, ((void *)0));
    }
}
...
int aofFsyncInProgress(void) {
    return bioPendingJobsOfType(1) != 0;
}


 
void aof_background_fsync(int fd) {
    bioCreateBackgroundJob(1,(void*)(long)fd,((void *)0),((void *)0));
}
...
void killAppendOnlyChild(void) {
    ...
    serverLog(2,"Killing running AOF rewrite child: %ld",
        (long) server.aof_child_pid);
    if (kill(server.aof_child_pid,10) != -1) {
...
void stopAppendOnly(void) {
    ((server . aof_state != 0)?(void)0 : (_serverAssert("server.aof_state != AOF_OFF","aof.c",237),_exit(1)));
...
int startAppendOnly(void) {
    ...
    newfd = open(server.aof_filename,01|02000|0100,0644);
    ((server . aof_state == 0)?(void)0 : (_serverAssert("server.aof_state == AOF_OFF","aof.c",255),_exit(1)));
    if (newfd == -1) {
    ...
    if (server.rdb_child_pid != -1) {
        ...
        serverLog(3,"AOF was enabled but there is already a child process saving an RDB file on disk. An AOF background was scheduled to start when possible.");
    } else {
        

 
        if (server.aof_child_pid != -1) {
            serverLog(3,"AOF was enabled but there is already an AOF rewriting in background. Stopping background AOF and starting a rewrite now.");
            killAppendOnlyChild();
        }
        if (rewriteAppendOnlyFileBackground() == -1) {
            close(newfd);
            serverLog(3,"Redis needs to enable the AOF but can't trigger a background AOF rewrite operation. Check the above logs for more info about the error.");
            return -1;
        }
    }
...
void flushAppendOnlyFile(int force) {
    ...
    if (server.aof_fsync == 2 && !force) {
        ...
        if (sync_in_progress) {
            ...
            serverLog(2,"Asynchronous AOF fsync is taking too long (disk is busy?). Writing the AOF buffer without waiting for fsync to complete, this may slow down Redis.");
        }
    }
    ...
    if (sync_in_progress) {
        if (server . latency_monitor_threshold && (latency) >= server . latency_monitor_threshold) latencyAddSample(("aof-write-pending-fsync"),(latency));;
    } else if (server.aof_child_pid != -1 || server.rdb_child_pid != -1) {
        if (server . latency_monitor_threshold && (latency) >= server . latency_monitor_threshold) latencyAddSample(("aof-write-active-child"),(latency));;
    } else {
        if (server . latency_monitor_threshold && (latency) >= server . latency_monitor_threshold) latencyAddSample(("aof-write-alone"),(latency));;
    }
    if (server . latency_monitor_threshold && (latency) >= server . latency_monitor_threshold) latencyAddSample(("aof-write"),(latency));;
    ...
    if (nwritten != (ssize_t)sdslen(server.aof_buf)) {
        ...
        if ((server.unixtime - last_write_error_log) > 30) {
            can_log = 1;
        ...
        if (nwritten == -1) {
            if (can_log) {
                serverLog(3,"Error writing to the AOF file: %s",
                    strerror(errno));
                server.aof_last_write_errno = errno;
            }
        } else {
            if (can_log) {
                serverLog(3,"Short write while writing to "
            ...
            if (ftruncate(server.aof_fd, server.aof_current_size) == -1) {
                if (can_log) {
                    serverLog(3, "Could not remove short write "
            ...
            server.aof_last_write_errno = 28;
        }
        ...
        if (server.aof_fsync == 1) {
            ...
            serverLog(3,"Can't recover from AOF write error when the AOF fsync policy is 'always'. Exiting...");
            exit(1);
        } else {
...
try_fsync:
    ...
    if (server.aof_fsync == 1) {
        ...
        fdatasync(server.aof_fd);  
        if (server . latency_monitor_threshold) { latency = mstime() - latency; };
        if (server . latency_monitor_threshold && (latency) >= server . latency_monitor_threshold) latencyAddSample(("aof-fsync-always"),(latency));;
        server.aof_fsync_offset = server.aof_current_size;
        server.aof_last_fsync = server.unixtime;
    } else if ((server.aof_fsync == 2 &&
                server.unixtime > server.aof_last_fsync)) {
        if (!sync_in_progress) {
...
void feedAppendOnlyFile(struct redisCommand *cmd, int dictid, robj **argv, int argc) {
    ...
    if (cmd->proc == expireCommand || cmd->proc == pexpireCommand ||
        ...
        ((!(exarg && pxarg))?(void)0 : (_serverAssert("!(exarg && pxarg)","aof.c",618),_exit(1)));
        if (exarg)
...
int loadAppendOnlyFile(char *filename) {
    ...
    if (fp == ((void *)0)) {
        serverLog(3,"Fatal error: can't open the append log file for reading: %s",strerror(errno));
        exit(1);
    }
    ...
    if (fp && fstat(fileno(fp),&sb) != -1 && sb.st_size == 0) {
    ...
    if (fread(sig,1,5,fp) != 5 || memcmp(sig,"REDIS",5) != 0) {
        ...
        if (fseek(fp,0,0) == -1) goto readerr;
    } else {
         
        rio rdb;

        serverLog(2,"Reading RDB preamble from AOF file...");
        if (fseek(fp,0,0) == -1) goto readerr;
        rioInitWithFile(&rdb,fp);
        if (rdbLoadRio(&rdb,((void *)0),1) != 0) {
            serverLog(3,"Error reading the RDB preamble of the AOF file, AOF loading aborted");
            goto readerr;
        } else {
            serverLog(2,"Reading the remaining AOF tail...");
        }
    }
    ...
    while(1) {
        ...
        ((fakeClient->bufpos == 0 && ((fakeClient->reply)->len) == 0)?(void)0 : (_serverAssert("fakeClient->bufpos == 0 && listLength(fakeClient->reply) == 0","aof.c",831),_exit(1)));


         
        (((fakeClient->flags & (1<<4)) == 0)?(void)0 : (_serverAssert("(fakeClient->flags & CLIENT_BLOCKED) == 0","aof.c",834),_exit(1)));

        
 
        freeFakeClientArgv(fakeClient);
        fakeClient->cmd = ((void *)0);
...
readerr:  
    if (!feof(fp)) {
        ...
        serverLog(3,"Unrecoverable error reading the append only file: %s", strerror(errno));
        exit(1);
    }

uxeof:  
    if (server.aof_load_truncated) {
        serverLog(3,"!!! Warning: short read while loading the AOF file !!!");
        serverLog(3,"!!! Truncating the AOF at offset %llu !!!",
            (unsigned long long) valid_up_to);
        if (valid_up_to == -1 || truncate(filename,valid_up_to) == -1) {
            if (valid_up_to == -1) {
                serverLog(3,"Last valid command offset is invalid");
            } else {
                serverLog(3,"Error truncating the AOF file: %s",
                    strerror(errno));
            }
        } else {
            
 
            if (server.aof_fd != -1 && lseek(server.aof_fd,0,2) == -1) {
                serverLog(3,"Can't seek the end of the AOF file: %s",
                    strerror(errno));
            } else {
                serverLog(3,
    ...
    if (fakeClient) freeFakeClient(fakeClient);  
    serverLog(3,"Unexpected end of file reading the append only file. You can: 1) Make a backup of your AOF file, then use ./redis-check-aof --fix <filename>. 2) Alternatively you can set the 'aof-load-truncated' configuration option to yes and restart the server.");
    exit(1);

fmterr:  
    if (fakeClient) freeFakeClient(fakeClient);  
    serverLog(3,"Bad file format reading the append only file: make a backup of your AOF file, then use ./redis-check-aof --fix <filename>");
    exit(1);
}
...
int rioWriteBulkObject(rio *r, robj *obj) {
    ...
    } else if ((obj->encoding == 0 || obj->encoding == 8)) {
        return rioWriteBulkString(r,obj->ptr,sdslen(obj->ptr));
    } else {
        _serverPanic("aof.c",920,"Unknown string encoding"),_exit(1);
    }
}
...
int rewriteListObject(rio *r, robj *key, robj *o) {
    ...
    if (o->encoding == 9) {
        ...
        quicklistIter *li = quicklistGetIterator(list, 0);
        quicklistEntry entry;

        while (quicklistNext(li,&entry)) {
            if (count == 0) {
                int cmd_items = (items > 64) ?
                    64 : items;
        ...
        _serverPanic("aof.c",953,"Unknown list encoding"),_exit(1);
    }
    return 1;
}
...
int rewriteSetObject(rio *r, robj *key, robj *o) {
    ...
    if (o->encoding == 6) {
        ...
        _serverPanic("aof.c",1000,"Unknown set encoding"),_exit(1);
    }
    return 1;
}
...
int rewriteSortedSetObject(rio *r, robj *key, robj *o) {
    ...
    if (o->encoding == 5) {
        ...
        ((eptr != ((void *)0))?(void)0 : (_serverAssert("eptr != NULL","aof.c",1019),_exit(1)));
        sptr = ziplistNext(zl,eptr);
        ((sptr != ((void *)0))?(void)0 : (_serverAssert("sptr != NULL","aof.c",1021),_exit(1)));

        while (eptr != ((void *)0)) {
            ((ziplistGet(eptr,&vstr,&vlen,&vll))?(void)0 : (_serverAssert("ziplistGet(eptr,&vstr,&vlen,&vll)","aof.c",1024),_exit(1)));
        ...
        _serverPanic("aof.c",1069,"Unknown sorted zset encoding"),_exit(1);
    }
    return 1;
}
...
static int rioWriteHashIteratorCursor(rio *r, hashTypeIterator *hi, int what) {
    if (hi->encoding == 5) {
        ...
        unsigned int vlen = (0x7fffffff * 2U + 1U);
        long long vll = 0x7fffffffffffffffLL;
    ...
    } else if (hi->encoding == 2) {
        sds value = hashTypeCurrentFromHashTable(hi, what);
        return rioWriteBulkString(r, value, sdslen(value));
    }

    _serverPanic("aof.c",1096,"Unknown hash encoding"),_exit(1);
    return 0;
}
...
int rewriteHashObject(rio *r, robj *key, robj *o) {
    ...
    while (hashTypeNext(hi) != -1) {
        ...
        if (rioWriteHashIteratorCursor(r, hi, 1) == 0) return 0;
        if (rioWriteHashIteratorCursor(r, hi, 2) == 0) return 0;
        if (++count == 64) count = 0;
        items--;
    }
...
int rewriteModuleObject(rio *r, robj *key, robj *o) {
    ...
    do { io . rio = r; io . type = mt; io . bytes = 0; io . error = 0; io . ver = 0; io . key = key; io . ctx = ((void *)0); } while(0);;
    ...
    for (j = 0; j < server.dbnum; j++) {
        ...
        while((de = dictNext(di)) != ((void *)0)) {
            ...
            if (o->type == 0) {
                ...
                _serverPanic("aof.c",1343,"Unknown object type"),_exit(1);
            }
...
int rewriteAppendOnlyFile(char *filename) {
    ...
    fp = fopen(tmpfile,"w");
    if (!fp) {
        serverLog(3, "Opening the temp file for AOF rewrite in rewriteAppendOnlyFile(): %s", strerror(errno));
        return -1;
    }
    ...
    if (server.aof_rewrite_incremental_fsync)
        rioSetAutoSync(&aof,(1024*1024*32));

    if (server.aof_use_rdb_preamble) {
        int error;
        if (rdbSaveRio(&aof,&error,(1<<0),((void *)0)) == -1) {
            errno = error;
            goto werr;
        }
    } else {
        if (rewriteAppendOnlyFileRio(&aof) == -1) goto werr;
    }
    ...
    while(mstime()-start < 1000 && nodata < 20) {
        if (aeWait(server.aof_pipe_read_data_from_parent, 1, 1) <= 0)
        {
            nodata++;
            continue;
        }
    ...
    if (anetNonBlock(((void *)0),server.aof_pipe_read_ack_from_parent) != 0)
        goto werr;
    ...
    if (syncRead(server.aof_pipe_read_ack_from_parent,&byte,1,5000) != 1 ||
        byte != '!') goto werr;
    serverLog(2,"Parent agreed to stop sending diffs. Finalizing AOF...");
    ...
    if (rename(tmpfile,filename) == -1) {
        serverLog(3,"Error moving temp append only file on the final destination: %s", strerror(errno));
        unlink(tmpfile);
        return -1;
    }
    serverLog(2,"SYNC append only file rewrite performed");
    return 0;

werr:
    serverLog(3,"Write error writing append only file on disk: %s", strerror(errno));
    fclose(fp);
    unlink(tmpfile);
    return -1;
}
...
void aofChildPipeReadable(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    if (read(fd,&byte,1) == 1 && byte == '!') {
        serverLog(2,"AOF rewrite child asks to stop sending diffs.");
        ...
        if (write(server.aof_pipe_write_ack_to_child,"!",1) != 1) {
            ...
            serverLog(3,"Can't send ACK to AOF child: %s",
                strerror(errno));
        }
    }
    
 
    aeDeleteFileEvent(server.el,server.aof_pipe_read_ack_from_child,1);
}
...
int aofCreatePipes(void) {
    ...
    if (anetNonBlock(((void *)0),fds[0]) != 0) goto error;
    if (anetNonBlock(((void *)0),fds[1]) != 0) goto error;
    if (aeCreateFileEvent(server.el, fds[2], 1, aofChildPipeReadable, ((void *)0)) == -1) goto error;
...
error:
    serverLog(3,"Error opening /setting AOF rewrite IPC pipes: %s",
        strerror(errno));
...
void aofClosePipes(void) {
    aeDeleteFileEvent(server.el,server.aof_pipe_read_ack_from_child,1);
    aeDeleteFileEvent(server.el,server.aof_pipe_write_data_to_child,2);
...
int rewriteAppendOnlyFileBackground(void) {
    ...
    if ((childpid = fork()) == 0) {
        ...
        if (rewriteAppendOnlyFile(tmpfile) == 0) {
...
void bgrewriteaofCommand(client *c) {
    ...
    } else if (rewriteAppendOnlyFileBackground() == 0) {
...
void aofUpdateCurrentSize(void) {
    ...
    if (server . latency_monitor_threshold) { latency = mstime(); } else { latency = 0; };
    if (fstat(server.aof_fd,&sb) == -1) {
        serverLog(3,"Unable to obtain the AOF file length. stat: %s",
            strerror(errno));
    } else {
        server.aof_current_size = sb.st_size;
    }
    if (server . latency_monitor_threshold) { latency = mstime() - latency; };
    if (server . latency_monitor_threshold && (latency) >= server . latency_monitor_threshold) latencyAddSample(("aof-fstat"),(latency));;
}
...
void backgroundRewriteDoneHandler(int exitcode, int bysignal) {
    if (!bysignal && exitcode == 0) {
        ...
        if (server . latency_monitor_threshold) { latency = mstime() - latency; };
        if (server . latency_monitor_threshold && (latency) >= server . latency_monitor_threshold) latencyAddSample(("aof-rewrite-diff-write"),(latency));;
        ...
        if (server.aof_fd == -1) {
            ...
            oldfd = open(server.aof_filename,00|04000);
        } else {
        ...
        if (server . latency_monitor_threshold) { latency = mstime() - latency; };
        if (server . latency_monitor_threshold && (latency) >= server . latency_monitor_threshold) latencyAddSample(("aof-rename"),(latency));;
        ...
        serverLog(2, "Background AOF rewrite finished successfully");
         
        if (server.aof_state == 2)
            server.aof_state = 1;

         
        if (oldfd != -1) bioCreateBackgroundJob(0,(void*)(long)oldfd,((void *)0),((void *)0));
