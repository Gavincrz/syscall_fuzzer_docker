...
# 41 "cluster.c"
# 1 "/usr/include/x86_64-linux-gnu/sys/file.h"
...
int clusterLoadConfig(char *filename) {
        ...
        *p = '\0';
        ...
        while(p) {
            ...
            if (!strcasecmp(s,"myself")) {
                ((server . cluster->myself == ((void *)0))?(void)0 : (_serverAssert("server.cluster->myself == NULL","cluster.c",189),_exit(1)));
                ...
                _serverPanic("cluster.c",210,"Unknown flag in redis cluster config file"),_exit(1);
            }
            if (p) s = p+1;
        }
        ...
        for (j = 8; j < argc; j++) {
            ...
            if (argv[j][0] == '[') {
                ...
                p = strchr(argv[j],'-');
                ((p != ((void *)0))?(void)0 : (_serverAssert("p != NULL","cluster.c",245),_exit(1)));
                *p = '\0';
                direction = p[1];  
                slot = atoi(argv[j]+1);
                if (slot < 0 || slot >= 16384) goto fmterr;
                p += 3;
            ...
            if (start < 0 || start >= 16384) goto fmterr;
            if (stop < 0 || stop >= 16384) goto fmterr;
    ...
    serverLog(2,"Node configuration loaded, I'm %.40s", myself->name);
...
int clusterSaveConfig(int do_fsync) {
    ...
    ci = clusterGenNodesDescription(32);
    ci = sdscatprintf(ci,"vars currentEpoch %llu lastVoteEpoch %llu\n",
        (unsigned long long) server.cluster->currentEpoch,
        (unsigned long long) server.cluster->lastVoteEpoch);
    content_size = sdslen(ci);

    if ((fd = open(server.cluster_configfile,01|0100,0644))
        == -1) goto err;
...
void clusterSaveConfigOrDie(int do_fsync) {
    if (clusterSaveConfig(do_fsync) == -1) {
        serverLog(3,"Fatal: can't update cluster config file.");
        exit(1);
    }
}
...
int clusterLockConfig(char *filename) {
    ...
    int fd = open(filename,01|0100,0644);
    if (fd == -1) {
        serverLog(3,
...
void clusterInit(void) {
    ...
    server.cluster->cant_failover_reason = 0;
    ...
    if (clusterLockConfig(server.cluster_configfile) == -1)
        exit(1);

     
    if (clusterLoadConfig(server.cluster_configfile) == -1) {
        
 
        myself = server.cluster->myself =
            createClusterNode(((void *)0),16|1);
        serverLog(2,"No cluster configuration found, I'm %.40s",
            myself->name);
    ...
    if (server.port > (65535-10000)) {
        serverLog(3, "Redis port number too high. "
    ...
    if (listenToPort(server.port+10000,
        server.cfd,&server.cfd_count) == -1)
    {
        exit(1);
    } else {
        int j;
        ...
        for (j = 0; j < server.cfd_count; j++) {
            if (aeCreateFileEvent(server.el, server.cfd[j], 1,
                clusterAcceptHandler, ((void *)0)) == -1)
                    _serverPanic("cluster.c",499,"Unrecoverable error creating Redis Cluster " "file event."),_exit(1);
    ...
    myself->cport = server.port+10000;
...
void clusterReset(int hard) {
    ...
    for (j = 0; j < 16384; j++) clusterDelSlot(j);
    ...
    if (hard) {
        ...
        serverLog(3, "configEpoch set to 0 via CLUSTER RESET HARD");

        
 
        oldname = sdsnewlen(myself->name, 40);
        dictDelete(server.cluster->nodes,oldname);
        sdsfree(oldname);
        getRandomHexChars(myself->name, 40);
        clusterAddNode(myself);
        serverLog(2,"Node hard reset, now I'm %.40s", myself->name);
    }
...
void freeClusterLink(clusterLink *link) {
    if (link->fd != -1) {
        aeDeleteFileEvent(server.el, link->fd, 1|2);
    }
    sdsfree(link->sndbuf);
    sdsfree(link->rcvbuf);
    if (link->node)
...
void clusterAcceptHandler(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    while(max--) {
        ...
        serverLog(1,"Accepted cluster node %s:%d", cip, cport);
...
int clusterCountNonFailingSlaves(clusterNode *n) {
    ...
    for (j = 0; j < n->numslaves; j++)
        if (!((n->slaves[j])->flags & 8)) okslaves++;
    return okslaves;
}
...
void freeClusterNode(clusterNode *n) {
    ...
    for (j = 0; j < n->numslaves; j++)
        n->slaves[j]->slaveof = ((void *)0);

     
    if (((n)->flags & 2) && n->slaveof) clusterNodeRemoveSlave(n->slaveof,n);

     
    nodename = sdsnewlen(n->name, 40);
    ((dictDelete(server . cluster->nodes,nodename) == 0)?(void)0 : (_serverAssert("dictDelete(server.cluster->nodes,nodename) == DICT_OK","cluster.c",875),_exit(1)));
    sdsfree(nodename);
...
void clusterRenameNode(clusterNode *node, char *newname) {
    ...
    serverLog(0,"Renaming node %.40s into %.40s",
        node->name, newname);
    retval = dictDelete(server.cluster->nodes, s);
    sdsfree(s);
    ((retval == 0)?(void)0 : (_serverAssert("retval == DICT_OK","cluster.c",957),_exit(1)));
...
void clusterHandleConfigEpochCollision(clusterNode *sender) {
    ...
    if (sender->configEpoch != myself->configEpoch ||
        !((sender)->flags & 1) || !((myself)->flags & 1)) return;
     
    if (memcmp(sender->name,myself->name,40) <= 0) return;
...
void clusterBlacklistAddNode(clusterNode *node) {
    dictEntry *de;
    sds id = sdsnewlen(node->name,40);

    clusterBlacklistCleanup();
    if (dictAdd(server.cluster->nodes_black_list,id,((void *)0)) == 0) {
        
 
        id = sdsdup(id);
    }
    de = dictFind(server.cluster->nodes_black_list,id);
    do { (de)->v . u64 = time(((void *)0))+60; } while(0);
    sdsfree(id);
}
...
void markNodeAsFailingIfNeeded(clusterNode *node) {
    ...
    if (!((node)->flags & 4)) return;  
    if (((node)->flags & 8)) return;  

    failures = clusterNodeFailureReportsCount(node);
     
    if (((myself)->flags & 1)) failures++;
    ...
    if (((myself)->flags & 1)) clusterSendFail(node->name);
    clusterDoBeforeSleep((1<<1)|(1<<2));
}
...
void clearNodeFailureIfNeeded(clusterNode *node) {
    ...
    ((((node)->flags & 8))?(void)0 : (_serverAssert("nodeFailed(node)","cluster.c",1225),_exit(1)));

    
 
    if (((node)->flags & 2) || node->numslots == 0) {
        serverLog(2,
            "Clear FAIL state for node %.40s: %s is reachable again.",
                node->name,
                ((node)->flags & 2) ? "replica" : "master without slots");
        node->flags &= ~8;
        clusterDoBeforeSleep((1<<1)|(1<<2));
    }
    ...
    if (((node)->flags & 1) && node->numslots > 0 &&
        (now - node->fail_time) >
...
void clusterProcessGossipSection(clusterMsg *hdr, clusterLink *link) {
    ...
    while(count--) {
        ...
        if (server.verbosity == 0) {
            ...
            serverLog(0,"GOSSIP %.40s %s:%d@%d %s",
                g->nodename,
                g->ip,
                ntohs(g->port),
                ntohs(g->cport),
        ...
        if (node) {
            ...
            if (sender && ((sender)->flags & 1) && node != myself) {
                if (flags & (8|4)) {
...
void nodeIp2String(char *buf, clusterLink *link, char *announced_ip) {
    if (announced_ip[0] != '\0') {
        memcpy(buf,announced_ip,46);
        buf[46-1] = '\0';  
    } else {
        anetPeerToString(link->fd, buf, 46, ((void *)0));
    }
}
                              ...
                              clusterMsg *hdr)
{
    ...
    if (node->link) freeClusterLink(node->link);
    node->flags &= ~64;
    serverLog(3,"Address updated for node %.40s, now %s:%d",
        node->name, node->ip, node->port);

    
 
    if (((myself)->flags & 2) && myself->slaveof == node)
...
void clusterUpdateSlotsConfigWith(clusterNode *sender, uint64_t senderConfigEpoch, unsigned char *slots) {
    ...
    uint16_t dirty_slots[16384];
    int dirty_slots_count = 0;

    

 
    curmaster = ((myself)->flags & 1) ? myself : myself->slaveof;

    if (sender == myself) {
        serverLog(3,"Discarding UPDATE message about myself.");
        return;
    }
    ...
    if (server.cluster_module_flags & (1<<2))
        return;
...
int clusterProcessPacket(clusterLink *link) {
    ...
    serverLog(0,"--- Processing packet of type %d, %lu bytes",
    ...
    if (sender && !((sender)->flags & 32)) {
        ...
        sender->repl_offset = intrev64(hdr->offset);
    ...
    if (type == 0 || type == 2) {
        serverLog(0,"Ping packet received: %p", (void*)link->node);
        ...
        if ((type == 2 || myself->ip[0] == '\0') &&
            server.cluster_announce_ip == ((void *)0))
        {
            char ip[46];

            if (anetSockName(link->fd,ip,sizeof(ip),((void *)0)) != -1 &&
                strcmp(ip,myself->ip))
            {
                memcpy(myself->ip,ip,46);
                serverLog(3,"IP address for this node updated to %s",
                    myself->ip);
                clusterDoBeforeSleep((1<<2));
            }
        }
    ...
    if (type == 0 || type == 1 ||
        type == 2)
    {
        serverLog(0,"%s packet received: %p",
            type == 0 ? "ping" : "pong",
            (void*)link->node);
        if (link->node) {
            if (((link->node)->flags & 32)) {
                ...
                serverLog(0,"Handshake with node %.40s completed.",
                    link->node->name);
                link->node->flags &= ~32;
                link->node->flags |= flags&(1|2);
                clusterDoBeforeSleep((1<<2));
                        ...
                        40) != 0)
            {
                ...
                serverLog(0,"PONG contains mismatching sender ID. About node %.40s added %d ms ago, having flags %d",
                    link->node->name,
        ...
        if (link->node && type == 1) {
            ...
            if (((link->node)->flags & 4)) {
                link->node->flags &= ~4;
                clusterDoBeforeSleep((1<<2)|
                                     (1<<1));
            } else if (((link->node)->flags & 8)) {
        ...
        clusterNode *sender_master = ((void *)0);  
        int dirty_slots = 0;  

        if (sender) {
            sender_master = ((sender)->flags & 1) ? sender : sender->slaveof;
            if (sender_master) {
        ...
        if (sender && ((sender)->flags & 1) && dirty_slots)
            ...
            for (j = 0; j < 16384; j++) {
                if (bitmapTestBit(hdr->myslots,j)) {
                    if (server.cluster->slots[j] == sender ||
                        server.cluster->slots[j] == ((void *)0)) continue;
        ...
        if (((server . pubsub_channels)->ht[0]. used+(server . pubsub_channels)->ht[1]. used) ||
        ...
        if (((sender)->flags & 1) && sender->numslots > 0 &&
            senderCurrentEpoch >= server.cluster->failover_auth_epoch)
        {
            server.cluster->failover_auth_count++;
            
 
            clusterDoBeforeSleep((1<<0));
        }
    } else if (type == 8) {
        ...
        resetManualFailover();
        server.cluster->mf_end = mstime() + 5000;
        server.cluster->mf_slave = sender;
        pauseClients(mstime()+(5000*2));
        serverLog(3,"Manual failover requested by replica %.40s.",
            sender->name);
    } else if (type == 7) {
        clusterNode *n;  
        uint64_t reportedConfigEpoch =
                    intrev64(hdr->data . update . nodecfg . configEpoch);
        ...
        if (((n)->flags & 2)) clusterSetNodeAsMaster(n);
        ...
        serverLog(3,"Received unknown packet type: %d", type);
    }
    return 1;
}
...
void clusterWriteHandler(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    if (nwritten <= 0) {
        serverLog(0,"I/O error writing to node link: %s",
...
void clusterReadHandler(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    while(1) {  
        ...
        if (rcvbuflen < 8) {
            ...
            if (rcvbuflen == 8) {
                ...
                if (memcmp(hdr->sig,"RCmb",4) != 0 ||
                    ntohl(hdr->totlen) < (sizeof(clusterMsg)-sizeof(union clusterMsgData)))
                {
                    serverLog(3,
        ...
        if (nread == -1 && errno == 11) return;  

        if (nread <= 0) {
             
            serverLog(0,"I/O error reading from node link: %s",
...
void clusterSendMessage(clusterLink *link, unsigned char *msg, size_t msglen) {
    if (sdslen(link->sndbuf) == 0 && msglen != 0)
        aeCreateFileEvent(server.el,link->fd,2|4,
...
void clusterBuildMessageHdr(clusterMsg *hdr, int type) {
    ...
    master = (((myself)->flags & 2) && myself->slaveof) ?
    ...
    hdr->sig[3] = 'b';
    hdr->type = htons(type);
    memcpy(hdr->sender,myself->name,40);

    

 
    memset(hdr->myip,0,46);
    if (server.cluster_announce_ip) {
        strncpy(hdr->myip,server.cluster_announce_ip,46);
        hdr->myip[46-1] = '\0';
    }
    ...
    if (myself->slaveof != ((void *)0))
        memcpy(hdr->slaveof,myself->slaveof->name, 40);
    ...
    if (((myself)->flags & 1) && server.cluster->mf_end)
        hdr->mflags[0] |= (1<<0);
...
void clusterSetGossipEntry(clusterMsg *hdr, int i, clusterNode *n) {
    ...
    memcpy(gossip->nodename,n->name,40);
...
void clusterSendPing(clusterLink *link, int type) {
    ...
    wanted = floor(((server . cluster->nodes)->ht[0]. used+(server . cluster->nodes)->ht[1]. used)/10);
...
void clusterBroadcastPong(int target) {
    ...
    while((de = dictNext(di)) != ((void *)0)) {
        ...
        if (!node->link) continue;
        if (node == myself || ((node)->flags & 32)) continue;
        if (target == 1) {
            int local_slave =
                ((node)->flags & 2) && node->slaveof &&
                (node->slaveof == myself || node->slaveof == myself->slaveof);
            if (!local_slave) continue;
        }
        clusterSendPing(node->link,1);
    }
    dictReleaseIterator(di);
}
...
void clusterSendUpdate(clusterLink *link, clusterNode *node) {
    ...
    clusterBuildMessageHdr(hdr,7);
    memcpy(hdr->data.update.nodecfg.nodename,node->name,40);
...
int clusterSendModuleMessageToTarget(const char *target, uint64_t module_id, uint8_t type, unsigned char *payload, uint32_t len) {
    ...
    if (target != ((void *)0)) {
        ...
        if (node == ((void *)0) || node->link == ((void *)0)) return -1;
    }
...
void clusterRequestFailoverAuth(void) {
    ...
    clusterBuildMessageHdr(hdr,5);
    

 
    if (server.cluster->mf_end) hdr->mflags[0] |= (1<<1);
    totlen = sizeof(clusterMsg)-sizeof(union clusterMsgData);
    hdr->totlen = htonl(totlen);
...
void clusterSendFailoverAuthIfNeeded(clusterNode *node, clusterMsg *request) {
    ...
    uint64_t requestCurrentEpoch = intrev64(request->currentEpoch);
    uint64_t requestConfigEpoch = intrev64(request->configEpoch);
    unsigned char *claimed_slots = request->myslots;
    int force_ack = request->mflags[0] & (1<<1);
    int j;
    ...
    if (((myself)->flags & 2) || myself->numslots == 0) return;
    ...
    if (((node)->flags & 1) || master == ((void *)0) ||
        (!((master)->flags & 8) && !force_ack))
    {
        if (((node)->flags & 1)) {
            serverLog(3,
    ...
    clusterDoBeforeSleep((1<<2)|(1<<3));
    clusterSendFailoverAuth(node);
    serverLog(3, "Failover auth granted to %.40s for epoch %llu",
...
int clusterGetSlaveRank(void) {
    ...
    ((((myself)->flags & 2))?(void)0 : (_serverAssert("nodeIsSlave(myself)","cluster.c",2826),_exit(1)));
...
void clusterLogCantFailover(int reason) {
    ...
    serverLog(3,"Currently unable to failover: %s", msg);
}
...
void clusterFailoverReplaceYourMaster(void) {
    ...
    if (((myself)->flags & 1) || oldmaster == ((void *)0)) return;
...
void clusterHandleSlaveFailover(void) {
    ...
    server.cluster->todo_before_sleep &= ~(1<<0);
    ...
    if (((myself)->flags & 1) ||
        ...
        (!((myself->slaveof)->flags & 8) && !manual_failover) ||
    ...
    if (mstime() < server.cluster->failover_auth_time) {
        clusterLogCantFailover(2);
        return;
    }

     
    if (auth_age > auth_timeout) {
        clusterLogCantFailover(3);
        return;
    }
    ...
    if (server.cluster->failover_auth_sent == 0) {
        ...
        serverLog(3,"Starting a failover election for epoch %llu.",
            (unsigned long long) server.cluster->currentEpoch);
...
void clusterHandleSlaveMigration(int max_slaves) {
    ...
    if (server.cluster->state != 0) return;

    
 
    if (mymaster == ((void *)0)) return;
    for (j = 0; j < mymaster->numslaves; j++)
        if (!((mymaster->slaves[j])->flags & 8) &&
            !((mymaster->slaves[j])->flags & 4)) okslaves++;
    ...
    while((de = dictNext(di)) != ((void *)0)) {
        ...
        if (((node)->flags & 2) || ((node)->flags & 8)) is_orphaned = 0;
        if (!(node->flags & 256)) is_orphaned = 0;

         
        if (((node)->flags & 1)) okslaves = clusterCountNonFailingSlaves(node);
    ...
    if (target && candidate == myself &&
        (mstime()-target->orphaned_time) > 5000 &&
       !(server.cluster_module_flags & (1<<1)))
    {
        serverLog(3,"Migrating to orphaned master %.40s",
            target->name);
...
void manualFailoverCheckTimeout(void) {
    if (server.cluster->mf_end && server.cluster->mf_end < mstime()) {
        serverLog(3,"Manual failover timed out.");
        resetManualFailover();
    }
}
...
void clusterCron(void) {
 ...
 
    {
        ...
        if (changed) {
            ...
            if (curr_ip) {
                ...
                strncpy(myself->ip,server.cluster_announce_ip,46);
                myself->ip[46-1] = '\0';
            } else {
                myself->ip[0] = '\0';  
            }
    ...
    while((de = dictNext(di)) != ((void *)0)) {
        ...
        if (((node)->flags & 32) && now - node->ctime > handshake_timeout) {
            clusterDelNode(node);
            continue;
        }
        ...
        if (node->link == ((void *)0)) {
            ...
            fd = anetTcpNonBlockBindConnect(server.neterr, node->ip,
                node->cport, (server . bindaddr_count ? server . bindaddr[0] : ((void *)0)));
            if (fd == -1) {
                ...
                serverLog(0, "Unable to connect to "
            ...
            link->fd = fd;
            node->link = link;
            aeCreateFileEvent(server.el,link->fd,1,
            ...
            serverLog(0,"Connecting with Node %.40s at %s:%d",
    ...
    if (!(iteration % 10)) {
        ...
        if (min_pong_node) {
            serverLog(0,"Pinging node %.40s", min_pong_node->name);
            clusterSendPing(min_pong_node->link, 0);
        }
    }
    ...
    while((de = dictNext(di)) != ((void *)0)) {
        ...
        if (((myself)->flags & 2) && ((node)->flags & 1) && !((node)->flags & 8)) {
        ...
        if (delay > server.cluster_node_timeout) {
            ...
            if (!(node->flags & (4|8))) {
                serverLog(0,"*** NODE %.40s possibly failing",
                    node->name);
                node->flags |= 4;
                update_state = 1;
            }
    ...
    if (((myself)->flags & 2)) {
        ...
        if (!(server.cluster_module_flags & (1<<1)))
    ...
    if (update_state || server.cluster->state == 1)
        clusterUpdateState();
}
...
void clusterBeforeSleep(void) {
    ...
    if (server.cluster->todo_before_sleep & (1<<0))
        clusterHandleSlaveFailover();

     
    if (server.cluster->todo_before_sleep & (1<<1))
        clusterUpdateState();

     
    if (server.cluster->todo_before_sleep & (1<<2)) {
...
int clusterAddSlot(clusterNode *n, int slot) {
    if (server.cluster->slots[slot]) return -1;
...
int clusterDelSlot(int slot) {
    ...
    if (!n) return -1;
    ((clusterNodeClearSlotBit(n,slot) == 1)?(void)0 : (_serverAssert("clusterNodeClearSlotBit(n,slot) == 1","cluster.c",3719),_exit(1)));
...
void clusterUpdateState(void) {
    ...
    server.cluster->todo_before_sleep &= ~(1<<1);
    ...
    if (((myself)->flags & 1) &&
        server.cluster->state == 1 &&
        mstime() - first_call_time < 2000) return;
    ...
    if (new_state != server.cluster->state) {
        ...
        serverLog(3,"Cluster state changed: %s",
            new_state == 0 ? "ok" : "fail");
...
int verifyClusterConfigWithData(void) {
    ...
    if (((myself)->flags & 2)) return 0;

     
    for (j = 1; j < server.dbnum; j++) {
        if (((server . db[j]. dict)->ht[0]. used+(server . db[j]. dict)->ht[1]. used)) return -1;
    }
    ...
    for (j = 0; j < 16384; j++) {
        ...
        if (server.cluster->slots[j] == ((void *)0)) {
            serverLog(3, "I have keys for unassigned slot %d. "
                                    "Taking responsibility for it.",j);
            clusterAddSlot(myself,j);
        } else {
            serverLog(3, "I have keys for slot %d, but the slot is "
...
void clusterSetMaster(clusterNode *n) {
    ((n != myself)?(void)0 : (_serverAssert("n != myself","cluster.c",3933),_exit(1)));
    ((myself->numslots == 0)?(void)0 : (_serverAssert("myself->numslots == 0","cluster.c",3934),_exit(1)));
...
const char *clusterGetMessageTypeString(int type) {
    ...
    case 5: return "auth-req";
    case 6: return "auth-ack";
    case 7: return "update";
    case 8: return "mfstart";
...
void clusterReplyMultiBulkSlots(client *c) {
    ...
    while((de = dictNext(di)) != ((void *)0)) {
        ...
        if (!((node)->flags & 1) || node->numslots == 0) continue;
        ...
        for (j = 0; j < 16384; j++) {
            ...
            if (start != -1 && (!bit || j == 16384-1)) {
                ...
                addReplyBulkCBuffer(c, node->name, 40);

                 
                for (i = 0; i < node->numslaves; i++) {
                    
 
                    if (((node ->slaves[i])->flags & 8)) continue;
                    addReplyArrayLen(c, 3);
                    addReplyBulkCString(c, node->slaves[i]->ip);
                    addReplyLongLong(c, node->slaves[i]->port);
                    addReplyBulkCBuffer(c, node->slaves[i]->name, 40);
                    nested_elements++;
                }
...
((void *)0)
        ...
        if (getLongLongFromObject(c->argv[3], &port) != 0) {
        ...
        if (c->argc == 5) {
            if (getLongLongFromObject(c->argv[4], &cport) != 0) {
        ...
        addReplyBulkCBuffer(c,myself->name, 40);
    } else if (!strcasecmp(c->argv[1]->ptr,"slots") && c->argc == 2) {
         
        clusterReplyMultiBulkSlots(c);
    } else if (!strcasecmp(c->argv[1]->ptr,"flushslots") && c->argc == 2) {
         
        if (((server . db[0]. dict)->ht[0]. used+(server . db[0]. dict)->ht[1]. used) != 0) {
               ...
               !strcasecmp(c->argv[1]->ptr,"delslots")) && c->argc >= 3)
    {
        ...
        int j, slot;
        unsigned char *slots = zmalloc(16384);
        ...
        for (j = 0; j < 16384; j++) {
            if (slots[j]) {
                ...
                retval = del ? clusterDelSlot(j) :
                               clusterAddSlot(myself,j);
                ((retval == 0)?(void)0 : (_serverAssertWithInfo(c,((void *)0),"retval == C_OK","cluster.c",4319),_exit(1)));
            }
        }
        zfree(slots);
        clusterDoBeforeSleep((1<<1)|(1<<2));
        ...
        if (!strcasecmp(c->argv[3]->ptr,"migrating") && c->argc == 5) {
                ...
                server.cluster->importing_slots_from[slot])
            {
                ...
                if (clusterBumpConfigEpochWithoutConsensus() == 0) {
                    serverLog(3,
        ...
        clusterDoBeforeSleep((1<<2)|(1<<1));
        addReply(c,shared.ok);
    } else if (!strcasecmp(c->argv[1]->ptr,"bumpepoch") && c->argc == 2) {
         
        int retval = clusterBumpConfigEpochWithoutConsensus();
        sds reply = sdscatprintf(sdsempty(),"+%s %llu\r\n",
                (retval == 0) ? "BUMPED" : "STILL",
                (unsigned long long) myself->configEpoch);
        addReplySds(c,reply);
        ...
        myepoch = (((myself)->flags & 2) && myself->slaveof) ?
        ...
        if (getLongLongFromObjectOrReply(c,c->argv[2],&slot,((void *)0)) != 0)
            return;
        if (getLongLongFromObjectOrReply(c,c->argv[3],&maxkeys,((void *)0))
            != 0)
            return;
        ...
        if (!n) {
            ...
            return;
        } else if (((myself)->flags & 2) && myself->slaveof == n) {
            addReplyError(c,"Can't forget my master!");
            return;
        }
        clusterBlacklistAddNode(n);
        clusterDelNode(n);
        clusterDoBeforeSleep((1<<1)|
                             (1<<2));
        addReply(c,shared.ok);
        ...
        if (((myself)->flags & 1) &&
            (myself->numslots != 0 || ((server . db[0]. dict)->ht[0]. used+(server . db[0]. dict)->ht[1]. used) != 0)) {
            addReplyError(c,
               ...
               (c->argc == 2 || c->argc == 3))
    {
        ...
        if (((myself)->flags & 1)) {
            ...
            return;
        } else if (myself->slaveof == ((void *)0)) {
            addReplyError(c,"I'm a replica but my master is unknown to me");
            return;
        } else if (!force &&
                   (((myself->slaveof)->flags & 8) ||
                    myself->slaveof->link == ((void *)0)))
        {
        ...
        if (takeover) {
            ...
            serverLog(3,"Taking over the master (user request).");
            ...
            serverLog(3,"Forced failover user request accepted.");
            server.cluster->mf_can_start = 1;
        } else {
            serverLog(3,"Manual failover user request accepted.");
    ...
    } else if (!strcasecmp(c->argv[1]->ptr,"set-config-epoch") && c->argc == 3)
    {
        ...
        if (getLongLongFromObjectOrReply(c,c->argv[2],&epoch,((void *)0)) != 0)
            return;

        if (epoch < 0) {
            addReplyErrorFormat(c,"Invalid config epoch specified: %lld",epoch);
        } else if (((server . cluster->nodes)->ht[0]. used+(server . cluster->nodes)->ht[1]. used) > 1) {
               ...
               (c->argc == 2 || c->argc == 3))
    {
        ...
        if (((myself)->flags & 1) && ((c->db->dict)->ht[0]. used+(c->db->dict)->ht[1]. used) != 0) {
...
void createDumpPayload(rio *payload, robj *o, robj *key) {
    ...
    ((rdbSaveObjectType(payload,o))?(void)0 : (_serverAssert("rdbSaveObjectType(payload,o)","cluster.c",4786),_exit(1)));
    ((rdbSaveObject(payload,o,key))?(void)0 : (_serverAssert("rdbSaveObject(payload,o,key)","cluster.c",4787),_exit(1)));
...
void restoreCommand(client *c) {
    ...
    if (getLongLongFromObjectOrReply(c,c->argv[2],&ttl,((void *)0)) != 0) {
        return;
    } else if (ttl < 0) {
...
migrateCachedSocket* migrateGetSocket(client *c, robj *host, robj *port, long timeout) {
    ...
    if (((server . migrate_cached_sockets)->ht[0]. used+(server . migrate_cached_sockets)->ht[1]. used) == 64) {
...
void migrateCloseTimedoutSockets(void) {
    ...
    while((de = dictNext(di)) != ((void *)0)) {
        ...
        if ((server.unixtime - cs->last_use_time) > 10) {
            close(cs->fd);
            zfree(cs);
            dictDelete(server.migrate_cached_sockets,((de)->key));
        }
...
void migrateCommand(client *c) {
    ...
    if (getLongFromObjectOrReply(c,c->argv[5],&timeout,((void *)0)) != 0 ||
        getLongFromObjectOrReply(c,c->argv[4],&dbid,((void *)0)) != 0)
    {
        return;
    }
...
try_again:
    ...
    if (password) {
        ((rioWriteBulkCount(&cmd,'*',2))?(void)0 : (_serverAssertWithInfo(c,((void *)0),"rioWriteBulkCount(&cmd,'*',2)","cluster.c",5154),_exit(1)));
        ((rioWriteBulkString(&cmd,"AUTH",4))?(void)0 : (_serverAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,\"AUTH\",4)","cluster.c",5155),_exit(1)));
        ((rioWriteBulkString(&cmd,password, sdslen(password)))?(void)0 : (_serverAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,password, sdslen(password))","cluster.c",5157),_exit(1)));
    ...
    if (select) {
        ((rioWriteBulkCount(&cmd,'*',2))?(void)0 : (_serverAssertWithInfo(c,((void *)0),"rioWriteBulkCount(&cmd,'*',2)","cluster.c",5163),_exit(1)));
        ((rioWriteBulkString(&cmd,"SELECT",6))?(void)0 : (_serverAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,\"SELECT\",6)","cluster.c",5164),_exit(1)));
        ((rioWriteBulkLongLong(&cmd,dbid))?(void)0 : (_serverAssertWithInfo(c,((void *)0),"rioWriteBulkLongLong(&cmd,dbid)","cluster.c",5165),_exit(1)));
    }
    ...
    for (j = 0; j < num_keys; j++) {
        ...
        ((rioWriteBulkCount(&cmd,'*',replace ? 5 : 4))?(void)0 : (_serverAssertWithInfo(c,((void *)0),"rioWriteBulkCount(&cmd,'*',replace ? 5 : 4)","cluster.c",5192),_exit(1)));


        if (server.cluster_enabled)
            ((rioWriteBulkString(&cmd,"RESTORE-ASKING",14))?(void)0 : (_serverAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,\"RESTORE-ASKING\",14)","cluster.c",5196),_exit(1)));

        else
            ((rioWriteBulkString(&cmd,"RESTORE",7))?(void)0 : (_serverAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,\"RESTORE\",7)","cluster.c",5198),_exit(1)));
        (((kv[j]->encoding == 0 || kv[j]->encoding == 8))?(void)0 : (_serverAssertWithInfo(c,((void *)0),"sdsEncodedObject(kv[j])","cluster.c",5199),_exit(1)));
        ((rioWriteBulkString(&cmd,kv[j]->ptr, sdslen(kv[j]->ptr)))?(void)0 : (_serverAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,kv[j]->ptr, sdslen(kv[j]->ptr))","cluster.c",5201),_exit(1)));

        ((rioWriteBulkLongLong(&cmd,ttl))?(void)0 : (_serverAssertWithInfo(c,((void *)0),"rioWriteBulkLongLong(&cmd,ttl)","cluster.c",5202),_exit(1)));

        
 
        createDumpPayload(&payload,ov[j],kv[j]);
        ((rioWriteBulkString(&cmd,payload . io . buffer . ptr, sdslen(payload . io . buffer . ptr)))?(void)0 : (_serverAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,payload.io.buffer.ptr, sdslen(payload.io.buffer.ptr))","cluster.c",5209),_exit(1)));
        ...
        if (replace)
            ((rioWriteBulkString(&cmd,"REPLACE",7))?(void)0 : (_serverAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,\"REPLACE\",7)","cluster.c",5215),_exit(1)));
    }
...
clusterNode *getNodeByQuery(client *c, struct redisCommand *cmd, robj **argv, int argc, int *hashslot, int *error_code) {
    ...
    if (c->flags & (1<<17) &&
        (cmd->flags & (1ULL<<1) || cmd->proc == evalCommand ||
    ...
    if (n != myself && error_code) *error_code = 4;
    return n;
}
...
void clusterRedirectClient(client *c, clusterNode *n, int hashslot, int error_code) {
               ...
               error_code == 3)
    {
        addReplySds(c,sdscatprintf(sdsempty(),
            "-%s %d %s:%d\r\n",
            (error_code == 3) ? "ASK" : "MOVED",
            hashslot,n->ip,n->port));
    } else {
        _serverPanic("cluster.c",5651,"getNodeByQuery() unknown error."),_exit(1);
    }
}
...
int clusterRedirectBlockedClientIfNeeded(client *c) {
         ...
         c->btype == 4))
    {
        ...
        if (server.cluster->state == 1) {
            clusterRedirectClient(c,((void *)0),0,5);
            return 1;
        }
