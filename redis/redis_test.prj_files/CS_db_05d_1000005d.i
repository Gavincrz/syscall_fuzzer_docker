...
robj *lookupKeyReadWithFlags(redisDb *db, robj *key, int flags) {
    ...
    if (expireIfNeeded(db,key) == 1) {
        ...
        if (server.masterhost == ((void *)0)) {
            ...
            notifyKeyspaceEvent((1<<11), "keymiss", key, db->id);
            return ((void *)0);
        }
        ...
        if (server.current_client &&
            ...
            server.current_client->cmd->flags & (1ULL<<1))
        {
            server.stat_keyspace_misses++;
            notifyKeyspaceEvent((1<<11), "keymiss", key, db->id);
            return ((void *)0);
...
robj *lookupKeyRead(redisDb *db, robj *key) {
    return lookupKeyReadWithFlags(db,key,0);
}
...
void dbAdd(redisDb *db, robj *key, robj *val) {
    ...
    ((retval == 0)?(void)0 : (_serverAssertWithInfo(((void *)0),key,"retval == DICT_OK","db.c",182),_exit(1)));
    if (val->type == 1 ||
        val->type == 3)
...
void dbOverwrite(redisDb *db, robj *key, robj *val) {
    ...
    ((de != ((void *)0))?(void)0 : (_serverAssertWithInfo(((void *)0),key,"de != NULL","db.c",197),_exit(1)));
    dictEntry auxentry = *de;
    robj *old = ((de)->v . val);
    if (server.maxmemory_policy & (1<<1)) {
        val->lru = old->lru;
    }
    do { if ((db->dict)->type ->valDup) (de)->v . val = (db->dict)->type ->valDup((db->dict)->privdata, val); else (de)->v . val = (val); } while(0);

    if (server.lazyfree_lazy_server_del) {
        freeObjAsync(old);
        do { if ((db->dict)->type ->valDup) (&auxentry)->v . val = (db->dict)->type ->valDup((db->dict)->privdata, ((void *)0)); else (&auxentry)->v . val = (((void *)0)); } while(0);
    }

    if ((db->dict)->type ->valDestructor) (db->dict)->type ->valDestructor((db->dict)->privdata, (&auxentry)->v . val);
}
...
robj *dbRandomKey(redisDb *db) {
    dictEntry *de;
    int maxtries = 100;
    int allvolatile = ((db->dict)->ht[0]. used+(db->dict)->ht[1]. used) == ((db->expires)->ht[0]. used+(db->expires)->ht[1]. used);
...
int dbSyncDelete(redisDb *db, robj *key) {
    ...
    if (((db->expires)->ht[0]. used+(db->expires)->ht[1]. used) > 0) dictDelete(db->expires,key->ptr);
    if (dictDelete(db->dict,key->ptr) == 0) {
...
robj *dbUnshareStringValue(redisDb *db, robj *key, robj *o) {
    ((o->type == 0)?(void)0 : (_serverAssert("o->type == OBJ_STRING","db.c",323),_exit(1)));
    if (o->refcount != 1 || o->encoding != 0) {
...
long long emptyDb(int dbnum, int flags, void(callback)(void*)) {
    ...
    for (int j = startdb; j <= enddb; j++) {
        removed += ((server . db[j]. dict)->ht[0]. used+(server . db[j]. dict)->ht[1]. used);
        if (async) {
...
void flushdbCommand(client *c) {
    ...
    if (getFlushCommandFlags(c,&flags) == -1) return;
...
void scanCallback(void *privdata, const dictEntry *de) {
    ...
    if (o == ((void *)0)) {
        ...
        sds sdskey = ((de)->key);
        key = createStringObject(sdskey,sdslen(sdskey));
        val = createStringObjectFromLongDouble(*(double*)((de)->v . val),0);
    } else {
        _serverPanic("db.c",589,"Type not handled in SCAN callback."),_exit(1);
    }
...
int parseScanCursorOrReply(client *c, robj *o, unsigned long *cursor) {
    ...
    *cursor = strtoul(o->ptr, &eptr, 10);
    if (isspace(((char*)o->ptr)[0]) || eptr[0] != '\0' || errno == 34)
    {
...
void scanGenericCommand(client *c, robj *o, unsigned long cursor) {
    ...
    ((o == ((void *)0) || o->type == 2 || o->type == 4 || o->type == 3)?(void)0 : (_serverAssert("o == NULL || o->type == OBJ_SET || o->type == OBJ_HASH || o->type == OBJ_ZSET","db.c",638),_exit(1)));
    ...
    while (i < c->argc) {
        ...
        if (!strcasecmp(c->argv[i]->ptr, "count") && j >= 2) {
            if (getLongFromObjectOrReply(c, c->argv[i+1], &count, ((void *)0))
                != 0)
            {
                goto cleanup;
            }
    ...
    if (ht) {
        ...
        privdata[1] = o;
        do {
            cursor = dictScan(ht, cursor, scanCallback, ((void *)0), privdata);
        } while (cursor &&
              maxiterations-- &&
              ((keys)->len) < (unsigned long)count);
    } else if (o->type == 2) {
        int pos = 0;
        int64_t ll;
        ...
        while(p) {
            ...
            listAddNodeTail(keys,
                (vstr != ((void *)0)) ? createStringObject((char*)vstr,vlen) :
                                 createStringObjectFromLongLong(vll));
            p = ziplistNext(o->ptr,p);
        }
        cursor = 0;
    } else {
        _serverPanic("db.c",737,"Not handled encoding in SCAN."),_exit(1);
    }
    ...
    while (node) {
        ...
        if (!filter && use_pattern) {
            if ((kobj->encoding == 0 || kobj->encoding == 8)) {
                ...
                ((kobj->encoding == 1)?(void)0 : (_serverAssert("kobj->encoding == OBJ_ENCODING_INT","db.c",756),_exit(1)));
        ...
        if (!filter && o == ((void *)0) && expireIfNeeded(c->db, kobj)) filter = 1;
...
void scanCommand(client *c) {
    ...
    if (parseScanCursorOrReply(c,c->argv[1],&cursor) == -1) return;
    scanGenericCommand(c,((void *)0),cursor);
}

void dbsizeCommand(client *c) {
    addReplyLongLong(c,((c->db->dict)->ht[0]. used+(c->db->dict)->ht[1]. used));
}
...
void typeCommand(client *c) {
    ...
    o = lookupKeyReadWithFlags(c->db,c->argv[1],(1<<0));
    if (o == ((void *)0)) {
        type = "none";
    } else {
        switch(o->type) {
        ...
        case 3: type = "zset"; break;
        case 4: type = "hash"; break;
        case 6: type = "stream"; break;
        case 5: {
...
void shutdownCommand(client *c) {
    ...
    if (server.loading || server.sentinel_mode)
        flags = (flags & ~1) | 2;
    if (prepareForShutdown(flags) == 0) exit(0);
...
void renameGenericCommand(client *c, int nx) {
    ...
    notifyKeyspaceEvent((1<<2),"rename_from",
        c->argv[1],c->db->id);
    notifyKeyspaceEvent((1<<2),"rename_to",
        c->argv[2],c->db->id);
    server.dirty++;
...
void moveCommand(client *c) {
    ...
    if (getLongLongFromObject(c->argv[2],&dbid) == -1 ||
        dbid < (-0x7fffffff - 1) || dbid > 0x7fffffff ||
        selectDb(c,dbid) == -1)
    {
...
void scanDatabaseForReadyLists(redisDb *db) {
    ...
    while((de = dictNext(di)) != ((void *)0)) {
        robj *key = ((de)->key);
        robj *value = lookupKey(db,key,(1<<0));
...
int dbSwapDatabases(int id1, int id2) {
    if (id1 < 0 || id1 >= server.dbnum ||
        id2 < 0 || id2 >= server.dbnum) return -1;
    if (id1 == id2) return 0;
...
void swapdbCommand(client *c) {
    ...
    if (getLongFromObjectOrReply(c, c->argv[1], &id1,
        "invalid first DB index") != 0)
        return;

    if (getLongFromObjectOrReply(c, c->argv[2], &id2,
        "invalid second DB index") != 0)
        return;

     
    if (dbSwapDatabases(id1,id2) == -1) {
        addReplyError(c,"DB index is out of range");
        return;
    } else {
        server.dirty++;
...
int removeExpire(redisDb *db, robj *key) {
    ...
    ((dictFind(db->dict,key->ptr) != ((void *)0))?(void)0 : (_serverAssertWithInfo(((void *)0),key,"dictFind(db->dict,key->ptr) != NULL","db.c",1070),_exit(1)));
    return dictDelete(db->expires,key->ptr) == 0;
}
...
void setExpire(client *c, redisDb *db, robj *key, long long when) {
    ...
    ((kde != ((void *)0))?(void)0 : (_serverAssertWithInfo(((void *)0),key,"kde != NULL","db.c",1083),_exit(1)));
    de = dictAddOrFind(db->expires,((kde)->key));
    do { (de)->v . s64 = when; } while(0);

    int writable_slave = server.masterhost && server.repl_slave_ro == 0;
    if (c && writable_slave && !(c->flags & (1<<1)))
...
long long getExpire(redisDb *db, robj *key) {
    ...
    if (((db->expires)->ht[0]. used+(db->expires)->ht[1]. used) == 0 ||
       (de = dictFind(db->expires,key->ptr)) == ((void *)0)) return -1;

    
 
    ((dictFind(db->dict,key->ptr) != ((void *)0))?(void)0 : (_serverAssertWithInfo(((void *)0),key,"dictFind(db->dict,key->ptr) != NULL","db.c",1103),_exit(1)));
    return ((de)->v . s64);
}
...
int *getKeysUsingCommandTable(struct redisCommand *cmd,robj **argv, int argc, int *numkeys) {
    ...
    for (j = cmd->firstkey; j <= last; j += cmd->keystep) {
        if (j >= argc) {
            ...
            if (cmd->flags & (1ULL<<3) || cmd->arity < 0) {
                zfree(keys);
                *numkeys = 0;
                return ((void *)0);
            } else {
                _serverPanic("db.c",1222,"Redis built-in command declared keys positions not matching the arity requirements."),_exit(1);
            }
        }
        keys[i++] = j;
    }
...
int *getKeysFromCommand(struct redisCommand *cmd, robj **argv, int argc, int *numkeys) {
    if (cmd->flags & (1ULL<<14)) {
        return moduleGetCommandKeysViaAPI(cmd,argv,argc,numkeys);
    } else if (!(cmd->flags & (1ULL<<3)) && cmd->getkeys_proc) {
