...
void openChildInfoPipe(void) {
    if (pipe(server.child_info_pipe) == -1) {
        ...
        closeChildInfoPipe();
    } else if (anetNonBlock(((void *)0),server.child_info_pipe[0]) != 0) {
        closeChildInfoPipe();
    } else {
...
void sendChildInfo(int ptype) {
    ...
    server.child_info_data.magic = 0xC17DDA7A12345678LL;
...
void receiveChildInfo(void) {
    ...
    if (read(server.child_info_pipe[0],&server.child_info_data,wlen) == wlen &&
        server.child_info_data.magic == 0xC17DDA7A12345678LL)
    {
        if (server.child_info_data.process_type == 0) {
            server.stat_rdb_cow_bytes = server.child_info_data.cow_size;
        } else if (server.child_info_data.process_type == 1) {
