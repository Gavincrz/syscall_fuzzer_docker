...
robj *lookupKeyByPattern(redisDb *db, robj *pattern, robj *subst, int writeflag) {
    ...
    if (fieldobj) {
        if (o->type != 4) goto noobj;

        
 
        o = hashTypeGetValueObject(o, fieldobj->ptr);
    } else {
        if (o->type != 0) goto noobj;
...
void sortCommand(client *c) {
    ...
    int syntax_error = 0;
    robj *sortval, *sortby = ((void *)0), *storekey = ((void *)0);
    ...
    while(j < c->argc) {
        ...
        if (!strcasecmp(c->argv[j]->ptr,"asc")) {
            ...
            if ((getLongFromObjectOrReply(c, c->argv[j+1], &limit_start, ((void *)0))
                 != 0) ||
                (getLongFromObjectOrReply(c, c->argv[j+2], &limit_count, ((void *)0))
                 != 0))
            {
                syntax_error++;
                break;
            ...
            if (strchr(c->argv[j+1]->ptr,'*') == ((void *)0)) {
                dontsort = 1;
            } else {
    ...
    if (sortval->type == 3)
        zsetConvert(sortval, 7);

     
    switch(sortval->type) {
    case 1: vectorlen = listTypeLength(sortval); break;
    case 2: vectorlen =  setTypeSize(sortval); break;
    case 3: vectorlen = ((((zset*)sortval->ptr)->dict)->ht[0]. used+(((zset*)sortval->ptr)->dict)->ht[1]. used); break;
    default: vectorlen = 0; _serverPanic("sort.c",320,"Bad SORT type"),_exit(1);  
    }
    ...
    if ((sortval->type == 3 || sortval->type == 1) &&
        dontsort &&
    ...
    if (sortval->type == 1 && dontsort) {
    ...
    } else if (sortval->type == 1) {
        listTypeIterator *li = listTypeInitIterator(sortval,0,1);
        listTypeEntry entry;
        ...
        listTypeReleaseIterator(li);
    } else if (sortval->type == 2) {
        setTypeIterator *si = setTypeInitIterator(sortval);
        sds sdsele;
        while((sdsele = setTypeNextObject(si)) != ((void *)0)) {
            vector[j].obj = createObject(0,sdsele);
            vector[j].u.score = 0;
            vector[j].u.cmpobj = ((void *)0);
            j++;
        }
        setTypeReleaseIterator(si);
    } else if (sortval->type == 3 && dontsort) {
        ...
        if (desc) {
            long zsetlen = ((((zset*)sortval->ptr)->dict)->ht[0]. used+(((zset*)sortval->ptr)->dict)->ht[1]. used);
        ...
        while(rangelen--) {
            ((ln != ((void *)0))?(void)0 : (_serverAssertWithInfo(c,sortval,"ln != NULL","sort.c",426),_exit(1)));
            sdsele = ln->ele;
        ...
        _serverPanic("sort.c",452,"Unknown type"),_exit(1);
    }
    ((j == vectorlen)?(void)0 : (_serverAssertWithInfo(c,sortval,"j == vectorlen","sort.c",454),_exit(1)));
    ...
    if (!dontsort) {
        for (j = 0; j < vectorlen; j++) {
            ...
            if (sortby) {
                ...
                byval = lookupKeyByPattern(c->db,sortby,vector[j].obj,storekey!=((void *)0));
                if (!byval) continue;
            } else {
            ...
            if (alpha) {
                ...
                if ((byval->encoding == 0 || byval->encoding == 8)) {
                    char *eptr;

                    vector[j].u.score = strtod(byval->ptr,&eptr);
                    if (eptr[0] != '\0' || errno == 34 ||
                        ( cs_isnan( (double) (vector[j]. u . score) ) ))
                    {
                        int_conversion_error = 1;
                    }
                } else if (byval->encoding == 1) {
                    

 
                    vector[j].u.score = (long)byval->ptr;
                } else {
                    ((1 != 1)?(void)0 : (_serverAssertWithInfo(c,sortval,"1 != 1","sort.c",487),_exit(1)));
                }
            }
    ...
    if (int_conversion_error) {
        ...
        for (j = start; j <= end; j++) {
            ...
            while((ln = listNext(&li))) {
                ...
                if (sop->type == 0) {
                    ...
                    ((sop->type == 0)?(void)0 : (_serverAssertWithInfo(c,sortval,"sop->type == SORT_OP_GET","sort.c",536),_exit(1)));
                }
            }
        }
    } else {
            ...
            if (!getop) {
                ...
                while((ln = listNext(&li))) {
                    ...
                    if (sop->type == 0) {
                        ...
                        listTypePush(sobj,val,1);
                        decrRefCount(val);
                    } else {
                         
                        ((sop->type == 0)?(void)0 : (_serverAssertWithInfo(c,sortval,"sop->type == SORT_OP_GET","sort.c",567),_exit(1)));
                    }
                }
            }
        }
        if (outputlen) {
            ...
            notifyKeyspaceEvent((1<<4),"sortstore",storekey,
                                c->db->id);
