...
slowlogEntry *slowlogCreateEntry(client *c, robj **argv, int argc, long long duration) {
    ...
    for (j = 0; j < slargc; j++) {
        ...
        if (slargc != argc && j == slargc-1) {
            se->argv[j] = createObject(0,
                sdscatprintf(sdsempty(),"... (%d more arguments)",
                argc-slargc+1));
        } else {
             
            if (argv[j]->type == 0 &&
                (argv[j]->encoding == 0 || argv[j]->encoding == 8) &&
                sdslen(argv[j]->ptr) > 128)
            {
                sds s = sdsnewlen(argv[j]->ptr, 128);

                s = sdscatprintf(s,"... (%lu more bytes)",
                    (unsigned long)
                    sdslen(argv[j]->ptr) - 128);
                se->argv[j] = createObject(0,s);
            } else if (argv[j]->refcount == 0x7fffffff) {
                se->argv[j] = argv[j];
            } else {
...
void slowlogInit(void) {
    ...
    ((server . slowlog)->free = (slowlogFreeEntry));
}
...
void slowlogPushEntryIfNeeded(client *c, robj **argv, int argc, long long duration) {
    ...
    while (((server . slowlog)->len) > server.slowlog_max_len)
        listDelNode(server.slowlog,((server . slowlog)->tail));
}

 
void slowlogReset(void) {
    while (((server . slowlog)->len) > 0)
        listDelNode(server.slowlog,((server . slowlog)->tail));
}
...
((void *)0)
        ...
        addReplyLongLong(c,((server . slowlog)->len));
               ...
               !strcasecmp(c->argv[1]->ptr,"get"))
    {
        ...
        if (c->argc == 3 &&
            getLongFromObjectOrReply(c,c->argv[2],&count,((void *)0)) != 0)
            return;
