...
void analyzeLatencyForEvent(char *event, struct latencyStats *ls) {
    ...
    if (ls->samples) {
        ...
        ls->period = time(((void *)0)) - ls->period;
...
sds createLatencyReport(void) {
    ...
    if (((server . latency_events)->ht[0]. used+(server . latency_events)->ht[1]. used) == 0 &&
    ...
    if (eventnum == 0 && advices == 0) {
        ...
        if (advise_relax_fsync_policy && server.aof_fsync == 1) {
...
void latencyCommandReplyWithLatestEvents(client *c) {
    ...
    addReplyArrayLen(c,((server . latency_events)->ht[0]. used+(server . latency_events)->ht[1]. used));
    di = dictGetIterator(server.latency_events);
    while((de = dictNext(di)) != ((void *)0)) {
        char *event = ((de)->key);
        struct latencyTimeSeries *ts = ((de)->v . val);
        int last = (ts->idx + 160 - 1) % 160;
...
sds latencyCommandGenSparkeline(char *event, struct latencyTimeSeries *ts) {
    ...
    graph = sparklineRender(graph,seq,80,4,1);
